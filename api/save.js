...
export default async function handler(req, res){
  try{
    ...
    // Read existing cloud to prevent older overwrites when NOT leader
    ...
    const existingTs = existing?.payload?.updatedAt
      ? Number(existing.payload.updatedAt)
      : (existing?.updated_at ? Date.parse(existing.updated_at) : 0);
