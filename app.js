...
const NW_MKT_BACKFILL_MAX_SPAN_MS = 60 * 24 * 60 * 60 * 1000; // max 60d backfill per run (interval auto-scales)
...
/* ================= MINI CHART ENGINE (NO CDN • ADBLOCK-SAFE) =================
   If Chart.js is blocked (CDN / AdBlock / DNS filters), we still render all charts.
   This is NOT full Chart.js; it's a minimal line-chart engine that supports the
   subset used by this dashboard (update/resize, plugins, crosshair, last-dot).
============================================================================= */
(function(){
  if (window.Chart) return;

  const _globalPlugins = [];
  const _pluginsById = Object.create(null);

  function normPad(p){
    if (!p) return {left:6,right:10,top:6,bottom:6};
    if (typeof p === "number") return {left:p,right:p,top:p,bottom:p};
    return {
      left:  Number(p.left)   || 6,
      right: Number(p.right)  || 10,
      top:   Number(p.top)    || 6,
      bottom:Number(p.bottom) || 6
    };
  }

  function evtXY(evt, canvas){
    ...
    if (evt && evt.touches && evt.touches[0]){
      ...
      cx = (typeof evt.clientX === "number") ? evt.clientX : 0;
      cy = (typeof evt.clientY === "number") ? evt.clientY : 0;
    }
    return { x: cx - rect.left, y: cy - rect.top, w: rect.width, h: rect.height };
  }

  function isObjPoint(v){ return v && typeof v === "object" && ("x" in v) && ("y" in v); }

  function SimpleChart(canvas, config){
    this.canvas = canvas;
    this.ctx = canvas.getContext("2d");
    this.config = config || {};
    this.data = (this.config.data) ? this.config.data : { labels: [], datasets: [{ data: [] }] };
    if (!this.data.datasets) this.data.datasets = [{ data: [] }];
    if (!this.data.datasets[0]) this.data.datasets[0] = { data: [] };

    this.options = this.config.options || {};
    this._localPlugins = Array.isArray(this.config.plugins) ? this.config.plugins : [];
    this._meta = { data: [] };
    this.chartArea = { left:0, right:0, top:0, bottom:0, width:0, height:0 };
    this.scales = { x: { min:0, max:0, getValueForPixel: (px)=>px }, y: { min:0, max:0 } };
  ...
  SimpleChart.register = function(...pls){
    for (const p of pls){
      if (!p || typeof p !== "object") continue;
      const id = String(p.id || "");
      if (id){
        _pluginsById[id] = p;
        // de-dup global array by id
        const has = _globalPlugins.some(x => x && x.id === id);
        if (!has) _globalPlugins.push(p);
      } else {
        _globalPlugins.push(p);
      }
    }
  };

  SimpleChart.registry = {
    plugins: {
      get: (id) => _pluginsById[String(id || "")]
    }
  };

  SimpleChart.prototype.getDatasetMeta = function(/*idx*/){
    return this._meta;
  };

  SimpleChart.prototype._resize = function(){
    const rect = this.canvas.getBoundingClientRect();
    const w = Math.max(10, rect.width || 10);
    const h = Math.max(10, rect.height || 10);
    const dpr = this._dpr;
  ...
  SimpleChart.prototype._buildScalesAndMeta = function(){
    const ds0 = this.data.datasets[0] || { data: [] };
    const arr = Array.isArray(ds0.data) ? ds0.data : [];
    const labels = Array.isArray(this.data.labels) ? this.data.labels : [];

    const xType = (this.options.scales && this.options.scales.x && this.options.scales.x.type) ? String(this.options.scales.x.type) : "";
    const yType = (this.options.scales && this.options.scales.y && this.options.scales.y.type) ? String(this.options.scales.y.type) : "linear";
    const isLinearX = (xType === "linear") || arr.some(isObjPoint);

    const pad = normPad(this.options.layout && this.options.layout.padding);
    const left = pad.left + 6;
    const top  = pad.top + 6;
    const right = (this._cssW - pad.right - 8);
    const bottom = (this._cssH - pad.bottom - 10);
    ...
    for (let i=0;i<arr.length;i++){
      const v = arr[i];
      const x = isObjPoint(v) ? Number(v.x) : (isLinearX ? Number(labels[i]) : i);
      const y = isObjPoint(v) ? Number(v.y) : Number(v);

      if (!Number.isFinite(x)) continue;

      if (yType === "logarithmic"){
        if (Number.isFinite(y) && y > 0){
          xs.push(x); ys.push(Math.log10(y));
        } else {
    ...
    // x range
    let xMin = 0, xMax = Math.max(1, xs.length - 1);
    if (isLinearX){
      let mn = Infinity, mx = -Infinity;
      for (const x of xs){ if (Number.isFinite(x)){ mn = Math.min(mn, x); mx = Math.max(mx, x); } }
      if (mn !== Infinity && mx !== -Infinity){
        xMin = mn; xMax = (mx === mn) ? (mx + 1) : mx;
      }
    } else {
      xMin = 0; xMax = Math.max(1, xs.length - 1);
    }
    ...
    const xToPx = (x) => {
      ...
      return ca.left + clamp(t, 0, 1) * ca.width;
    };
    const yToPxLin = (y) => {
      const t = (y - yMin) / (yMax - yMin);
      return ca.bottom - clamp(t, 0, 1) * ca.height;
    };
    ...
    for (let i=0;i<arr.length;i++){
      const v = arr[i];
      const x = isObjPoint(v) ? Number(v.x) : (isLinearX ? Number(labels[i]) : i);
      const yRaw = isObjPoint(v) ? Number(v.y) : Number(v);

      if (!Number.isFinite(x) || !Number.isFinite(yRaw)) { pts[i] = null; continue; }

      if (yType === "logarithmic"){
        if (yRaw <= 0) { pts[i] = null; continue; }
        const y = Math.log10(yRaw);
        pts[i] = { x: xToPx(x), y: yToPxLin(y), _x:x, _y:yRaw };
      } else {
        pts[i] = { x: xToPx(x), y: yToPxLin(yRaw), _x:x, _y:yRaw };
      }
    }
    ...
    this.scales = {
      x: {
        ...
        max: xMax,
        getValueForPixel: (px) => {
          // category charts expect index; linear expects x value
          const t = (px - ca.left) / (ca.width || 1);
          const v = xMin + clamp(t, 0, 1) * (xMax - xMin);
          return isLinearX ? v : (clamp(t, 0, 1) * Math.max(1, (arr.length - 1)));
        }
      },
      y: { min: yMin, max: yMax }
    };

    this._isLinearX = isLinearX;
    this._yType = yType;
  };

  SimpleChart.prototype._draw = function(){
    const ctx = this.ctx;
    const ds0 = this.data.datasets[0] || { data: [] };
    const arr = Array.isArray(ds0.data) ? ds0.data : [];
    ...
    this._buildScalesAndMeta();

    const ca = this.chartArea;

    // draw subtle grid (y only)
    const gridCol = (this.options.scales && this.options.scales.y && this.options.scales.y.grid && this.options.scales.y.grid.color) || "rgba(255,255,255,.08)";
    ctx.save();
    ...
    for (let i=1;i<=lines;i++){
      const y = ca.top + (ca.height * i / (lines+1));
    ...
    // dataset styling
    const stroke = ds0.borderColor || "#3b82f6";
    const fill = ds0.backgroundColor || "rgba(59,130,246,.14)";
    const lineW = Number(ds0.borderWidth) || 2;
    const doFill = !!ds0.fill;

    // path
    const pts = this._meta.data || [];
    ctx.save();
    ctx.lineWidth = lineW;
    ...
    if (doFill){
      ...
      if (started){
        // close to bottom
        const last = (()=>{ for(let i=pts.length-1;i>=0;i--){ if(pts[i]) return pts[i]; } return null; })();
        if (last){
    ...
    // plugin hooks
    const plugs = this._collectPlugins();
    for (const p of plugs){ try{ p.beforeDatasetsDraw && p.beforeDatasetsDraw(this); } catch{} }
    for (const p of plugs){ try{ p.afterDatasetsDraw && p.afterDatasetsDraw(this); } catch{} }
    for (const p of plugs){ try{ p.afterDraw && p.afterDraw(this); } catch{} }
  };

  SimpleChart.prototype._collectPlugins = function(){
    const out = [];
    const seen = new Set();
    const add = (p) => {
    ...
    _globalPlugins.forEach(add);
    this._localPlugins.forEach(add);
    return out;
  };

  SimpleChart.prototype.update = function(/*mode*/){
    this._draw();
  };

  SimpleChart.prototype.getElementsAtEventForMode = function(evt /*, mode, opts, useFinal*/){
    const pts = this._meta && this._meta.data ? this._meta.data : [];
    if (!pts.length) return [];

    const p = evtXY(evt, this.canvas);
    const x = p.x; // css px (ctx already scaled via transform)
    const ca = this.chartArea;

    // clamp to chart area
    const xx = clamp(x, ca.left, ca.right);
...
/* ================= BINANCE FALLBACK (ADBLOCK / DNS RESILIENCE) =================
   Some networks/extensions block api.binance.com. We try an alternate data domain too. */
...
async function fetchBinanceJSON(path){
  const p = String(path||"");
  if (!p.startsWith("/")) return fetchBinanceJSON("/" + p);
  let lastErr = null;
  ...
  if (lastErr) console.warn("[fetchBinanceJSON] all bases failed:", lastErr);
  return null;
}
...
// Reset ticker timing on tab hide/show to avoid jumpiness
document.addEventListener("visibilitychange", () => {
  try{
    if (!document.hidden && marketTickerAnim){
...
function initMarketTicker(){
  ...
  if (!marketTickerCfg){
    marketTickerCfg = [
      { key:"btcDom",   label:"BTC.D",     kind:"pct",        decimals:2, perfKey:"btcDom"   },
      { key:"mktCapUsd",label:"MKT CAP",   kind:"usdCompact", decimals:2, perfKey:"mktCap24h"},
      { key:"vol24hUsd",label:"VOL 24H",   kind:"usdCompact", decimals:2, perfKey:"vol24h"   },
      { key:"injOpen",  label:"INJ OPEN",  kind:"usd",        decimals:4, perfKey:null      },
      { key:"injPrice", label:"INJ PRICE", kind:"usd",        decimals:4, perfKey:"inj24h"   },
      { key:"injHigh",  label:"INJ HIGH",  kind:"usd",        decimals:4, perfKey:null      },
      { key:"injLow",   label:"INJ LOW",   kind:"usd",        decimals:4, perfKey:null      }
    ];
  }
...
function updateMarketInjTargets(){
  // keep INJ values "real-time" from the already-loaded candles/price feed
  const px = safe(targetPrice);
  if (Number.isFinite(px) && px > 0) marketTarget.injPrice = px;

  const o = tfReady?.d ? fin(candle?.d?.open) : marketTarget.injOpen;
  const h = tfReady?.d ? fin(candle?.d?.high) : marketTarget.injHigh;
  const l = tfReady?.d ? fin(candle?.d?.low)  : marketTarget.injLow;

  if (Number.isFinite(o) && o > 0) marketTarget.injOpen = o;
  if (Number.isFinite(h) && h > 0) marketTarget.injHigh = h;
  if (Number.isFinite(l) && l > 0) marketTarget.injLow  = l;

  const open = fin(marketTarget.injOpen);
  if (Number.isFinite(open) && open > 0 && Number.isFinite(px)) marketPerf.inj24h = pctChange(px, open);
}
...
function updateMarketTickerUI(){
  ...
  // smooth + render (do not introduce zeros)
  for (const it of marketTickerCfg){
    const key = it.key;
    const tgt = fin(marketTarget[key]);
    const cur = fin(marketTarget[key + "__disp"]);

    // store display on the same object (keeps scope small)
    let disp = Number.isFinite(cur) ? cur : NaN;
    const prev = disp;

    if (Number.isFinite(tgt) && tgt > 0){
      if (!Number.isFinite(disp)) disp = tgt;
      else disp = tick(disp, tgt);
      marketTarget[key + "__disp"] = disp;
      renderTickerValue(key, it.kind, disp, Number.isFinite(prev) ? prev : disp, it.decimals);
    }
    else if (Number.isFinite(disp)){
      // keep last displayed if provider temporarily missing
      renderTickerValue(key, it.kind, disp, disp, it.decimals);
    } else {
      // still unknown: leave em dash
    }
  ...
  // keep seamless wrap accurate if widths change (e.g., 999M -> 1.2B)
  if (marketTickerAnim?.compute){
...
function saveNWLocalSilent(){
  ...
  if (!nwOriginTs) nwOriginTs = safe(nwTAll?.[0]) || Date.now();
...
function startAllTimers(){
  ...
  restSyncTimer = setInterval(() => {
    ...
    safeAsync(() => loadMarketGlobal(false), "loadMarketGlobal");
  }, REST_SYNC_MS);
...
async function loadCandleSnapshot(isRefresh=false) {
  ...
  const [d, w, m, y] = await Promise.all([
    fetchBinanceJSON("/api/v3/klines?symbol=INJUSDT&interval=1d&limit=1"),
    fetchBinanceJSON("/api/v3/klines?symbol=INJUSDT&interval=1w&limit=1"),
    fetchBinanceJSON("/api/v3/klines?symbol=INJUSDT&interval=1M&limit=1"),
    fetchBinanceJSON("/api/v3/klines?symbol=INJUSDT&interval=1w&limit=52")
  ]);
  // ✅ network ok
  markLastOk();
...
async function fetchKlines1mRange(startTime, endTime) {
  ...
  while (cursor < end && out.length < DAY_MINUTES) {
    const urlPath = `/api/v3/klines?symbol=INJUSDT&interval=1m&limit=1000&startTime=${cursor}&endTime=${end}`;
    const d = await fetchBinanceJSON(urlPath);
...
function syncStakeTimelineUI(forceToEnd=false, moved="end"){
  const startR = $("stakeRangeStart");
  const endR   = $("stakeRangeEnd");
  const meta = $("stakeTimelineMeta");
  const last = $("stakeLast");
  if (!startR || !endR || !meta) return;

  const n = stakeData.length;
  if (last && n) last.textContent = `${safe(stakeData[n-1]).toFixed(4)} INJ`;
  if (last && !n) last.textContent = "—";

  if (!n){
    startR.min = 0; startR.max = 0; startR.value = 0;
    endR.min = 0; endR.max = 0; endR.value = 0;
    meta.textContent = "—";
    if (stakeChart){
  ...
  startR.min = 0; startR.max = String(n - 1);
  endR.min   = 0; endR.max   = String(n - 1);

  if (forceToEnd){
    const endIdx = n - 1;
    const startIdx = Math.max(0, endIdx - Math.min(60, n-1));
    startR.value = String(startIdx);
    endR.value   = String(endIdx);
  }

  let a = clamp(parseInt(startR.value || "0", 10), 0, n - 1);
  let b = clamp(parseInt(endR.value   || "0", 10), 0, n - 1);

  if (a > b){
    if (moved === "start") b = a;
    else a = b;
    startR.value = String(a);
    endR.value   = String(b);
  }

  if (stakeChart){
    stakeChart.options.scales.x.min = a;
    stakeChart.options.scales.x.max = b;

    // Dynamic y-scale on the selected window: keeps the curve visible even for small increases
    const seg = stakeData.slice(a, b + 1).map(safe).filter(v => Number.isFinite(v));
    if (seg.length){
      let minV = Math.min(...seg);
      let maxV = Math.max(...seg);
      if (!Number.isFinite(minV)) minV = 0;
      if (!Number.isFinite(maxV)) maxV = minV || 0;
      const span = Math.max(0, maxV - minV);
      const pad = Math.max(0.0001, span * 0.08, maxV * 0.002);
      stakeChart.options.scales.y.min = Math.max(0, minV - pad);
      stakeChart.options.scales.y.max = maxV + pad;
    } else {
      stakeChart.options.scales.y.min = undefined;
      stakeChart.options.scales.y.max = undefined;
    }
  ...
  const fromTs = labelToTs(stakeLabels[a]);
  const toTs   = labelToTs(stakeLabels[b]);
  const from = fromTs ? fmtDTShort(fromTs) : (stakeLabels[a] || "");
  const to   = toTs ? fmtDTShort(toTs) : (stakeLabels[b] || "");
  meta.textContent = n <= 1 ? `${to}` : `${from} → ${to}`;
}

$("stakeRangeStart")?.addEventListener("input", () => {
  stakeFollow = false;
  syncStakeTimelineUI(false, "start");
}, { passive:true });

$("stakeRangeEnd")?.addEventListener("input", () => {
  stakeFollow = false;
  syncStakeTimelineUI(false, "end");
}, { passive:true });

$("stakeLiveBtn")?.addEventListener("click", () => {
  stakeFollow = true;
  const a = $("stakeRangeStart");
  const b = $("stakeRangeEnd");
  if (a && b){
    const n = stakeData.length;
    const endIdx = Math.max(0, n - 1);
    const startIdx = Math.max(0, endIdx - Math.min(60, Math.max(0, n - 1)));
    a.value = String(startIdx);
    b.value = String(endIdx);
  }
  if (stakeChart?.resetZoom) stakeChart.resetZoom();
  syncStakeTimelineUI(true, "end");
}, { passive:true });
...
function syncFeesTimelineUI(forceToEnd=false) {
  ...
  if (feesChart) {
    feesChart.options.scales.x.min = minIdx;
    feesChart.options.scales.x.max = maxIdx;
    feesChart.update("none");
  }

  const fromTs = feesTimes[minIdx] || labelToTs(feesLabels[minIdx]);
  const toTs   = feesTimes[maxIdx] || labelToTs(feesLabels[maxIdx]);
  const from = fromTs ? fmtHHMM(fromTs) : (feesLabels[minIdx] || "");
  const to   = toTs ? fmtHHMM(toTs) : (feesLabels[maxIdx] || "");
  meta.textContent = n <= 1 ? `${to}` : `${from} → ${to}`;
}

$("feesTimeline")?.addEventListener("input", () => syncFeesTimelineUI(false), { passive: true });
$("feesLiveBtn")?.addEventListener("click", () => {
  const slider = $("feesTimeline");
  if (slider) slider.value = String(Math.max(0, feesValues.length - 1));
...
function injBigIntToNumber(amountStr, decimals, keep=6){
  // ✅ No BigInt / no BigInt literals -> avoids Android parsing crashes
  // Works with arbitrary-length integer strings by doing pure string math.
  const raw = (amountStr == null) ? "0" : String(amountStr).trim();
  if (!raw) return 0;

  // keep only digits (Injective amounts are non-negative)
  ...
  const dec = Math.max(0, Math.min(30, Number(decimals) || 0));
  const k   = Math.max(0, Math.min(12, Number(keep) || 0));

  if (dec === 0) return Number(s);

  // Ensure at least one digit on the left of the decimal point
  const need = dec + 1;
  const padded = (s.length < need) ? ("0".repeat(need - s.length) + s) : s;

  const intPart = padded.slice(0, padded.length - dec);
  const fracFull = padded.slice(padded.length - dec);

  // Take only the first `k` decimals for readability
  const fracPart = (k > 0) ? fracFull.slice(0, k) : "";
  const fracTrim = fracPart.replace(/0+$/, "");

  const out = fracTrim ? (intPart + "." + fracTrim) : intPart;
...
let nwUsdAll = [];
let nwInjAll = [];
let nwOriginTs = 0; // first-ever timestamp for this address (used to unlock TFs even after compaction)
...
function nwEnsureSortedInPlace(){
  if (!nwTAll || nwTAll.length < 2) return;

  // Fast check: already sorted?
  for (let i=1;i<nwTAll.length;i++){
    const a = safe(nwTAll[i-1]);
    const b = safe(nwTAll[i]);
    if (b && a && b < a){
      // not sorted -> rebuild aligned triples and sort
      const tmp = [];
      const n = Math.min(nwTAll.length, nwUsdAll.length, nwInjAll.length);
      for (let k=0;k<n;k++){
        const t = safe(nwTAll[k]);
        const u = safe(nwUsdAll[k]);
        const inj = safe(nwInjAll[k]);
        if (t) tmp.push({ t, u, inj });
      }
      tmp.sort((x,y)=>x.t - y.t);
      nwTAll = tmp.map(p=>p.t);
      nwUsdAll = tmp.map(p=>p.u);
      nwInjAll = tmp.map(p=>p.inj);
      return;
    }
  }
}
function clampNWArrays(){
  ...
  // keep origin stable (used for TF unlocks)
  if (!nwOriginTs) nwOriginTs = safe(nwTAll?.[0]) || 0;
  else if (nwTAll.length) {
    const a = safe(nwOriginTs);
    const b = safe(nwTAll[0]);
    nwOriginTs = (a && b) ? Math.min(a, b) : (a || b || 0);
  }
}
function saveNWLocal(){
  ...
  // keep a stable "origin" so TF unlocks won't regress after compaction/merges
  if (!nwOriginTs) nwOriginTs = safe(nwTAll?.[0]) || Date.now();
  ...
  try{
    const payload = {
      ...
      // legacy (backward compatibility with earlier saves)
      tAll: nwTAll,
      usdAll: nwUsdAll,
      injAll: nwInjAll
    };
...
function loadNWLocal(){
  ...
  try{
    ...
    const tArr = Array.isArray(obj.times) ? obj.times : (Array.isArray(obj.tAll) ? obj.tAll : []);
    const uArr = Array.isArray(obj.usd) ? obj.usd : (Array.isArray(obj.usdAll) ? obj.usdAll : []);
    const jArr = Array.isArray(obj.inj) ? obj.inj : (Array.isArray(obj.injAll) ? obj.injAll : []);

    nwTAll = tArr.map(Number);
    nwUsdAll = uArr.map(Number);
    nwInjAll = jArr.map(Number);

    nwTf = typeof obj.tf === "string" ? obj.tf : "live";
    nwScale = (obj.scale === "log") ? "log" : "linear";

    // stable TF unlock anchor
    nwOriginTs = safe(obj.originTs) || safe(nwTAll?.[0]) || 0;
...
function nwHasSpan(tf){
  // Use the earliest known origin (stable across compaction + cloud merges).
  const w = nwWindowMs(tf);

  // Small unlock cushion for 1w (users usually mean ~7 days, not exact to the minute)
  const wUnlock = (tf === "1w") ? (w * 0.97) : w;

  const now = Date.now();
  const oldestArr = safe(nwTAll?.[0]) || 0;
  const origin = safe(nwOriginTs) || 0;

  const base = (oldestArr && origin) ? Math.min(oldestArr, origin) : (oldestArr || origin || 0);
  if (!base) return false;

  // Primary: do we have history older than the window?
  if (base <= (now - wUnlock)) return true;

  // Fallback: compare last point span (useful if device clock changes)
  const last = safe(nwTAll?.[nwTAll.length - 1]) || 0;
  if (last && (last - base) >= wUnlock) return true;
...
function nwBuildView(tf){
  ...
  // Convenience arrays (some helpers still expect these)
...
async function nwBackfillFromMarketRange(startTs, endTs){
  ...
  let baseInj = safe(nwInjAll?.[baseIdx]);

  // If INJ is missing (bad merge), try a safe fallback using the latest known price
  if ((!Number.isFinite(baseInj) || baseInj <= 0) && Number.isFinite(targetPrice) && targetPrice > 0){
    const baseUsd = safe(nwUsdAll?.[baseIdx]);
    if (Number.isFinite(baseUsd) && baseUsd > 0) baseInj = baseUsd / safe(targetPrice);
  }
  ...
  // Auto-scale interval so we can backfill long gaps without hitting REST limits
  const span = end - start;
  let interval = "1m";
  if (span > 2 * 24 * 60 * 60 * 1000) interval = "15m";
  if (span > 20 * 24 * 60 * 60 * 1000) interval = "1h";
  if (span > 220 * 24 * 60 * 60 * 1000) interval = "1d";
  ...
  const maxTotal = clamp(Math.ceil(span / stepMs) + 10, 20, 2400);

  let kl = [];
  try{
    kl = await fetchKlinesRange("INJUSDT", interval, start, end, maxTotal);
  } catch {}
...
function initNWChart(){
      ...
      scales: {
        x: {
          ...
          ticks: {
            ...
            maxTicksLimit: 6,
            callback: (v) => {
              const x = netWorthChart?.scales?.x;
              const span = x ? Math.abs(safe(x.max) - safe(x.min)) : spanMsFromPoints(netWorthChart?.data?.datasets?.[0]?.data || []);
              return fmtAxisX(safe(v), span);
            }
          },
          grid: { display: false },
          border: { display: false }
  ...
  attachCrosshair2(
    ...
    $("nwReadout"),
    (i, lbs, ds) => {
      const pt = seriesPointAt(ds, lbs, i);
      return fmtNWReadout(pt.ts, pt.v);
    },
    {
      // Net Worth: keep the value fixed while interacting, then return to real-time
      idleMs: 2400,
      bottomLabelFn: (i, lbs, ds) => {
        const pt = seriesPointAt(ds, lbs, i);
        if (!pt.ts) return "";
        // On ALL show full date (requested)
        return (nwTf === "all" || nwTf === "1y") ? fmtDDMMYYYY(pt.ts) : fmtDDMM(pt.ts);
      },
      onEnd: () => {
        // restore latest value after user stops interacting
        try { nwSetReadoutLatest(); } catch {}
      }
    }
  );
}
...
function nwSetReadoutLatest(){
  const ro = $("nwReadout");
  if (!ro || !netWorthChart) return;
  const ds = netWorthChart.data.datasets?.[0]?.data || [];
  const lbs = netWorthChart.data.labels || [];
  if (!ds.length) { ro.textContent = "—"; return; }
  const i = ds.length - 1;
  const pt = seriesPointAt(ds, lbs, i);
  ro.textContent = fmtNWReadout(pt.ts, pt.v);
}
function nwReanchorCrosshair(){
  if (!netWorthChart?.$crosshairActive) return;
  const ts = safe(netWorthChart.$crosshairTs);
  if (!ts) return;
  const ds = netWorthChart.data.datasets?.[0]?.data || [];
  if (!ds.length) return;

  // Find nearest point by timestamp (keeps crosshair stable while new points arrive)
  let bestI = 0;
  let bestD = Infinity;
  for (let i=0;i<ds.length;i++){
    const x = (ds[i] && typeof ds[i] === "object") ? safe(ds[i].x) : safe(netWorthChart.data.labels?.[i]);
    if (!x) continue;
    const d = Math.abs(x - ts);
    if (d < bestD){ bestD = d; bestI = i; }
  }
  netWorthChart.$crosshairIndex = clamp(bestI, 0, ds.length - 1);
}
function drawNW(force=false){
  ...
  // Time-uniform x-axis (linear) + {x,y} points (prevents ALL from being dominated by today's dense points)
  netWorthChart.data.labels = view.labels; // numeric timestamps (for helpers)
  netWorthChart.data.datasets[0].data = view.points;
  netWorthChart.options.scales.y.type = (nwScale === "log") ? "logarithmic" : "linear";

  // Keep crosshair anchored to the same timestamp while the chart updates
  try { nwReanchorCrosshair(); } catch {}
  ...
  // If the user is NOT interacting, keep the readout on the latest value.
  if (netWorthChart && $("nwReadout")) {
    if (!netWorthChart.$crosshairActive) nwSetReadoutLatest();
  }
}
...
$("nwTfSwitch")?.addEventListener("click", (e) => {
  ...
  if (netWorthChart){
    netWorthChart.$crosshairActive = false;
    netWorthChart.$crosshairIndex = null;
    netWorthChart.$crosshairTs = null;
    netWorthChart.$crosshairBottomLabel = "";
  }
...
function recordNetWorthPoint(){
  ...
  // stable origin used for TF unlocks (won't move even if arrays get compacted)
  if (!nwOriginTs) nwOriginTs = safe(nwTAll?.[0]) || now;
...
function buildCloudPayload(){
  return {
    ...
    nw: { originTs: (nwOriginTs || safe(nwTAll?.[0]) || 0), times: nwTAll, usd: nwUsdAll, inj: nwInjAll },
    events: eventsAll
  };
}
...
function mergeNW(payloadNw){
  ...
  const t = Array.isArray(payloadNw.times) ? payloadNw.times : (Array.isArray(payloadNw.tAll) ? payloadNw.tAll : []);
  const u = Array.isArray(payloadNw.usd) ? payloadNw.usd : (Array.isArray(payloadNw.usdAll) ? payloadNw.usdAll : []);
  const j = Array.isArray(payloadNw.inj) ? payloadNw.inj : (Array.isArray(payloadNw.injAll) ? payloadNw.injAll : []);

  const prevOrigin = safe(nwOriginTs) || safe(nwTAll?.[0]) || 0;

  // Merge by timestamp while keeping USD + INJ aligned on the same timeline
  const map = new Map();

  for (let i=0;i<nwTAll.length;i++){
    const ts = safe(nwTAll[i]);
    if (!ts) continue;
    map.set(ts, { usd: safe(nwUsdAll[i]), inj: safe(nwInjAll[i]) });
  }

  const n = Math.max(t.length, u.length, j.length);
  for (let i=0;i<n;i++){
    const ts = safe(t[i]);
    if (!ts) continue;
    ...
    const cur = map.get(ts) || { usd: NaN, inj: NaN };

    if ((!Number.isFinite(cur.usd) || cur.usd <= 0) && Number.isFinite(usd) && usd > 0) cur.usd = usd;
    if ((!Number.isFinite(cur.inj) || cur.inj <= 0) && Number.isFinite(inj) && inj > 0) cur.inj = inj;
  ...
  for (const ts of times){
    ...
    const usd = safe(row?.usd);
    let inj = safe(row?.inj);

    // fill missing INJ with last known (keeps backfill possible)
    if (!Number.isFinite(inj) || inj <= 0) inj = lastInj;
    if (Number.isFinite(inj) && inj > 0) lastInj = inj;

    // keep only valid USD points for the chart
  ...
  const remoteOrigin = safe(payloadNw.originTs || payloadNw.origin || 0);
  const earliest = safe(nwTAll?.[0]) || 0;
  const candidate = Math.min(
    remoteOrigin > 0 ? remoteOrigin : Infinity,
    prevOrigin > 0 ? prevOrigin : Infinity,
    earliest > 0 ? earliest : Infinity
  );
  nwOriginTs = Number.isFinite(candidate) && candidate !== Infinity ? candidate : (earliest || prevOrigin || 0);
...
function resetPerAddressStateUI(){
  ...
  stakeFollow = true;
  try{ setText("stakeLast","—"); setText("stakeTimelineMeta","—"); const a=$("stakeRangeStart"), b=$("stakeRangeEnd"); if(a){ a.min=0; a.max=0; a.value=0; } if(b){ b.min=0; b.max=0; b.value=0; } }catch{}
...
function fmtDDMMYYYY_HHMM(ts){
  if (!ts) return "";
  const d = new Date(ts);
  return `${pad2(d.getDate())}/${pad2(d.getMonth()+1)}/${d.getFullYear()} ${pad2(d.getHours())}:${pad2(d.getMinutes())}`;
}
function seriesPointAt(ds, lbs, i){
  const p = ds?.[i];
  if (p && typeof p === "object"){
    return { ts: safe(p.x), v: safe(p.y) };
  }
  const ts = labelToTs(lbs?.[i]);
  return { ts, v: safe(p) };
}
function spanMsFromPoints(ds){
  if (!ds || ds.length < 2) return 0;
  const a = (ds[0] && typeof ds[0] === "object") ? safe(ds[0].x) : safe(ds[0]);
  const b = (ds[ds.length - 1] && typeof ds[ds.length - 1] === "object") ? safe(ds[ds.length - 1].x) : safe(ds[ds.length - 1]);
  const span = Math.abs((b || 0) - (a || 0));
  return Number.isFinite(span) ? span : 0;
}
function fmtNWReadout(ts, usd){
  const v = Number.isFinite(+usd) ? +usd : 0;
  if (!ts) return `— • $${v.toFixed(2)}`;

  // richer timestamp on wider timeframes (ALL must include the date)
  const when =
    (nwTf === "all" || nwTf === "1y") ? fmtDDMMYYYY_HHMM(ts) :
    (nwTf === "1m" || nwTf === "1w") ? fmtDDMM_HHMM(ts) :
    fmtHHMM(ts);

  return `${when} • $${v.toFixed(2)}`;
}
...
const crosshairPlugin2 = {
  ...
  afterDraw(ch) {
    ...
    const label = String(ch?.$crosshairBottomLabel || "").trim();
    if (!label) return;
    ...
    const w = Math.max(32, tw + padX * 2);
    const h = 18 + padY;

    const y = ch.chartArea.bottom - h - 6; // inside plot, above x-axis
    let x = el.x - w / 2;
    x = clamp(x, ch.chartArea.left + 2, ch.chartArea.right - w - 2);
    ...
    ctx.fillStyle = isLight ? "rgba(255,255,255,0.78)" : "rgba(0,0,0,0.55)";
    ctx.strokeStyle = isLight ? "rgba(15,23,42,0.10)" : "rgba(255,255,255,0.12)";
    ctx.lineWidth = 1;
    ctx.fill();
    ctx.stroke();

    ctx.fillStyle = isLight ? "rgba(15,23,42,0.90)" : "rgba(255,255,255,0.92)";
...
function attachCrosshair2(ch, overlayEl, formatter, opts = {}){
  if (!ch || !overlayEl) return;
  try { Chart.register(crosshairPlugin2); } catch {}
  const canvas = ch.canvas;

  const idleMs = Math.max(350, safe(opts.idleMs) || 950);
  const bottomLabelFn = (typeof opts.bottomLabelFn === "function") ? opts.bottomLabelFn : null;
  ...
  const scheduleEnd = () => {
    clearTimer();
    ch.$crosshairTimer = setTimeout(end, idleMs);
  };
  ...
  const setIdx = (i, active) => {
    ...
    const pt = seriesPointAt(ds, lbs, idx);
    ch.$crosshairTs = safe(pt.ts) || null;

    if (bottomLabelFn){
      try { ch.$crosshairBottomLabel = String(bottomLabelFn(idx, lbs, ds) || ""); } catch { ch.$crosshairBottomLabel = ""; }
    } else {
      ch.$crosshairBottomLabel = "";
    }
  ...
  // start on click/tap (and mousedown for desktop)
  canvas.addEventListener("mousedown", start, { passive:true });
  canvas.addEventListener("click", start, { passive:true });
  canvas.addEventListener("touchstart", start, { passive:true });

  // drag/hover while active
  canvas.addEventListener("mousemove", move, { passive:true });
  canvas.addEventListener("touchmove", move, { passive:true });

  // end if leaving the canvas (also cancels idle)
  canvas.addEventListener("mouseleave", end, { passive:true });
  canvas.addEventListener("touchend", () => scheduleEnd(), { passive:true });
  canvas.addEventListener("touchcancel", end, { passive:true });
}
...
async function fetchKlinesRange(symbol, interval, startTime, endTime, maxTotal=2400){
  ...
  while (out.length < maxTotal && guard++ < 10){
    const limit = 1000;
    const url = cursor
      ? `/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${limit}&startTime=${cursor}&endTime=${end}`
    : `/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`;
    const d = await fetchBinanceJSON(url);
    if (!Array.isArray(d) || !d.length) break;
    out.push(...d);
    if (!cursor) break;
...
(async function boot() {
  ...
  initMarketTicker();
  safeAsync(() => loadMarketGlobal(true), "loadMarketGlobal");
