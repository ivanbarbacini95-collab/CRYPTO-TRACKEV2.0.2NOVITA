...
const FEES_LOCAL_VER = 1;
const CANDLE_SNAP_LOCAL_VER = 1;
const PRICE_CHART_LOCAL_VER = 1;

const LAST_PRICE_KEY = "inj_last_price";
const CANDLE_SNAP_KEY = `inj_candlesnap_v${CANDLE_SNAP_LOCAL_VER}`;
const PRICE_CHART_KEY = `inj_pricechart_v${PRICE_CHART_LOCAL_VER}`;
...
window.addEventListener("online", () => {
  ...
  if (address) safeAsync(() => cloudPull(), "cloudPull(online)");
  if (liveMode) {
    // LIVE: refresh candle + chart + account and reconnect sockets
    safeAsync(() => loadCandleSnapshot(true), "loadCandleSnapshot(online)");
    safeAsync(() => loadChartToday(true), "loadChartToday(online)");
    if (address) safeAsync(() => loadAccount(true), "loadAccount(online)");
    ...
    // Refresh mode (pull-to-refresh)
    safeAsync(() => loadCandleSnapshot(true), "loadCandleSnapshot(online)");
    safeAsync(() => loadChartToday(true), "loadChartToday(online)");
    if (address) safeAsync(() => loadAccount(true), "loadAccount(online)");
  }

  // attempt to backfill missing Net Worth points after reconnect
  safeAsync(() => nwBackfillFromBinance(), "nwBackfillFromBinance");
}, { passive:true });
...
function saveLastPrice(p){
  const v = safe(p);
  if (!Number.isFinite(v) || v <= 0) return;
  const now = Date.now();
  if ((now - _lastPriceSaveAt) < 4000) return;
  _lastPriceSaveAt = now;
  try{ localStorage.setItem(LAST_PRICE_KEY, String(v)); } catch {}
}
function loadLastPrice(){
  try{ return safe(localStorage.getItem(LAST_PRICE_KEY)); } catch { return 0; }
}

function saveCandleSnapLocal(){
  try{
    localStorage.setItem(CANDLE_SNAP_KEY, JSON.stringify({ v:CANDLE_SNAP_LOCAL_VER, t:Date.now(), candle, tfReady }));
  } catch {}
}
function loadCandleSnapLocal(){
  try{
    const raw = localStorage.getItem(CANDLE_SNAP_KEY);
    if (!raw) return false;
    const obj = JSON.parse(raw);
    if (!obj || obj.v !== CANDLE_SNAP_LOCAL_VER) return false;
    const c = obj.candle || {};
    const r = obj.tfReady || {};
    for (const k of ["d","w","m","y"]){
      candle[k] = { t:safe(c?.[k]?.t), open:safe(c?.[k]?.open), high:safe(c?.[k]?.high), low:safe(c?.[k]?.low) };
      tfReady[k] = !!r?.[k] || (!!candle[k].open && !!candle[k].high && !!candle[k].low);
    }
    return true;
  } catch { return false; }
}

function savePriceChartLocal(){
  try{
    localStorage.setItem(PRICE_CHART_KEY, JSON.stringify({
      v: PRICE_CHART_LOCAL_VER,
      t: Date.now(),
      labels: chartLabels,
      data: chartData,
      last: lastChartMinuteStart,
      boot: !!chartBootstrappedToday,
      start: candle?.d?.t || 0
    }));
  } catch {}
}
function loadPriceChartLocal(){
  try{
    const raw = localStorage.getItem(PRICE_CHART_KEY);
    if (!raw) return false;
    const obj = JSON.parse(raw);
    if (!obj || obj.v !== PRICE_CHART_LOCAL_VER) return false;
    chartLabels = Array.isArray(obj.labels) ? obj.labels : [];
    chartData   = Array.isArray(obj.data) ? obj.data.map(Number) : [];
    lastChartMinuteStart = safe(obj.last);
    chartBootstrappedToday = !!obj.boot;
    const lastClose = chartData.length ? safe(chartData[chartData.length-1]) : 0;
    if (!targetPrice && lastClose) targetPrice = lastClose;
  try{ saveLastPrice(lastClose); } catch {}
...
function urlGetAddr(){
  try{
    ...
    return String(u.searchParams.get("a") || u.searchParams.get("addr") || "").trim();
  } catch { return ""; }
}
function urlSetAddr(a){
  try{
    const u = new URL(location.href);
...
let address = (urlGetAddr() || (sessionStorage.getItem("currentAddress") || "") || (localStorage.getItem("inj_address") || "")).trim();
let pendingAddress = address || "";
if (address) { try{ urlSetAddr(address); } catch {} }
try{ if(address) sessionStorage.setItem("currentAddress", address); } catch {}
...
/* ================= STATE ================= */
let targetPrice = 0;
// offline-friendly: restore last known price
try{ const lp = loadLastPrice(); if (!targetPrice && lp) targetPrice = lp; } catch {}
...
async function loadCandleSnapshot(isRefresh=false) {
  if (!isRefresh && !liveMode) return;
  if (!hasInternet()) { loadCandleSnapLocal(); return; }
...
async function loadChartToday(isRefresh=false) {
  if (!isRefresh && !liveMode) return;
  if (!hasInternet()) { loadPriceChartLocal(); return; }
...
function updateChartFrom1mKline(k) {
  ...
  try{
    ...
    const last = safe(arr[arr.length-1]);
    if (last) saveLastPrice(last);
  } catch {}
    }
    return;
  }
...
function maybeAddStakePoint(currentStake) {
  ...
  pushEvent({
    ...
    // Record tx fee (Fees card)
    safeAsync(() => recordWithdrawalFeeForTs(ts), "recordWithdrawalFeeForTs");
}
...
function feeStoreKey(addr){
  const a = (addr || "").trim();
  return a ? `inj_fees_v${FEES_LOCAL_VER}_${a}` : null;
}
function clampFeesAll(){
  const MAX = 4000;
  if (feeTimesAll.length <= MAX) return;
  const over = feeTimesAll.length - MAX;
  feeTimesAll = feeTimesAll.slice(over);
  feeLabelsAll = feeLabelsAll.slice(over);
  feeValuesAll = feeValuesAll.slice(over);
}
function saveFeesLocal(){
  const key = feeStoreKey(address);
  if (!key) return;
  try{
    localStorage.setItem(key, JSON.stringify({ v: FEES_LOCAL_VER, t: Date.now(), labels: feeLabelsAll, values: feeValuesAll, times: feeTimesAll }));
    cloudBumpLocal(1);
  } catch {}
  cloudMarkDirty({ fees:true });
}
function saveFeesLocalSilent(){
  const key = feeStoreKey(address);
  if (!key) return;
  try{
    localStorage.setItem(key, JSON.stringify({ v: FEES_LOCAL_VER, t: Date.now(), labels: feeLabelsAll, values: feeValuesAll, times: feeTimesAll }));
...
function loadFeesLocal(){
  ...
  try{
    ...
    if (!raw) return false;
    const obj = JSON.parse(raw);
    if (!obj || obj.v !== FEES_LOCAL_VER) return false;
    feeLabelsAll = Array.isArray(obj.labels) ? obj.labels : [];
    feeValuesAll = Array.isArray(obj.values) ? obj.values.map(Number) : [];
    feeTimesAll  = Array.isArray(obj.times) ? obj.times.map(Number) : [];
    clampFeesAll();
    return true;
  } catch { return false; }
}
function rebuildFeesView(){
  feeLabels = feeLabelsAll.slice();
  feeValues = feeValuesAll.slice();
  feeTimes  = feeTimesAll.slice();
}
function feesTotalInj(){
  let s = 0;
  for (const v of feeValuesAll){ const n = safe(v); if (Number.isFinite(n) && n > 0) s += n; }
  return s;
}
function updateFeesUI(){
  const total = feesTotalInj();
  setText("feesTotal", total ? total.toFixed(6) : "0.000000");

  const px = safe(displayed?.price || targetPrice);
  const usd = (Number.isFinite(px) && px>0) ? (total * px) : 0;
  setText("feesTotalUsd", usd ? `• $${usd.toFixed(2)}` : "—");

  const last = feeValuesAll.length ? safe(feeValuesAll[feeValuesAll.length-1]) : 0;
  setText("feesLast", last ? `${last.toFixed(6)} INJ` : "—");
}
function initFeesChart(){
  ...
  feesChart = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        data: feeValues,
        borderColor: "#facc15",
        backgroundColor: "rgba(250,204,21,.10)",
        fill: false,
        tension: 0,
        spanGaps: false,
        pointRadius: 4,
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { enabled: false },
        ...(ZOOM_OK ? { zoom: { pan: { enabled: true, mode: "x", modifierKey: "ctrl" }, zoom: { wheel: { enabled: true }, pinch: { enabled: true }, mode: "x" } } } : {})
      },
      interaction: { mode: "index", intersect: false },
      scales: {
        x: { ticks: { color: axisTickColor(), maxRotation: 0, autoSkip: true, maxTicksLimit: 6 }, grid: { color: axisGridColor() } },
        y: { ticks: { color: axisTickColor(), callback: (v)=> `${Number(v).toFixed(4)}` }, grid: { color: axisGridColor() }, beginAtZero: true }
      }
    },
    plugins: [lastDotPlugin]
  });
  ...
  attachCrosshair2(feesChart, $("feesReadout"), (i, lbs, ds) => {
    const t = labelToTs(lbs?.[i]) || (feeTimes?.[i] || 0);
    const v = safe(ds?.[i]);
    return `${t ? new Date(t).toLocaleString() : "—"} • ${v.toFixed(6)} INJ`;
  });
}
function drawFeesChart(){
  ...
  feesChart.data.labels = feeLabels;
  feesChart.data.datasets[0].data = feeValues;
  try { feesChart.update("none"); } catch {}

  const tl = $("feesTimeline");
  if (tl){
    tl.max = String(Math.max(0, feeLabels.length-1));
    if (feesLive) tl.value = String(Math.max(0, feeLabels.length-1));
  }
  updateFeesTimelineMeta();
}
function updateFeesTimelineMeta(){
  const meta = $("feesTimelineMeta");
  if (!meta) return;
  const i = Number($("feesTimeline")?.value || 0);
  const t = feeTimes?.[i] || labelToTs(feeLabels?.[i]) || 0;
  const v = safe(feeValues?.[i]);
  meta.textContent = feeValues.length ? `${t ? new Date(t).toLocaleString() : "—"} • ${v.toFixed(6)} INJ` : "—";
}

async function fetchLatestWithdrawFee(address, tsHint){
  if (!address || !hasInternet()) return null;
  const addr = String(address).trim();
  const eventsSets = [
    [`message.sender='${addr}'`, `message.action='/cosmos.distribution.v1beta1.MsgWithdrawDelegatorReward'`],
    [`message.sender='${addr}'`, `message.action='withdraw_delegator_reward'`],
    [`message.sender='${addr}'`, `message.module='distribution'`]
  ];

  for (const evs of eventsSets){
    const qs = evs.map(e => "events=" + encodeURIComponent(e)).join("&");
    const url = `/cosmos/tx/v1beta1/txs?${qs}&pagination.limit=20&order_by=ORDER_BY_DESC`;
    let res;
    try { res = await fetchLCD(url); } catch { continue; }
    const txs = Array.isArray(res?.txs) ? res.txs : [];
    const rs  = Array.isArray(res?.tx_responses) ? res.tx_responses : [];
    if (!txs.length || !rs.length) continue;

    let best = null;
    for (let i=0;i<Math.min(txs.length, rs.length);i++){
      const ts = Date.parse(rs[i]?.timestamp || "") || 0;
      const feeArr = txs[i]?.auth_info?.fee?.amount || [];
      let fee = 0;
      for (const a of feeArr){
        const denom = String(a?.denom || "");
        const amt = safe(a?.amount);
        if (!Number.isFinite(amt) || amt <= 0) continue;
        let inj = amt;
        if (amt > 1e9) inj = amt / 1e18;
        else if (amt > 1e3 && denom.toLowerCase().includes("uinj")) inj = amt / 1e6;
        fee += inj;
      }
      if (!Number.isFinite(fee) || fee <= 0) continue;
      const score = tsHint ? Math.abs(ts - tsHint) : i;
      if (!best || score < best.score) best = { fee, score };
    }
...
function addFeePoint(ts, feeInj){
  const v = safe(feeInj);
  const t0 = safe(ts) || Date.now();
  if (!Number.isFinite(v) || v <= 0) return;

  const t = wdDeterministicTs(t0, Number(v.toFixed(6)));

  for (let i=feeTimesAll.length-1; i>=0 && i>=feeTimesAll.length-12; i--){
    if (safe(feeTimesAll[i]) === t) return;
  }

  feeTimesAll.push(t);
  feeLabelsAll.push(tsLabel(t));
  feeValuesAll.push(Number(v.toFixed(6)));
  clampFeesAll();
  saveFeesLocal();
  ...
  try{ flashYellow($("feesTotal")); } catch {}
}

async function recordWithdrawalFeeForTs(ts){
  if (!address || !hasInternet()) return;
  const fee = await fetchLatestWithdrawFee(address, ts);
  if (fee) addFeePoint(ts, fee);
}
...
function bindFeesUI(){
  $("feesLiveBtn")?.addEventListener("click", () => {
    feesLive = true;
    const tl = $("feesTimeline");
    if (tl) tl.value = String(Math.max(0, feeLabels.length-1));
    updateFeesTimelineMeta();
  });

  $("feesTimeline")?.addEventListener("input", () => {
    feesLive = false;
    updateFeesTimelineMeta();
  });
}
...
function nwBuildView(tf){
  ...
  // break straight lines after long gaps (e.g., app closed / offline)
  ...
  for (let i = 0; i < nwTAll.length; i++){
    ...
    if (t >= minT && Number.isFinite(u) && u > 0) {
      if (prevT != null && (t - prevT) > BREAK_MS){
        labels.push(tsLabel(prevT + 1));
        data.push(null);
      }
...
async function nwBackfillFromBinance(){
  if (!address || !hasInternet()) return;
  if (!nwTAll.length) return;
  const lastT = safe(nwTAll[nwTAll.length-1]) || 0;
  const now = Date.now();
  const gap = now - lastT;
  if (gap < 5 * 60 * 1000) return; // ignore tiny gaps

  // cap backfill to last 24h
  const start = Math.max(lastT + 60_000, now - 24 * 60 * 60 * 1000);
  const kl = await fetchKlines1mRange(start, now);
  if (!kl.length) return;

  const totalInjRef = safe(availableInj) + safe(stakeInj) + safe(rewardsInj);
  if (!Number.isFinite(totalInjRef) || totalInjRef <= 0) return;

  for (const k of kl){
    const t = safe(k?.[0]);
    const close = safe(k?.[4]);
    if (!t || !Number.isFinite(close) || close <= 0) continue;
    if (t <= lastT) continue;
    const usd = totalInjRef * close;
    if (!Number.isFinite(usd) || usd <= 0) continue;
    nwTAll.push(t);
    nwUsdAll.push(usd);
    nwInjAll.push(totalInjRef);
  }
...
let cloudDirtyWhat = { stake:false, wd:false, fees:false, nw:false, events:false };
...
function buildCloudPayload(){
  return {
    ...
    fees: { labels: feeLabelsAll, values: feeValuesAll, times: feeTimesAll },
...
function mergeFees(payloadFees){
  if (!payloadFees) return;
  const pl = Array.isArray(payloadFees.labels) ? payloadFees.labels : [];
  const pv = Array.isArray(payloadFees.values) ? payloadFees.values : [];
  const pt = Array.isArray(payloadFees.times) ? payloadFees.times : [];

  const map = new Map();

  for (let i=0;i<feeTimesAll.length;i++){
    const tIn = safe(feeTimesAll[i]) || labelToTs(feeLabelsAll[i]);
    const v = safe(feeValuesAll[i]);
    if (!tIn || !Number.isFinite(v) || v <= 0) continue;
    const t = wdDeterministicTs(tIn, Number(v.toFixed(6)));
    map.set(t, { v, l: String(feeLabelsAll[i] || tsLabel(t)) });
  }

  const n = Math.min(pl.length, pv.length, pt.length) || Math.max(pl.length, pv.length, pt.length);
  for (let i=0;i<n;i++){
    const tIn = safe(pt[i]) || labelToTs(pl[i]);
    const v = safe(pv[i]);
    if (!tIn || !Number.isFinite(v) || v <= 0) continue;
  ...
  const keys = [...map.keys()].sort((a,b)=>a-b);
  feeTimesAll  = keys;
  feeLabelsAll = keys.map(t => map.get(t).l || tsLabel(t));
  feeValuesAll = keys.map(t => safe(map.get(t).v));
  clampFeesAll();
}
...
try{
  ...
  if (pulled?.ok && pulled.data){
    ...
    try{ mergeWd(data.wd);
    try{ mergeFees(data.fees); } catch {} } catch {}
...
const res = await fetchJSON(url, {
  ...
  cloudDirty = false;
  cloudDirtyWhat = { stake:false, wd:false, fees:false, nw:false, events:false };
...
async function commitAddress(newAddr) {
  ...
  try{ sessionStorage.setItem("currentAddress", address); } catch {}
  try{ urlSetAddr(address); } catch {}
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  if (liveMode) {
    ...
    if (address && hasInternet()) { safeAsync(() => nwBackfillFromBinance(), "nwBackfillFromBinance(boot)"); }
...
function animate() {
  ...
  try{
    ...
    if (el){
      const d = displayed.price - op;
      el.classList.remove("up","down","flat");
      if (d > 0) { el.classList.add("up"); el.textContent = "▲"; }
      else if (d < 0) { el.classList.add("down"); el.textContent = "▼"; }
