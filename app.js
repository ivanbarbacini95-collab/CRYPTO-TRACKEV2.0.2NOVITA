...
function txFeeInj(tx){
  // Cosmos Tx schema: tx.auth_info.fee.amount is usually an array of coins.
  // Be defensive: some endpoints nest tx under tx.tx, or use authInfo naming.
  const feeObj =
    tx?.auth_info?.fee ??
    tx?.authInfo?.fee ??
    tx?.auth_info?.fee ??
  ...
  const addCoin = (c) => {
    if (!c) return;
    const denom = String(c?.denom || c?.Denom || "");
    const amtRaw = c?.amount ?? c?.Amount ?? 0;
    // keep precision enough for fees; numbers are small
    const amt = safe(amtRaw);
    if (!amt) return;
  ...
  if (Array.isArray(coins)){
    for (const c of coins) addCoin(c);
  } else if (coins && typeof coins === "object" && (coins.denom || coins.amount)){
    addCoin(coins);
  }

  // Some APIs may expose fee already in INJ (string/number)
  if (!sum){
    const alt = tx?.fee ?? tx?.fees ?? tx?.gas_fee ?? tx?.gasFee;
    if (typeof alt === "number" && Number.isFinite(alt) && alt > 0) sum = alt;
    else if (typeof alt === "string"){
      const v = safe(alt);
      if (Number.isFinite(v) && v > 0 && v < 1000) sum = v; // heuristic
    }
  }
...
async function feesBackfill(force=false){
  ...
  const now = Date.now();
  if (!force && (now - feesLastSyncAt) < 45_000) return;
  feesLastSyncAt = now;

  const have = new Set((feesIdsAll || []).map(h => String(h || "").toUpperCase()));
  let added = 0;

  const mkEvent = (key, quoted=true) => quoted ? `${key}='${address}'` : `${key}=${address}`;

  // Try multiple indexed keys; different nodes can expose different event indexes.
  const eventCandidates = [
    mkEvent("tx.fee_payer", true),
    mkEvent("tx.signer", true),
    mkEvent("message.sender", true),
    mkEvent("message.sender", false),
    mkEvent("tx.signer", false),
    mkEvent("transfer.sender", true),
    mkEvent("delegate.sender", true),
    mkEvent("withdraw_rewards.delegator", true),
  ];

  const evSet = new Set();
  const events = [];
  for (const e of eventCandidates){
    if (!e || evSet.has(e)) continue;
    evSet.add(e);
    events.push(e);
  }

  const LIMIT = 100;
  const MAX_PAGES_PER_EVENT = 6;
  const MAX_NEW_PER_RUN = force ? 600 : 220;
  ...
  for (const evRaw of events){
    ...
    while (pages < MAX_PAGES_PER_EVENT && added < MAX_NEW_PER_RUN){
      const ev = encodeURIComponent(evRaw);
      const keyParam = pageKey ? `&pagination.key=${encodeURIComponent(pageKey)}` : "";
      const url = `/cosmos/tx/v1beta1/txs?events=${ev}&order_by=ORDER_BY_DESC&pagination.limit=${LIMIT}${keyParam}`;

      const j = await fetchLCD(url);
      if (!j) break;

      const txs = Array.isArray(j?.txs) ? j.txs : [];
      const rs  = Array.isArray(j?.tx_responses) ? j.tx_responses : [];

      // Some nodes may omit txs and embed tx inside tx_responses.
      const n = Math.max(txs.length, rs.length);
      if (!n) break;

      for (let i = 0; i < n; i++){
        const tr = rs[i] || {};
        const hash = String(tr.txhash || tr.hash || "").toUpperCase();
        if (!hash || have.has(hash)) continue;

        const tx = txs[i] || tr.tx?.tx || tr.tx || null;
        if (!tx) continue;

        const ts =
          Date.parse(tr.timestamp || tr.block_timestamp || tr.blockTimestamp || "") ||
          safe(tr.block_time) || 0;
      ...
      pageKey = j?.pagination?.next_key || j?.pagination?.nextKey || "";
      if (!pageKey) break;
      pages++;
    }
  }
