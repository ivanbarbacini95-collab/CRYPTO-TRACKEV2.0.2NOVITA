...
async function renderSettingsSnapshot(){
  ...
  try{ renderDataTransferSnapshot(); }catch{}
}

/* ================= DATA TRANSFER (JSON) ================= */
const EXPORT_SCHEMA = "inj-portfolio-export";
const EXPORT_VER = 1;

/* Missing helper (used by mergeWd): keep it here so mergeWd can't crash */
function clampWdAll(maxKeep=2400){
  try{
    const n = Math.min(wdTimesAll.length, wdLabelsAll.length, wdValuesAll.length);
    wdTimesAll  = wdTimesAll.slice(0, n);
    wdLabelsAll = wdLabelsAll.slice(0, n);
    wdValuesAll = wdValuesAll.slice(0, n);

    if (wdTimesAll.length > maxKeep){
      wdTimesAll  = wdTimesAll.slice(-maxKeep);
      wdLabelsAll = wdLabelsAll.slice(-maxKeep);
      wdValuesAll = wdValuesAll.slice(-maxKeep);
    }
  }catch{}
}
...
function dlJSON(obj, filename){
  try{
    ...
    console.warn("dlJSON", e);
    try{ pushToast("Export failed"); }catch{}
  }
}
...
function normalizeStake(anyObj){
  const o = anyObj && typeof anyObj === "object" ? anyObj : {};
  const labels = Array.isArray(o.labels) ? o.labels.map(String) : [];
  const data   = Array.isArray(o.data)   ? o.data.map(safe) : [];
  const moves  = Array.isArray(o.moves)  ? o.moves.map(safe) : [];
  const types  = Array.isArray(o.types)  ? o.types.map(x => String(x || "Stake update")) : [];
  const n = Math.min(labels.length, data.length, moves.length, types.length);
  return { labels: labels.slice(0,n), data: data.slice(0,n), moves: moves.slice(0,n), types: types.slice(0,n) };
}
function normalizeWd(anyObj){
  const o = anyObj && typeof anyObj === "object" ? anyObj : {};
  const labels = Array.isArray(o.labels) ? o.labels.map(String) : [];
  const values = Array.isArray(o.values) ? o.values.map(safe) : [];
  const times  = Array.isArray(o.times)  ? o.times.map(safe)  : [];
  const n = Math.min(labels.length, values.length, times.length);
  return { labels: labels.slice(0,n), values: values.slice(0,n), times: times.slice(0,n) };
}
function normalizeNw(anyObj){
  const o = anyObj && typeof anyObj === "object" ? anyObj : {};
  // accept both storage shape (tAll/usdAll/injAll) and payload shape (times/usd/inj)
  const t = Array.isArray(o.times) ? o.times.map(safe) : (Array.isArray(o.tAll) ? o.tAll.map(safe) : []);
  const u = Array.isArray(o.usd)   ? o.usd.map(safe)   : (Array.isArray(o.usdAll) ? o.usdAll.map(safe) : []);
  const j = Array.isArray(o.inj)   ? o.inj.map(safe)   : (Array.isArray(o.injAll) ? o.injAll.map(safe) : []);
  const n = Math.min(t.length, u.length, j.length);
  return {
    times: t.slice(0,n),
    usd: u.slice(0,n),
    inj: j.slice(0,n),
    tf: (typeof o.tf === "string" ? o.tf : null),
    scale: (typeof o.scale === "string" ? o.scale : null)
  };
}
function normalizeApr(anyObj){
  const o = anyObj && typeof anyObj === "object" ? anyObj : {};
  const labels = Array.isArray(o.labels) ? o.labels.map(String) : [];
  const data   = Array.isArray(o.data)   ? o.data.map(safe) : [];
  const n = Math.min(labels.length, data.length);
  return { labels: labels.slice(0,n), data: data.slice(0,n) };
}
function normalizeEvents(anyObj){
  if (Array.isArray(anyObj)) return anyObj.filter(Boolean);
  if (anyObj && typeof anyObj === "object" && Array.isArray(anyObj.events)) return anyObj.events.filter(Boolean);
  return [];
}
function normalizeWalletPayload(anyObj){
  const o = anyObj && typeof anyObj === "object" ? anyObj : {};
  return {
    stake: normalizeStake(o.stake || o.stakeSeries || o.staked),
    wd: normalizeWd(o.wd || o.withdrawals || o.rewards),
    nw: normalizeNw(o.nw || o.netWorth),
    events: normalizeEvents(o.events || o.ev),
    apr: normalizeApr(o.apr || o.aprSeries),
    snapshot: (o.snapshot && typeof o.snapshot === "object") ? o.snapshot : null
  };
}

function mergeStakePayload(base, add){
  const b = normalizeStake(base);
  const a = normalizeStake(add);
  const map = new Map();
  for (let i=0;i<b.labels.length;i++){
    const k = String(b.labels[i]);
    map.set(k, { d: safe(b.data[i]), m: safe(b.moves[i]), t: String(b.types[i] || "Stake update") });
  }
  for (let i=0;i<a.labels.length;i++){
    const k = String(a.labels[i]);
    if (!map.has(k)) map.set(k, { d: safe(a.data[i]), m: safe(a.moves[i]), t: String(a.types[i] || "Stake update") });
  }
  const keys = [...map.keys()].sort((x,y)=>labelToTs(x)-labelToTs(y));
  const labels = clampArray(keys, 2400);
  return { labels, data: labels.map(k => map.get(k).d), moves: labels.map(k => map.get(k).m), types: labels.map(k => map.get(k).t) };
}
function mergeWdPayload(base, add){
  const b = normalizeWd(base);
  const a = normalizeWd(add);
  const map = new Map();

  for (let i=0;i<b.times.length;i++){
    const tIn = safe(b.times[i]) || labelToTs(b.labels[i]);
    const v = safe(b.values[i]);
    if (!tIn || !Number.isFinite(v) || v <= 0) continue;
    const t = wdDeterministicTs(tIn, v);
    map.set(t, { v, l: String(b.labels[i] || tsLabel(t)) });
  }
  for (let i=0;i<a.times.length;i++){
    const tIn = safe(a.times[i]) || labelToTs(a.labels[i]);
    const v = safe(a.values[i]);
    if (!tIn || !Number.isFinite(v) || v <= 0) continue;
    const t = wdDeterministicTs(tIn, v);
    if (!map.has(t)) map.set(t, { v, l: String(a.labels[i] || tsLabel(t)) });
  }

  const keys = [...map.keys()].sort((x,y)=>x-y);
  const times = keys.slice(-2400);
  return { times, labels: times.map(t => map.get(t).l || tsLabel(t)), values: times.map(t => safe(map.get(t).v)) };
}
function mergeNwPayload(base, add){
  const b = normalizeNw(base);
  const a = normalizeNw(add);
  const m1 = mergeUniqueByTs(b.times, b.usd, a.times, a.usd);
  const m2 = mergeUniqueByTs(b.times, b.inj, a.times, a.inj);
  return { times: m1.times, usd: m1.vals, inj: m2.vals, tf: (b.tf || a.tf || null), scale: (b.scale || a.scale || null) };
}
function mergeAprPayload(base, add){
  const b = normalizeApr(base);
  const a = normalizeApr(add);
  const map = new Map();
  for (let i=0;i<b.labels.length;i++) map.set(String(b.labels[i]), safe(b.data[i]));
  for (let i=0;i<a.labels.length;i++){
    const k = String(a.labels[i]);
    if (!map.has(k)) map.set(k, safe(a.data[i]));
  }
  const keys = [...map.keys()].sort((x,y)=>labelToTs(x)-labelToTs(y));
  const labels = clampArray(keys, 2400);
  return { labels, data: labels.map(k => safe(map.get(k))) };
}
function mergeEventsPayload(baseArr, addArr){
  const rankStatus = (s) => (s === "done" ? 3 : s === "pending" ? 2 : s === "error" ? 1 : 0);
  ...
  const map = new Map();
  const all = []
    .concat(Array.isArray(baseArr) ? baseArr : [])
    .concat(Array.isArray(addArr) ? addArr : []);
...
function getWalletPayloadFromLocal(addr){
  const a = String(addr||"").trim();
  if (!a) return normalizeWalletPayload({});
  const stakeRaw = readLocalJSON(stakeStoreKey(a));
  const wdRaw    = readLocalJSON(wdStoreKey(a));
  const nwRaw    = readLocalJSON(nwStoreKey(a));
  const evRaw    = readLocalJSON(evStoreKey(a));
  const aprRaw   = readLocalJSON(aprStoreKey(a));
  const snap     = loadAccountSnapshot(a);

  return normalizeWalletPayload({
    stake: stakeRaw,
    wd: wdRaw,
    nw: nwRaw,
    events: evRaw,
...
function writeWalletPayloadToLocal(addr, wallet){
  const a = String(addr||"").trim();
  if (!a) return;
  const w = normalizeWalletPayload(wallet);

  try{
    const ks = stakeStoreKey(a);
    if (ks) localStorage.setItem(ks, JSON.stringify({ v: STAKE_LOCAL_VER, t: Date.now(), labels: w.stake.labels, data: w.stake.data, moves: w.stake.moves, types: w.stake.types }));
  }catch{}
  try{
    const kw = wdStoreKey(a);
    if (kw) localStorage.setItem(kw, JSON.stringify({ v: REWARD_WD_LOCAL_VER, t: Date.now(), labels: w.wd.labels, values: w.wd.values, times: w.wd.times }));
  }catch{}
  try{
    const kn = nwStoreKey(a);
    if (kn) localStorage.setItem(kn, JSON.stringify({ v: NW_LOCAL_VER, t: Date.now(), tAll: w.nw.times, usdAll: w.nw.usd, injAll: w.nw.inj, tf: (w.nw.tf || "all"), scale: (w.nw.scale || "lin") }));
  }catch{}
  try{
    const ke = evStoreKey(a);
    if (ke) localStorage.setItem(ke, JSON.stringify({ v: EV_LOCAL_VER, t: Date.now(), events: w.events.slice(0,1200) }));
  }catch{}
  try{
    const ka = aprStoreKey(a);
    if (ka) localStorage.setItem(ka, JSON.stringify({ v: APR_LOCAL_VER, t: Date.now(), labels: w.apr.labels, data: w.apr.data }));
  }catch{}
  try{
    if (w.snapshot && typeof w.snapshot === "object"){
      localStorage.setItem(SNAP_KEY_PREFIX + a, JSON.stringify(w.snapshot));
    }
  }catch{}
}

function collectStoredAddresses(){
  const out = new Set();
  try{
    for (let i=0;i<localStorage.length;i++){
      const k = localStorage.key(i);
      if (!k) continue;
      let m = k.match(/^inj_(?:stake_series_v\d+|reward_withdrawals_v\d+|networth_v\d+|events_v\d+|apr_series_v\d+)_(inj[a-z0-9]{20,80})$/i);
      if (m && m[1]) out.add(m[1]);
      m = k.match(/^inj_snap_(inj[a-z0-9]{20,80})$/i);
      if (m && m[1]) out.add(m[1]);
    }
  }catch{}
  return [...out].filter(isValidInjAddr);
}

function buildExportObject(singleAddr){
  const createdAt = Date.now();
  const wallets = {};
  if (singleAddr && isValidInjAddr(singleAddr)){
    wallets[singleAddr] = getWalletPayloadFromLocal(singleAddr);
  } else {
    const addrs = collectStoredAddresses();
    for (const a of addrs){
      wallets[a] = getWalletPayloadFromLocal(a);
    }
  }
  ...
  return {
    ...
    version: EXPORT_VER,
    createdAt,
    app: "Injective Portfolio",
    appVersion: (window.APP_VERSION || "2.0.2"),
    wallets,
    globals: { theme, viewMode, privacyOn }
  };
}

function renderDataTransferSnapshot(){
  const note = $("dataTransferNote");
  if (!note) return;

  const a = (typeof address === "string" ? address : "").trim();
  const stored = collectStoredAddresses();
  const count = stored.length;
  if (!a){
    note.textContent = `Saved wallets: ${count}`;
    return;
  }
  const w = getWalletPayloadFromLocal(a);
  note.textContent = `Wallet: ${shortAddr(a)} • Stake ${w.stake.labels.length} • NW ${w.nw.times.length} • WD ${w.wd.times.length} • Events ${w.events.length} • APR ${w.apr.labels.length} • Saved wallets: ${count}`;
}

function exportJsonCurrent(){
  const a = (typeof address === "string" ? address : "").trim();
  if (!isValidInjAddr(a)){
    try{ pushToast("Insert a valid address first"); }catch{}
    return;
  }
  const obj = buildExportObject(a);
  const d = new Date(obj.createdAt);
  const stamp = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
  dlJSON(obj, `inj-portfolio_${shortAddr(a)}_${stamp}.json`);
  try{ pushToast("Export ready"); }catch{}
  try{ renderDataTransferSnapshot(); }catch{}
}
function exportJsonAll(){
  const obj = buildExportObject(null);
  const d = new Date(obj.createdAt);
  const stamp = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
  dlJSON(obj, `inj-portfolio_ALL_${stamp}.json`);
  try{ pushToast("Export ready"); }catch{}
  try{ renderDataTransferSnapshot(); }catch{}
}

async function importJsonFromPicker(){
  const fileEl = $("importJsonFile");
  const modeEl = $("importMode");
  const file = fileEl?.files?.[0];
  const mode = String(modeEl?.value || "merge");

  if (!file){
    try{ pushToast("Select a JSON file"); }catch{}
    return;
  }
  ...
  try{
    ...
    console.warn("Import parse", e);
    try{ pushToast("Invalid JSON"); }catch{}
    return;
  }
  ...
  if (parsed && typeof parsed === "object"){
    if (parsed.schema === EXPORT_SCHEMA && parsed.wallets && typeof parsed.wallets === "object"){
      wallets = parsed.wallets;
    } else if (parsed.wallets && typeof parsed.wallets === "object"){
      wallets = parsed.wallets;
    } else if (parsed.address || parsed.addr){
      const a = String(parsed.address || parsed.addr).trim();
      if (a) wallets[a] = (parsed.wallet || parsed.data || parsed.payload || parsed);
    } else if (parsed.stake || parsed.wd || parsed.nw || parsed.events || parsed.apr){
      // no address: import into current wallet (if any)
      const a = (typeof address === "string" ? address : "").trim();
      if (isValidInjAddr(a)) wallets[a] = parsed;
    }
  }

  const addrs = Object.keys(wallets).filter(isValidInjAddr);
  if (!addrs.length){
    try{ pushToast("No wallet data found"); }catch{}
    return;
  }

  let touchedCurrent = false;
  for (const a of addrs){
    const incoming = normalizeWalletPayload(wallets[a]);
    const base = getWalletPayloadFromLocal(a);

    let out = incoming;
    if (mode !== "replace"){
      out = {
        stake: mergeStakePayload(base.stake, incoming.stake),
        wd: mergeWdPayload(base.wd, incoming.wd),
        nw: mergeNwPayload(base.nw, incoming.nw),
        events: mergeEventsPayload(base.events, incoming.events),
        apr: mergeAprPayload(base.apr, incoming.apr),
        snapshot: (base.snapshot || incoming.snapshot || null)
      };
    } else {
      out = {
    ...
    writeWalletPayloadToLocal(a, out);

    if (String(address||"").trim() === a) touchedCurrent = true;
  }

  // If the current wallet was updated, reload in-memory arrays + redraw
  if (touchedCurrent){
    try{ resetPerAddressStateUI(); }catch{}
    try{ loadStakeSeriesLocal(); drawStakeChart(); }catch{}
    try{ loadWdAllLocal(); drawRewardChart(); }catch{}
    try{ loadNWLocal(); drawNW(true); }catch{}
    try{ loadEvents(); renderEvents(); }catch{}
    try{ loadAprLocal(); drawAprChart(); }catch{}
    try{ cloudBumpLocal(1); cloudMarkDirty({ stake:true, wd:true, nw:true, events:true, apr:true }); }catch{}
  }

  try{
    const note = $("dataTransferNote");
    if (note) note.textContent = `Imported ${addrs.length} wallet(s) • ${new Date().toLocaleString()}`;
  }catch{}

  try{ pushToast(`Imported ${addrs.length} wallet(s)`); }catch{}
  try{ renderDataTransferSnapshot(); }catch{}
}

function bindDataTransferUI(){
  const exp = $("exportJsonBtn");
  const expAll = $("exportAllJsonBtn");
  const imp = $("importJsonBtn");
  const file = $("importJsonFile");

  if (exp) exp.onclick = exportJsonCurrent;
  if (expAll) expAll.onclick = exportJsonAll;

  if (imp){
    imp.onclick = ()=> safeAsync(() => importJsonFromPicker(), "importJson");
  }
  if (file){
    file.addEventListener("change", ()=>{
      try{
        ...
        const note = $("dataTransferNote");
        if (note && f) note.textContent = `Selected: ${f.name}`;
      }catch{}
    });
  }
