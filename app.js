...
async function fetchJSON(url, opts = {}) {
  // Robust fetch with timeout to avoid "page stuck loading" on slow/unresponsive endpoints
  const timeoutMs = Number.isFinite(+opts.timeoutMs) ? +opts.timeoutMs : 12_000;

  // Do not pass custom props to fetch()
  ...
  try{
    if (fetchOpts.signal) {
      // AbortSignal.any is supported in modern browsers; fallback to caller signal if not available
  ...
  try {
    const res = await fetch(url, { cache: "no-store", ...fetchOpts, signal });
...
function saveFeesAllLocalSilent(){
  const key = feesStoreKey(address);
  if (!key) return;
  try{
    localStorage.setItem(key, JSON.stringify({
      v: FEES_LOCAL_VER, t: Date.now(),
      ids: (feesIdsAll || []).slice(0, FEES_MAX_POINTS),
      times: (feesTimesAll || []).slice(0, FEES_MAX_POINTS),
      values: (feesValuesAll || []).slice(0, FEES_MAX_POINTS)
    }));
  } catch {}
}
...
let wdLastRewardsSeen = null;
  ...
  feesIdsAll = []; feesTimesAll = []; feesValuesAll = [];
  feesLabels = []; feesValues = []; feesTimes = [];
  feesLastFetchAt = 0;
let wdMinFilter = 0;
...
function rebuildWdView() {
  ...
  try { if (typeof updateFeesTotalsUI === "function") updateFeesTotalsUI(); } catch (e) { console.warn("[updateFeesTotalsUI]", e); }
}
...
function maybeRecordRewardWithdrawal(newRewards) {
  ...
  if (diff > REWARD_WITHDRAW_THRESHOLD) {
    ...
    // Fees are derived from withdraw txs; resync a small window to catch the fee
    safeAsync(() => feesSyncFromChain("withdraw"), "feesSyncFromChain(withdraw)");
...
/* ================= FEES (persist + auto) =================
   Tracks fees paid on reward-withdraw transactions.
   - Boot: pulls latest withdraw txs (lightweight) to populate.
   - On new withdrawal detection: re-syncs last txs to catch the new fee.
   - Stored per-address (local + cloud).
*/
const FEES_MAX_POINTS = 2400;
const FEES_FETCH_COOLDOWN_MS = 12_000;
...
function clampFeesAll(){
  const n = feesTimesAll.length;
  if (n <= FEES_MAX_POINTS) return;
  const cut = n - FEES_MAX_POINTS;
  feesIdsAll   = feesIdsAll.slice(cut);
  feesTimesAll = feesTimesAll.slice(cut);
  feesValuesAll= feesValuesAll.slice(cut);
}
function saveFeesAllLocal(){
  const key = feesStoreKey(address);
  if (!key) return;
  try{
    localStorage.setItem(key, JSON.stringify({
...
function loadFeesAllLocal(){
  ...
  try{
    ...
    feesIdsAll   = Array.isArray(obj.ids) ? obj.ids : [];
    feesTimesAll = Array.isArray(obj.times) ? obj.times : [];
    feesValuesAll= Array.isArray(obj.values) ? obj.values : [];
...
function feesTotalInj(){
  ...
  for (let i=0;i<feesValuesAll.length;i++){
    const v = Number(feesValuesAll[i]);
    if (Number.isFinite(v) && v > 0) s += v;
  }
  return s;
}
function updateFeesTotalsUI(){
  const totEl = $("feesTotal");
  const usdEl = $("feesTotalUsd");
  const lastEl= $("feesLast");

  const total = feesTotalInj();
  if (totEl) totEl.textContent = total.toFixed(6);

  const last = feesValuesAll.length ? safe(feesValuesAll[feesValuesAll.length - 1]) : 0;
  if (lastEl) lastEl.textContent = (last && Number.isFinite(last)) ? `${last.toFixed(6)} INJ` : "—";

  const px = (Number.isFinite(displayed?.price) && displayed.price > 0) ? displayed.price :
             (Number.isFinite(targetPrice) && targetPrice > 0) ? targetPrice : 0;
  if (usdEl) usdEl.textContent = px ? `≈ $${(total * px).toFixed(2)}` : "—";
}
...
function rebuildFeesView(){
  ...
  for (let i=0;i<feesTimesAll.length;i++){
    const t = safe(feesTimesAll[i]);
    const v = safe(feesValuesAll[i]);
    if (!t || !Number.isFinite(v) || v <= 0) continue;
    feesTimes.push(t);
    feesValues.push(v);
    feesLabels.push(tsLabel(t));
  }
...
function initFeesChart(){
  ...
  attachCrosshair2(feesChart, $("feesReadout"), (i, lbs, ds) => {
    ...
    const px = (Number.isFinite(displayed?.price) && displayed.price > 0) ? displayed.price :
               (Number.isFinite(targetPrice) && targetPrice > 0) ? targetPrice : 0;
    const usd = (px && v) ? ` • ≈ $${(v * px).toFixed(4)}` : "";
    return `${t ? new Date(t).toLocaleString() : "—"} • Fee ${v.toFixed(6)} INJ${usd}`;
  });
}
...
/* ===== Chain sync ===== */
function denomToInj(aIn, denom){
  const a = safe(aIn);
  if (!a || !Number.isFinite(a)) return 0;
  const d = String(denom || "").toLowerCase();
  if (d === "inj") return a / 1e18;
  if (d === "uinj") return a / 1e6;
  // heuristic fallback
  return (a > 1e10) ? (a / 1e18) : a;
}
function txHasWithdrawReward(tx){
  const msgs = tx?.body?.messages;
  if (!Array.isArray(msgs) || !msgs.length) return false;
  for (const m of msgs){
    const t = String(m?.["@type"] || m?.type_url || m?.type || "");
    if (t.includes("MsgWithdrawDelegatorReward")) return true;
  }
  return false;
}
function txFeeInj(tx){
  const fee = tx?.auth_info?.fee;
  const arr = fee?.amount;
  if (Array.isArray(arr) && arr.length){
    const it = arr.find(x => String(x?.denom||"").toLowerCase() === "inj" || String(x?.denom||"").toLowerCase() === "uinj") || arr[0];
    return denomToInj(it?.amount, it?.denom);
  }
  return 0;
}
async function fetchTxsByEvents(events, limit=30){
  const evs = Array.isArray(events) ? events : [];
  const qs = evs.map(e => "events=" + encodeURIComponent(String(e))).join("&");
  const path = `/cosmos/tx/v1beta1/txs?${qs}&order_by=ORDER_BY_DESC&pagination.limit=${Math.max(1, Math.min(100, limit|0))}`;
  return fetchLCD(path);
}
async function fetchRecentWithdrawFees(limit=30){
  if (!address || !hasInternet()) return [];
  let res = null;

  const tries = [
    [`message.sender='${address}'`, `message.action='/cosmos.distribution.v1beta1.MsgWithdrawDelegatorReward'`],
    [`message.sender='${address}'`, `message.action='withdraw_delegator_reward'`],
    [`message.sender='${address}'`, `message.module='distribution'`]
  ];

  for (const evs of tries){
    try{
      res = await fetchTxsByEvents(evs, limit);
      const txrs = Array.isArray(res?.tx_responses) ? res.tx_responses : [];
      if (txrs.length) break;
    }catch{ res = null; }
  }

  if (!res){
    try{ res = await fetchTxsByEvents([`message.sender='${address}'`], limit); }catch{ res = null; }
  }

  const txrs = Array.isArray(res?.tx_responses) ? res.tx_responses : Array.isArray(res?.txResponses) ? res.txResponses : [];
  const txs  = Array.isArray(res?.txs) ? res.txs : [];

  const out = [];
  for (let i=0;i<txrs.length;i++){
    const txr = txrs[i];
    const tx = txs[i] || txr?.tx;
    if (!tx) continue;
    if (!txHasWithdrawReward(tx)) continue;

    const fee = txFeeInj(tx);
    if (!Number.isFinite(fee) || fee <= 0) continue;

    const ts = Date.parse(String(txr?.timestamp || ""));
    const ms = Number.isFinite(ts) ? ts : Date.now();
    const id = String(txr?.txhash || txr?.txHash || "");
    out.push({ id, ts: ms, fee: Number(fee.toFixed(6)) });
  }
  return out;
}
function mergeFees(payloadFees){
  if (!payloadFees) return;
  const ids   = Array.isArray(payloadFees.ids) ? payloadFees.ids : [];
  const times = Array.isArray(payloadFees.times) ? payloadFees.times : [];
  const vals  = Array.isArray(payloadFees.values) ? payloadFees.values : [];
  ...
  for (let i=0;i<feesTimesAll.length;i++){
    ...
    const key = id || `t_${t}_${Math.round(v*1e6)}`;
    map.set(key, { id, t, v });
  }

  // remote / chain
  const n = Math.max(ids.length, times.length, vals.length);
  for (let i=0;i<n;i++){
    const t = safe(times[i]);
    const v = safe(vals[i]);
    if (!t || !Number.isFinite(v) || v <= 0) continue;
    const id = String(ids[i] || "");
    const key = id || `t_${t}_${Math.round(v*1e6)}`;
    if (!map.has(key)) map.set(key, { id, t, v });
  }

  const arr = [...map.values()].sort((a,b) => safe(a.t) - safe(b.t));

  feesIdsAll   = arr.map(x => String(x.id || ""));
  feesTimesAll = arr.map(x => safe(x.t));
  feesValuesAll= arr.map(x => Number(safe(x.v).toFixed(6)));

  clampFeesAll();
}
async function feesSyncFromChain(reason=""){
  if (!address || !hasInternet()) return;
  const now = Date.now();
  if ((now - feesLastFetchAt) < FEES_FETCH_COOLDOWN_MS) return;
  feesLastFetchAt = now;

  const items = await fetchRecentWithdrawFees(40);
  if (!items.length) return;

  mergeFees({
    ids: items.map(x => x.id || ""),
    times: items.map(x => x.ts || 0),
    values: items.map(x => x.fee || 0)
  });
...
function cloudSetState(state){
  ...
  if (advWhat){
    ...
    if (cloudDirtyWhat.fees) list.push("Fees");
...
function cloudMarkDirty(what = {}){
  ...
  if (what?.fees) cloudDirtyWhat.fees = true;
...
async function cloudPull(){
  ...
  try{
    ...
    if (data.fees) { try{ saveFeesAllLocalSilent(); } catch {} }
...
function resetPerAddressStateUI(){
  ...
  try{
    ...
    if (feesChart){
      ...
      try { feesChart.update("none"); } catch(e){ console.warn("[feesChart.update]", e); }
    }
    if (netWorthChart){
...
try{ localStorage.setItem(LAST_ADDR_KEY, address); }catch{}
  ...
  const hadFees = loadFeesAllLocal();
  if (!hadFees) rebuildFeesView(); else rebuildFeesView();
  // lightweight backfill from chain (only if online)
  safeAsync(() => feesSyncFromChain("boot"), "feesSyncFromChain(boot)");
...
// cloud pull for this address
  ...
  if (liveMode) safeAsync(() => loadAccount(), "loadAccount(commitAddress)");
  else {
    refreshLoaded = false;
    refreshConnUI();
    safeAsync(() => refreshLoadAllOnce(), "refreshLoadAllOnce(commitAddress)");
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  safeAsync(() => loadCandleSnapshot(liveMode ? false : true), "loadCandleSnapshot(boot)");

  safeAsync(() => loadChartToday(liveMode ? false : true), "loadChartToday(boot)");

  updatePriceTFButtons();
  updatePriceTitle();
  safeAsync(() => loadPriceChart(true), "loadPriceChart(boot)");
  ...
  if (liveMode) {
    ...
    refreshLoaded = false;
    refreshConnUI();
    safeAsync(() => refreshLoadAllOnce(), "refreshLoadAllOnce(boot)");
