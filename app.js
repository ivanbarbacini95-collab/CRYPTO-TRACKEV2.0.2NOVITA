...
const CARD_BORDER_FX_ENABLED_KEY = "inj_card_fx_enabled";
...
let cardBorderFxEnabled = (() => {
  try{
    const raw = String(localStorage.getItem(CARD_BORDER_FX_ENABLED_KEY) || "1").toLowerCase();
...
function applyCardBorderFxEnabled(persist=false){
  cardBorderFxEnabled = !!cardBorderFxEnabled;

  try{
    document.body.classList.toggle("card-fx-off", !cardBorderFxEnabled);
    document.body.dataset.cardFx = cardBorderFxEnabled ? "on" : "off";
  }catch(_){}

  const btn = $("cardFxToggle");
  const rs = $("cardFxSpeedRange");
  const rl = $("cardFxLenRange");
  const os = $("cardFxSpeedReadout");
  const ol = $("cardFxLenReadout");

  if (btn){
    btn.textContent = cardBorderFxEnabled ? "ON" : "OFF";
    btn.classList.toggle("primary", cardBorderFxEnabled);
    btn.setAttribute("aria-pressed", cardBorderFxEnabled ? "true" : "false");
  }

  if (rs){ rs.disabled = !cardBorderFxEnabled; rs.style.opacity = cardBorderFxEnabled ? "1" : ".45"; }
  if (rl){ rl.disabled = !cardBorderFxEnabled; rl.style.opacity = cardBorderFxEnabled ? "1" : ".45"; }
  if (os) os.style.opacity = cardBorderFxEnabled ? "1" : ".55";
  if (ol) ol.style.opacity = cardBorderFxEnabled ? "1" : ".55";

  if (persist){
    try{ localStorage.setItem(CARD_BORDER_FX_ENABLED_KEY, cardBorderFxEnabled ? "1" : "0"); }catch(_){}
  }
}
...
function initCardBorderFxSettings(){
  const rs = $("cardFxSpeedRange");
  const rl = $("cardFxLenRange");
  const bt = $("cardFxToggle");

  applyCardBorderFxVars(false);
  applyCardBorderFxEnabled(false);
  ...
  if (bt && !bt._injBoundCardFxToggle){
    bt._injBoundCardFxToggle = true;
    bt.addEventListener("click", (e) => {
      e?.preventDefault?.();
      cardBorderFxEnabled = !cardBorderFxEnabled;
      applyCardBorderFxEnabled(true);
...
async function renderSettingsSnapshot(){
  ...
  try{
    const fxBtn = $("cardFxToggle");
    if (fxBtn){
      fxBtn.textContent = cardBorderFxEnabled ? "ON" : "OFF";
      fxBtn.classList.toggle("primary", !!cardBorderFxEnabled);
    }
  }catch(_){}
