...
const TAB_ADDR_KEY = "inj_address_tab_v1";
const GLOBAL_ADDR_KEY = "inj_address";

let address = (sessionStorage.getItem(TAB_ADDR_KEY) || localStorage.getItem(GLOBAL_ADDR_KEY) || "").trim();

function persistActiveAddress(addr){
  const a = String(addr||"").trim();
  try{ sessionStorage.setItem(TAB_ADDR_KEY, a); }catch{}
  try{ localStorage.setItem(GLOBAL_ADDR_KEY, a); }catch{}
}
// ensure this tab keeps its current address across refresh
try{ if (address) sessionStorage.setItem(TAB_ADDR_KEY, address); }catch{}
...
/* ================= WATCH ADDRESS LIST (favorites) ================= */
const WATCH_KEY = "inj_watch_addrs_v1";

const watchBar = $("watchBar");
const watchBarScroll = $("watchBarScroll");

const watchAddrInput = $("watchAddrInput");
const watchSaveBtn = $("watchSaveBtn");
...
function loadWatchAddrs(){
  try{
    const raw = localStorage.getItem(WATCH_KEY);
    if (!raw) return [];
    const arr = JSON.parse(raw);
    if (!Array.isArray(arr)) return [];
    // keep only plausible inj addresses, unique
    const out = [];
    const seen = new Set();
    for (const a of arr){
      const x = normAddr(a);
      if (!x) continue;
      if (!/^inj[a-z0-9]{20,80}$/i.test(x)) continue;
...
function saveWatchAddrs(arr){
  try{ localStorage.setItem(WATCH_KEY, JSON.stringify(arr || [])); }catch{}
}
let watchAddrs = loadWatchAddrs();

function addWatchAddr(addr){
  ...
  if (!watchAddrs.some(x => x.toLowerCase() === key)){
    watchAddrs = [a, ...watchAddrs].slice(0, 60); // cap
    saveWatchAddrs(watchAddrs);
    pushEvent({ kind:"done", title:"Saved", detail: shortAddr(a), status:"done" });
  } else {
    pushEvent({ kind:"info", title:"Already saved", detail: shortAddr(a), status:"ok" });
  }
  renderWatchUI();
}
function removeWatchAddr(addr){
  const a = normAddr(addr);
  if (!a) return;
  const key = a.toLowerCase();
  watchAddrs = watchAddrs.filter(x => x.toLowerCase() !== key);
  saveWatchAddrs(watchAddrs);
  renderWatchUI();
}

function renderWatchBar(){
  if (!watchBar || !watchBarScroll) return;
  const has = (watchAddrs.length > 0);
  watchBar.style.display = has ? "flex" : "none";
  if (!has) return;

  watchBarScroll.innerHTML = "";
  for (const a of watchAddrs){
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "watchchip";
    btn.setAttribute("role","tab");
    btn.setAttribute("aria-label", `Open ${shortAddr(a)}`);
    if (address && a.toLowerCase() === address.toLowerCase()){
      btn.classList.add("active");
    }
    btn.innerHTML = `<span class="dot" aria-hidden="true"></span><span>${shortAddr(a)}</span>`;
    btn.addEventListener("click", () => {
      if (a && a !== address) commitAddress(a);
      setActivePage("dashboard");
      showPage("dashboard");
    }, { passive:true });
    watchBarScroll.appendChild(btn);
  }
}
...
function renderWatchList(){
  ...
  watchList.innerHTML = "";
  watchEmpty.style.display = (watchAddrs.length ? "none" : "block");
  ...
  for (const a of watchAddrs){
    ...
    del.addEventListener("click", () => removeWatchAddr(a), { passive:true });
...
watchSaveBtn?.addEventListener("click", () => {
  addWatchAddr(watchAddrInput?.value || "");
...
/* ================= PULL TO REFRESH (refresh mode) ================= */
const pullRefresh = $("pullRefresh");
const pullText = $("pullText");
const pullInner = $("pullInner");

function setPullText(t){
  if (pullText) pullText.textContent = String(t||"");
}
function showPullUI(){
...
function canPullNow(){
  ...
  const pd = document.getElementById("pageDashboard");
  if (!pd || !pd.classList.contains("active")) return false;
...
window.addEventListener("touchmove", (e) => {
  if (!pulling) return;
  if (!canPullNow()) { pulling = false; return; }
  const t = e.touches && e.touches[0];
  if (!t) return;

  pullDist = Math.max(0, t.clientY - pullStartY);
  if (pullDist <= 2) return;

  // prevent native overscroll once we are pulling
  try{ e.preventDefault(); }catch{}
...
window.addEventListener("touchend", () => {
  if (!pulling) return;
  const doIt = pullArmed;
  pulling = false;
  pullArmed = false;
  pullDist = 0;
  if (doIt) doManualRefresh();
  else hidePullUI();
}, { passive:true });
...
/* ================= INJ VOLATILITY INDEX ================= */
const volGauge = $("volGauge");
const volTfSwitch = $("volTfSwitch");
const volIndexVal = $("volIndexVal");
const volStateEl = $("volState");
const volAnnEl = $("volAnn");
const volTfPill = $("volTfPill");

const VOL_TF_KEY = "inj_vol_tf";
let volTf = (localStorage.getItem(VOL_TF_KEY) || "15m").toLowerCase();

const VOL_TFS = {
  "15m": { label:"15M", interval:"1m",  window:15,  intervalSec:60,    limit:120 },
  "1h":  { label:"1H",  interval:"1m",  window:60,  intervalSec:60,    limit:180 },
  "4h":  { label:"4H",  interval:"5m",  window:48,  intervalSec:300,   limit:240 },
  "1d":  { label:"1D",  interval:"15m", window:96,  intervalSec:900,   limit:220 },
  "1w":  { label:"1W",  interval:"4h",  window:42,  intervalSec:14400, limit:120 },
};
...
function volState(idx){
  if (idx < 20) return "CALM";
  if (idx < 40) return "NORMAL";
  if (idx < 60) return "ACTIVE";
  if (idx < 80) return "HOT";
  return "EXTREME";
}
function fmtPct(n){
...
function computeAnnVol(closes, intervalSec){
  const xs = (closes || []).filter(v => Number.isFinite(v) && v > 0);
  if (xs.length < 8) return null;

  const rets = [];
  for (let i=1; i<xs.length; i++){
    const r = Math.log(xs[i] / xs[i-1]);
  ...
  v = v / Math.max(1, (rets.length - 1));
  const st = Math.sqrt(v);

  const secYear = 365 * 24 * 3600;
  const periodsPerYear = secYear / Math.max(1, intervalSec);
  const ann = st * Math.sqrt(periodsPerYear) * 100;
  return ann;
}
function volIndexFromAnn(ann){
  // Map 0%..200% annualized -> 0..100 index
  return clamp((ann / 200) * 100, 0, 100);
}

function setVolTf(tf){
  const t = (tf || "15m").toLowerCase();
  if (!VOL_TFS[t]) return;
  volTf = t;
  try{ localStorage.setItem(VOL_TF_KEY, volTf); }catch{}
  updateVolTfButtons();
  volFetch(true);
}
function updateVolTfButtons(){
  if (volTfPill) volTfPill.textContent = (VOL_TFS[volTf]?.label || volTf).toUpperCase();
  const btns = volTfSwitch?.querySelectorAll(".tf-btn") || [];
  btns.forEach(b => b.classList.toggle("active", (b.dataset.vtf || "").toLowerCase() === volTf));
}
volTfSwitch?.addEventListener("click", (e) => {
  const btn = e.target?.closest(".tf-btn");
  if (!btn) return;
  const tf = (btn.dataset.vtf || "").toLowerCase();
  setVolTf(tf);
}, { passive:true });

function updateVolText(){
  if (volIndexVal) volIndexVal.textContent = Number.isFinite(volTarget) ? String(Math.round(volTarget)) : "--";
  const st = volState(volTarget);
  if (volStateEl) volStateEl.textContent = st;
  if (volAnnEl) volAnnEl.textContent = `Annualized ${fmtPct(volAnnPct)}`;
}
...
function drawVolGauge(){
  if (!volGauge) return;
  resizeCanvasToCSS(volGauge);

  const ctx = volGauge.getContext("2d");
  if (!ctx) return;

  const w = volGauge.width;
  const h = volGauge.height;
  ...
  const colYellow = cssVar("--yellow") || "#facc15";
  const colAmber  = cssVar("--amber")  || "#f59e0b";
  const colRed    = cssVar("--red")    || "#ef4444";

  const segs = [
    { a0:0,  a1:20, c:colGreen },
    { a0:20, a1:40, c:colBlue },
    { a0:40, a1:60, c:colYellow },
    { a0:60, a1:80, c:colAmber },
    { a0:80, a1:100,c:colRed },
  ];

  // Base track
  ctx.save();
  ctx.lineCap = "round";
  ctx.lineWidth = Math.max(10*dpr, r*0.10);
  ctx.globalAlpha = 0.22;
  ctx.strokeStyle = "rgba(249,250,251,.22)";
  if (document.body.dataset.theme === "light") ctx.strokeStyle = "rgba(15,23,42,.18)";
  ctx.beginPath();
  ...
  ctx.save();
  ctx.lineCap = "round";
  ctx.lineWidth = Math.max(9*dpr, r*0.085);
  ctx.globalAlpha = 0.92;
  for (const s of segs){
    const aA = start + (s.a0/100)*span;
    const aB = start + (s.a1/100)*span;
    ctx.strokeStyle = s.c;
    ctx.beginPath();
    ctx.arc(cx, cy, r, aA, aB);
    ctx.stroke();
  }
  ctx.restore();
  ...
  // Ticks
  ctx.save();
  const tickCol = (document.body.dataset.theme === "light") ? "rgba(15,23,42,.50)" : "rgba(249,250,251,.45)";
  ctx.strokeStyle = tickCol;
  for (let i=0;i<=100;i+=2){
    const a = start + (i/100)*span;
    const major = (i % 10 === 0);
    const len = major ? r*0.14 : r*0.08;
    const r0 = r - (r*0.12);
    const r1 = r0 - len;
    ctx.lineWidth = major ? (2*dpr) : (1*dpr);
    ctx.beginPath();
  ...
  // Labels (CALM..EXTREME)
  ctx.save();
  ctx.fillStyle = (document.body.dataset.theme === "light") ? "rgba(15,23,42,.62)" : "rgba(249,250,251,.60)";
  ctx.font = `${Math.max(10*dpr, r*0.12)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
  ...
  for (const L of labels){
    const a = start + (L.at/100)*span;
    const rr = r - (r*0.34);
    ctx.fillText(L.t, cx + Math.cos(a)*rr, cy + Math.sin(a)*rr);
  }
  ctx.restore();

  // Needle (animated)
  const idx = clamp(volShown, 0, 100);
  const needleA = start + (idx/100)*span;
  ...
  ctx.lineWidth = Math.max(3*dpr, r*0.03);
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(cx, cy);
  ...
  // Hub
  ctx.save();
  ctx.fillStyle = (document.body.dataset.theme === "light") ? "rgba(15,23,42,.78)" : "rgba(249,250,251,.70)";
  ctx.beginPath();
  ctx.arc(cx, cy, Math.max(7*dpr, r*0.06), 0, Math.PI*2);
  ctx.fill();
  ctx.fillStyle = "rgba(250,204,21,.92)";
  ctx.beginPath();
  ctx.arc(cx, cy, Math.max(3*dpr, r*0.026), 0, Math.PI*2);
  ctx.fill();
  ctx.restore();

  // Value text
  ctx.save();
  ctx.fillStyle = (document.body.dataset.theme === "light") ? "rgba(15,23,42,.88)" : "rgba(249,250,251,.88)";
  ctx.font = `900 ${Math.max(24*dpr, r*0.22)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText(String(Math.round(idx)), cx, cy - r*0.34);
  ctx.font = `800 ${Math.max(12*dpr, r*0.12)}px ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial`;
  ctx.globalAlpha = 0.72;
  ctx.fillText("VOL INDEX", cx, cy - r*0.22);
  ctx.restore();
}

function volAnimate(){
  const hidden = (!volGauge || document.body.dataset.view === "lite" || volGauge.offsetParent === null);
  if (hidden){
    // keep CPU low when canvas is not visible
    setTimeout(() => requestAnimationFrame(volAnimate), 650);
    return;
  }
  const diff = volTarget - volShown;
  volShown += diff * 0.14;
  if (Math.abs(diff) < 0.08) volShown = volTarget;
  drawVolGauge();
  requestAnimationFrame(volAnimate);
}

async function volFetch(force=false){
  if (!hasInternet()) return;
  const cfg = VOL_TFS[volTf] || VOL_TFS["15m"];
  const now = Date.now();

  const minGap = liveMode ? 15000 : 0;
  if (!force && minGap && (now - volLastFetch) < minGap) return;
  if (volFetchLock) return;

  volFetchLock = true;
  try{
    const url = `https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=${cfg.interval}&limit=${cfg.limit}`;
    ...
    const slice = closes.slice(-Math.max(8, cfg.window + 1));
    const ann = computeAnnVol(slice, cfg.intervalSec);
    if (!Number.isFinite(ann)) return;

    volAnnPct = ann;
    volTarget = volIndexFromAnn(ann);
    updateVolText();
    updateVolTfButtons();
    volLastFetch = now;
...
function startVolTimer(){
  stopVolTimer();
  if (!liveMode) return;
  volTimer = setInterval(() => { volFetch(false); }, 15000);
}
function stopVolTimer(){
...
const CARD_CATALOG = [
  ...
  { id:"injVolCard",          name:"INJ Volatility Index" },
];
...
function setMode(isLive){
  ...
  if (!liveMode) {
    stopAllTimers();
    stopAllSockets();
    try{ stopVolTimer(); }catch{}
    try{ hidePullUI(); setPullText("Pull to refresh"); }catch{}
    wsTradeOnline = false;
    wsKlineOnline = false;
    accountOnline = false;
    ...
    modeLoading = false;
    refreshConnUI();
    // REFRESH mode: no automatic updates. Pull down to refresh.
    try{ setPullText("Pull to refresh"); }catch{}
} else {
    ...
    startTradeWS();
    startKlineWS();
    try{ startVolTimer(); }catch{}
    try{ volFetch(true); }catch{}
    loadCandleSnapshot();
    loadChartToday();
...
(async function boot() {
  ...
  applyPrivacy(privacyOn);
  try{ renderWatchUI(); }catch{}
  try{ updateVolTfButtons(); updateVolText(); }catch{}
  try{ requestAnimationFrame(volAnimate); }catch{}
  try{ volFetch(true); }catch{}
  try{ if (liveMode) startVolTimer(); }catch{}
  updatePriceScaleBtn();
