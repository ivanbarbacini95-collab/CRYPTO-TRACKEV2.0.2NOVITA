...
/* cloud prefs */
const CLOUD_PREF_VER = 1;
const CLOUD_PREF_KEY = `inj_cloudprefs_v${CLOUD_PREF_VER}`;
let cloudEnabled = true;
let cloudLeader = false;

function cloudLoadPrefs(){
  try{
    const raw = localStorage.getItem(CLOUD_PREF_KEY);
    if (!raw) return;
    const obj = JSON.parse(raw);
    cloudEnabled = (obj?.enabled !== false);
    cloudLeader = !!obj?.leader;
  } catch {}
}
function cloudSavePrefs(){
  try{ localStorage.setItem(CLOUD_PREF_KEY, JSON.stringify({ v:CLOUD_PREF_VER, enabled: !!cloudEnabled, leader: !!cloudLeader })); } catch {}
}
...
function cloudSetState(state){
  ...
  if (st){
    if (!cloudEnabled) st.textContent = "Cloud: OFF";
    else if (state === "saving") st.textContent = hasInternet() ? "Cloud: Saving" : "Cloud: Offline cache";
  ...
  // menu
  if (cloudDotMenu) {
    cloudDotMenu.classList.remove("ok","saving","err","off");
    if (!cloudEnabled) cloudDotMenu.classList.add("off");
    else if (state === "saving") cloudDotMenu.classList.add("saving");
    else if (state === "error") cloudDotMenu.classList.add("err");
    else cloudDotMenu.classList.add("ok");
  }
  if (cloudTextMenu) {
    cloudTextMenu.textContent = (!cloudEnabled) ? "Off"
      : (state === "saving") ? "Saving"
      : (state === "error") ? "Error"
  ...
  if (advDot) {
    advDot.classList.remove("ok","saving","err","off");
    if (!cloudEnabled) advDot.classList.add("off");
    else if (state === "saving") advDot.classList.add("saving");
  ...
  // Settings page
  const sDot = $("cloudSettingsDot");
  const sText = $("cloudSettingsText");
  const sLast = $("cloudSettingsLast");
  if (sDot){
    sDot.classList.remove("ok","saving","err","off");
    if (!cloudEnabled) sDot.classList.add("off");
    else if (state === "saving") sDot.classList.add("saving");
    else if (state === "error") sDot.classList.add("err");
    else sDot.classList.add("ok");
  }
  if (sText){
    sText.textContent = (!cloudEnabled) ? "Off"
      : (state === "saving") ? (hasInternet() ? "Saving…" : "Offline cache")
      : (state === "error") ? "Error"
      : (hasInternet() ? "Synced" : "Offline cache");
  }
  if (sLast){
    sLast.textContent = cloudLastSync ? new Date(cloudLastSync).toLocaleString() : "—";
  }
...
async function cloudPull(){
  if (!address) return;
  if (!cloudEnabled) { cloudSetState("off"); return; }
...
async function cloudPush(){
  ...
  const loadUrl = `${CLOUD_LOAD_API}?address=${encodeURIComponent(address)}`;
  const saveUrl = `${CLOUD_SAVE_API}?address=${encodeURIComponent(address)}`;
  // merge-before-post: pull latest first to avoid overwrites (multi-device safe)
try{
  const pulled = await fetchJSON(loadUrl);
...
const res = await fetchJSON(saveUrl, {
    method: "POST",
...
async function cloudForceSync(){
  ...
  try{
    await cloudPull();
    // after pull, push current merged state immediately
    cloudDirty = true;
    cloudDirtyWhat = { stake:true, wd:true, nw:true, events:true };
    await cloudPush();
  } catch (e){
    console.warn("[cloudForceSync] error", e);
    cloudSetState("error");
  }
}
...
async function renderSettingsSnapshot(){
  ...
  // Cloud settings UI
  const enBtn = $("cloudEnabledBtn");
  const ldBtn = $("cloudLeaderBtn");
  if (enBtn) enBtn.textContent = cloudEnabled ? "ON" : "OFF";
  if (ldBtn) ldBtn.textContent = cloudLeader ? "ON" : "OFF";
  cloudSetState(cloudEnabled ? "synced" : "off");
...
(async function boot() {
  ...
  cloudLoadMeta();
  cloudRenderMeta();
  // Cloud settings controls
  const enBtn = $("cloudEnabledBtn");
  const ldBtn = $("cloudLeaderBtn");
  const fcBtn = $("cloudForceBtn");
  ...
  enBtn?.addEventListener("click", () => {
    ...
    renderCloudButtons();
    cloudSetState(cloudEnabled ? "synced" : "off");
    // if just enabled, pull once to align
    if (cloudEnabled) { cloudPull().catch(()=>{}); }
  });
