...
function tuneTickerSpeed(){
  const track = $("tickerTrack");
  if (!track) return;
  // width of half-track (because we duplicated)
  const half = track.scrollWidth / 2;
  if (!half || !Number.isFinite(half)) return;
  const pxPerSec = 70; // readable on mobile/desktop
...
function updateMarketInjTargets(){
  ...
  const o = tfReady?.d ? safe(candle?.d?.open) : marketTarget.injOpen;
  const h = tfReady?.d ? safe(candle?.d?.high) : marketTarget.injHigh;
  const l = tfReady?.d ? safe(candle?.d?.low)  : marketTarget.injLow;
  ...
  const open = safe(marketTarget.injOpen);
  if (open > 0 && Number.isFinite(px)) marketPerf.inj24h = pctChange(px, open);
}
...
function updateMarketTickerUI(){
  ...
  for (const it of marketTickerCfg){
    const key = it.key;
    const tgt = safe(marketTarget[key]);
    const cur = safe(marketTarget[key + "__disp"]);
    ...
    if (it.perfKey){
      const pv = safe(marketPerf[it.perfKey]);
      renderTickerPerf(key, pv);
    }
  }
}
...
function setAddressDisplay(addr) {
  ...
  addressDisplay.innerHTML = `<span class="tag"><strong>Wallet:</strong> ${shortAddr(addr)} <button class="wallet-copy-btn" id="copyWalletBtn" type="button" aria-label="Copy wallet address" title="Copy wallet address">⧉</button></span>`;
}
