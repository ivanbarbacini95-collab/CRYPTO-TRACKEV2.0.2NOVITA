...
const CLOUD_LOAD_API = "/api/load";
const CLOUD_SAVE_API = "/api/save";
...
/* ================= CLOUD SYNC (Settings + Force Sync + Leader) ================= */
let CLOUD_ENABLED = true;      // per-address persisted
let CLOUD_LEADER = false;      // per-address persisted
let cloudLastOkAt = 0;

function cloudKey(addr){ return `inj_cloud_cfg__${(addr||"").trim().toLowerCase()}`; }
function loadCloudCfg(addr){
  try{
    const raw = localStorage.getItem(cloudKey(addr));
    if(!raw) return;
    const cfg = JSON.parse(raw);
    if(typeof cfg.enabled === "boolean") CLOUD_ENABLED = cfg.enabled;
    if(typeof cfg.leader === "boolean") CLOUD_LEADER = cfg.leader;
  }catch(_){}
}
function saveCloudCfg(addr){
  try{
    localStorage.setItem(cloudKey(addr), JSON.stringify({ enabled: CLOUD_ENABLED, leader: CLOUD_LEADER, ts: Date.now() }));
  }catch(_){}
}

function setCloudDotState(state, text){
  const dots = [ $("cloudMenuDot"), $("cloudSettingsDot") ].filter(Boolean);
  dots.forEach(d => { d.classList.remove("online","loading","offline"); d.classList.add(state); });
  const a = $("cloudMenuStatus");
  const b = $("cloudSettingsStatus");
  if(a) a.textContent = text || state;
  if(b) b.textContent = text || state;
}

// Wrapper to reuse your single internet-dot during refresh loading (already in your code),
// but keep separate cloud-dot indicators in menu/settings.
async function cloudLoad(addr){
  if(!CLOUD_ENABLED) return null;
  try{
    setCloudDotState("loading","Loading");
    const r = await fetch(`${CLOUD_LOAD_API}?address=${encodeURIComponent(addr)}`, { method:"GET" });
    if(!r.ok) throw new Error("load failed");
    const data = await r.json();
    setCloudDotState("online","Online");
    cloudLastOkAt = Date.now();
    return data || null;
  }catch(e){
    setCloudDotState("offline","Offline");
    return null;
  }
}

async function cloudSave(addr, payload){
  if(!CLOUD_ENABLED) return false;
  try{
    setCloudDotState("loading","Loading");
    const r = await fetch(`${CLOUD_SAVE_API}?address=${encodeURIComponent(addr)}`, {
      method:"POST",
      headers:{ "Content-Type":"application/json" },
      body: JSON.stringify({ payload, leader: !!CLOUD_LEADER, updatedAt: Date.now() })
    });
...
// These functions MUST exist in your app already (they do in your full build):
// - exportFullStateForAddress(addr)   -> object
// - importFullStateForAddress(addr, payload) -> merges/applies
// If names differ, we alias to the existing ones below.
function _getLocalPayload(addr){
  if(typeof exportFullStateForAddress === "function") return exportFullStateForAddress(addr);
  if(typeof collectLocalPayload === "function") return collectLocalPayload(addr);
  // fallback: minimal
  return { address: addr, updatedAt: Date.now() };
}
function _applyCloudPayload(addr, payload){
  if(!payload) return;
  if(typeof importFullStateForAddress === "function") return importFullStateForAddress(addr, payload);
  if(typeof applyCloudPayload === "function") return applyCloudPayload(addr, payload);
}

async function forceCloudSync(addr){
  if(!addr) return;
  // If leader: push local -> pull -> apply (confirm)
  // Else: pull -> apply -> push (merge back)
  if(CLOUD_LEADER){
    const local = _getLocalPayload(addr);
    await cloudSave(addr, local);
    const remote = await cloudLoad(addr);
    if(remote?.payload) _applyCloudPayload(addr, remote.payload);
  }else{
    const remote = await cloudLoad(addr);
    if(remote?.payload) _applyCloudPayload(addr, remote.payload);
    const local = _getLocalPayload(addr);
...
function wireCloudSettingsUI(){
  const t = $("cloudSyncToggle");
  const l = $("cloudLeaderToggle");
  const f = $("cloudForceSync");

  if(t){
    t.textContent = CLOUD_ENABLED ? "ON" : "OFF";
    t.classList.toggle("primary", CLOUD_ENABLED);
    t.onclick = () => {
      CLOUD_ENABLED = !CLOUD_ENABLED;
      t.textContent = CLOUD_ENABLED ? "ON" : "OFF";
      t.classList.toggle("primary", CLOUD_ENABLED);
      if(currentWalletAddress) saveCloudCfg(currentWalletAddress);
      setCloudDotState(CLOUD_ENABLED ? "online" : "offline", CLOUD_ENABLED ? "Online" : "Disabled");
    };
  }
  if(l){
    l.textContent = CLOUD_LEADER ? "ON" : "OFF";
    l.classList.toggle("primary", CLOUD_LEADER);
    l.onclick = () => {
      CLOUD_LEADER = !CLOUD_LEADER;
      l.textContent = CLOUD_LEADER ? "ON" : "OFF";
      l.classList.toggle("primary", CLOUD_LEADER);
      if(currentWalletAddress) saveCloudCfg(currentWalletAddress);
    };
  ...
  if(f){
    f.onclick = async () => {
      ...
      await forceCloudSync(currentWalletAddress);
    };
  }
}

// Keep these globals aligned with your app (best-effort)
