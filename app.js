...
function nwBuildView(tf){
  ...
  // Break the line on offline/sleep gaps to avoid "straight fake lines"
  const BREAK_MS = 2 * 60 * 1000; // 2 minutes
  let lastT = null;
  ...
  for (let i = 0; i < nwTAll.length; i++){
    ...
    if (t >= minT && Number.isFinite(u) && u > 0) {
      if (lastT != null && (t - lastT) > BREAK_MS) {
        // Insert a null point to force a visible gap
        labels.push(tsLabel(t - 1));
        data.push(null);
      }
