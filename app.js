...
function setCopyButtonState(ok){
  const btn = $("copyWalletBtn") || $("copyAddrBtn");
  if (!btn) return;
...
$("copyWalletBtn")?.addEventListener("click", async (e) => {
  e?.preventDefault?.();
  const a = String(address||"").trim();
  if (!a) return;
  try{
...
/* ================= PULL TO REFRESH (Dashboard • REFRESH mode) ================= */
const pullRefreshEl = $("pullRefresh");
...
function prSet(visible, loading=false, text=""){
  if (!pullRefreshEl) return;
  pullRefreshEl.classList.toggle("show", !!visible);
  pullRefreshEl.classList.toggle("loading", !!loading);
  if (pullRefreshText && text) pullRefreshText.textContent = text;
  if (!visible){
    pullRefreshEl.style.transform = "";
    pullRefreshEl.style.opacity = "";
  }
}
...
function prCanPull(){
  ...
  return (!liveMode) && onDash && !refreshLoading && !prRefreshing && (window.scrollY <= 0);
}

function prUpdateVisual(dy){
  if (!pullRefreshEl) return;
  const cap = Math.min(86, Math.max(0, dy));
  // Smooth "follow finger"
  pullRefreshEl.style.opacity = String(Math.min(1, cap / 60));
  pullRefreshEl.style.transform = `translate3d(-50%, ${cap - 24}px, 0)`;
}
...
window.addEventListener("touchstart", (e) => {
  ...
  prDy = 0;
  prReady = false;
  prSet(true, false, "Pull to refresh");
  prUpdateVisual(0);
}, { passive:true });
...
window.addEventListener("touchmove", (e) => {
  ...
  prDy = dy;
  prReady = dy >= 62;
  prSet(true, false, prReady ? "Release to refresh" : "Pull to refresh");
  prUpdateVisual(dy);
  // prevent overscroll bounce while pulling
  e.preventDefault();
}, { passive:false });
...
window.addEventListener("touchend", async () => {
  ...
  if (liveMode){ prSet(false); return; }
  prRefreshing = true;
  prSet(true, true, "Refreshing…");
  try{
    await refreshLoadAllOnce();
  } catch {}
  prSet(true, false, "Updated");
  setTimeout(() => { prSet(false); prRefreshing = false; }, 700);
}, { passive:true });
...
const candle = {
  ...
  t: { t: 0, open: 0, high: 0, low: 0 }, // TOTAL (project start → today)
};
const tfReady = { d: false, w: false, m: false, y: false, t: false };
...
function startTradeWS() {
  ...
  wsTrade.onmessage = (e) => {
    ...
    if (tfReady.t) { candle.t.high = Math.max(candle.t.high, p); candle.t.low = Math.min(candle.t.low, p); }
  };
}
...
/* ================= PROJECT START (for TOTAL bar) ================= */
const PROJECT_START_KEY = "inj_project_start_ms";
// Project creation date (UTC). Used for the INJ Price TOTAL bar (from start → today).
const PROJECT_START_DEFAULT_MS = Date.UTC(2026, 1, 10, 0, 0, 0); // 2026-02-10

function getProjectStartMs(){
  let v = +localStorage.getItem(PROJECT_START_KEY);
  if (Number.isFinite(v) && v > 0) return v;
  v = PROJECT_START_DEFAULT_MS;
  try{ localStorage.setItem(PROJECT_START_KEY, String(v)); } catch {}
  return v;
}

async function fetchKlinesPaged(interval, startTime, endTime){
  const out = [];
  let st = Math.max(0, safe(startTime));
  const en = Math.max(st, safe(endTime));
  if (!st || !en || en <= st) return out;

  // Binance limit is 1000; paginate until end
  for (let guard=0; guard<12; guard++){
    const url = "https://api.binance.com/api/v3/klines?symbol=INJUSDT"
      + `&interval=${encodeURIComponent(interval)}`
      + `&startTime=${Math.floor(st)}`
      + `&endTime=${Math.floor(en)}`
      + "&limit=1000";
    ...
    const lastOpen = safe(chunk[chunk.length - 1]?.[0]);
    if (!lastOpen) break;

    // advance start: next candle
    st = lastOpen + 1;
...
async function loadTotalCandleSnapshot(isRefresh=false){
  if (!isRefresh && !liveMode) return;
  if (!hasInternet()) return;

  const startMs = getProjectStartMs();
  const endMs = Date.now();
  const days = (endMs - startMs) / 86400000;

  let interval = "1d";
  if (days > 980) interval = "1w";
  if (days > 2400) interval = "1M";

  const kl = await fetchKlinesPaged(interval, startMs, endMs);
  if (!Array.isArray(kl) || !kl.length) return;

  const first = kl[0];
  candle.t.t = safe(first?.[0]);
  candle.t.open = safe(first?.[1]);
  ...
  candle.t.high = Number.isFinite(hi) ? hi : 0;
  candle.t.low  = Number.isFinite(lo) ? lo : 0;

  if (candle.t.open && candle.t.high && candle.t.low) tfReady.t = true;
}
...
async function loadCandleSnapshot(isRefresh=false) {
  ...
  try{ await loadTotalCandleSnapshot(isRefresh); } catch {}
}
...
function animate() {
  ...
  const pT = tfReady.t ? pctChange(targetPrice, candle.t.open) : 0;
  ...
  updatePerf("arrowTotal", "pctTotal", pT);
  ...
  const tUp   = "linear-gradient(90deg, rgba(34,197,94,.50), rgba(59,130,246,.24))";
  const tDown = "linear-gradient(270deg, rgba(239,68,68,.42), rgba(59,130,246,.22))";
  ...
  renderBar($("totalBar"), $("totalLine"), targetPrice, candle.t.open, candle.t.low, candle.t.high, tUp, tDown);
  ...
  if (tfReady.t) {
    setText("totalMin", safe(candle.t.low).toFixed(3));
    setText("totalOpen", safe(candle.t.open).toFixed(3));
    setText("totalMax", safe(candle.t.high).toFixed(3));
  } else { setText("totalMin", "--"); setText("totalOpen", "--"); setText("totalMax", "--"); }
