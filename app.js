...
==================================================== */
const TICKER_MARKET_MS = 15_000;
const TICKER_MCAP_MS   = 60_000;
const TICKER_BLOCK_MS  = 4_500;

let __tickerStarted = false;
let __tickerClockT = null, __tickerMarketT = null, __tickerMcapT = null, __tickerBlockT = null;
...
async function tickerFetchBinance24h(){
  ...
  try{
    ...
    setTickerText("injChange", fmtPct2(pct).padStart(9, "\u00A0"));
    setTickerChangeClasses(pct);
    setTickerText("vol24h", fmtUSDShort(volQ).padStart(11, "\u00A0"));
  } catch {}
}
...
async function tickerFetchMarketCap(){
  if (!hasInternet()) return;
  try{
    const d = await fetchJSON("https://api.coingecko.com/api/v3/simple/price?ids=injective-protocol&vs_currencies=usd&include_market_cap=true");
    const mc = d?.["injective-protocol"]?.usd_market_cap;
    if (mc != null){
      setTickerText("mcap", fmtUSDShort(mc).padStart(11, "\u00A0"));
    }
  } catch {}
}
...
async function tickerFetchBlock(){
  try{
    ...
    if (h != null){
      setTickerText("block", ("#" + String(h)).padStart(11, "\u00A0"));
    }
  } catch {}
}
...
function setupTickerMarquee(){
  ...
  const apply = () => {
    ...
    if (w > 0){
      track.style.setProperty("--tkW", `${w}px`);
