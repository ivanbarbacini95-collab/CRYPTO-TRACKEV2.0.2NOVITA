...
async function fetchFearGreed(){
  try{
    const r = await fetch("https://api.alternative.me/fng/?limit=60&format=json", { cache:"no-store" });
    const j = await r.json();
    ...
    setText("fgUpdated", "Updated: " + nowLabel());

    // Build chart (oldest -> newest)
    const pts = arr.slice().reverse().map(d => ({
    ...
    if(fgChartInstance){
      fgChartInstance.data.labels = labels;
      fgChartInstance.data.datasets[0].data = data;
      fgChartInstance.update("none");
    }else{
...
function renderTools(){
  ...
  if(!toolsFgTimer){
    fetchFearGreed();
    toolsFgTimer = setInterval(fetchFearGreed, 60_000);
  }
}
