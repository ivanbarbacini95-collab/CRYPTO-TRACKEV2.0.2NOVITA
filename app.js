/* Injective Portfolio • build 2026-02-22 • full bundle (smooth + y-scale + backup + no banner) */
/* =========================================================
...
/* ===== Remove legacy 'Risorse non caricate…' banner (clean UI) ===== */
(function(){
  function kill(){
    try{
      const selectors = [
      ...
      const needle = "Risorse non caricate";
      // remove simple leaf nodes that contain the warning text
      document.querySelectorAll("body *").forEach(el=>{
        if(!el || el.tagName==="SCRIPT" || el.tagName==="STYLE") return;
        if(el.childElementCount===0){
          const t = (el.textContent||"").trim();
          if(t.startsWith(needle)) el.remove();
        }
      });
    }catch(_){ /* noop */ }
  }
  ...
  function armObserver(){
    try{
      ...
      // keep it running; throttled + lightweight
    }catch(_){ /* noop */ }
  }
