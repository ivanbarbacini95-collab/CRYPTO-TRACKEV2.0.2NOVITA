...
/* ===== Cloud Sync UI wiring (robust) ===== */
function $(id){ return document.getElementById(id); }

function getCurrentWalletAddress(){
  // Prefer explicit global if present
  if (typeof currentWalletAddress === "string" && currentWalletAddress.trim()) return currentWalletAddress.trim();
  // Try common globals
  if (typeof ACTIVE_ADDRESS === "string" && ACTIVE_ADDRESS.trim()) return ACTIVE_ADDRESS.trim();
  if (typeof activeAddress === "string" && activeAddress.trim()) return activeAddress.trim();
  if (typeof currentAddress === "string" && currentAddress.trim()) return currentAddress.trim();
  // Try per-tab storage
  try{
    const tabId = sessionStorage.getItem("inj_tab_id");
    if(tabId){
      const k = `inj_active_address__${tabId}`;
...
function cloudCfgKey(addr){ return `inj_cloud_cfg__${String(addr||"").trim().toLowerCase()}`; }

function cloudLoadCfgFor(addr){
  try{
    const raw = localStorage.getItem(cloudCfgKey(addr));
    if(!raw) return { enabled:true, leader:false };
    const cfg = JSON.parse(raw);
    return { enabled: typeof cfg.enabled==="boolean"?cfg.enabled:true, leader: typeof cfg.leader==="boolean"?cfg.leader:false };
  }catch(_){
    return { enabled:true, leader:false };
  }
}

function cloudSaveCfgFor(addr, cfg){
  try{
    localStorage.setItem(cloudCfgKey(addr), JSON.stringify({ ...cfg, ts: Date.now() }));
  }catch(_){}
}

function setCloudDotState(state, text){
  const dot = $("cloudSettingsDot");
  const label = $("cloudSettingsStatus");
  if(dot){
...
async function cloudApiLoad(addr){
  if(!addr) return null;
  try{
    setCloudDotState("loading","Loading");
    const r = await fetch(`/api/load?address=${encodeURIComponent(addr)}`);
...
async function cloudApiSave(addr, payload, leader){
  if(!addr) return false;
  try{
    setCloudDotState("loading","Loading");
    const r = await fetch(`/api/save?address=${encodeURIComponent(addr)}`,{
      method:"POST",
      headers:{ "Content-Type":"application/json" },
      body: JSON.stringify({ payload, leader: !!leader, updatedAt: Date.now() })
    });
...
function applyPayload(addr, payload){
  if(!payload) return;
...
async function forceSyncNow(){
  ...
  if(!addr) return;
  const cfg = cloudLoadCfgFor(addr);
  if(!cfg.enabled){
    setCloudDotState("offline","Disabled");
    return;
  }
  if(cfg.leader){
    // push local then pull
    await cloudApiSave(addr, exportPayload(addr), true);
    const remote = await cloudApiLoad(addr);
    if(remote && remote.payload) applyPayload(addr, remote.payload);
  }else{
    // pull then push merged local
    const remote = await cloudApiLoad(addr);
    if(remote && remote.payload) applyPayload(addr, remote.payload);
    await cloudApiSave(addr, exportPayload(addr), false);
  }
}

function wireCloudSyncButtons(){
  const addr = getCurrentWalletAddress();
  const cfg = cloudLoadCfgFor(addr);
  ...
  if(t){
    ...
    t.onclick = () => {
      const addr2 = getCurrentWalletAddress();
      const cfg2 = cloudLoadCfgFor(addr2);
      cfg2.enabled = !cfg2.enabled;
      cloudSaveCfgFor(addr2, cfg2);
      wireCloudSyncButtons();
      setCloudDotState(cfg2.enabled ? "online" : "offline", cfg2.enabled ? "Online" : "Disabled");
    };
  }
  if(l){
    ...
    l.onclick = () => {
      const addr2 = getCurrentWalletAddress();
      const cfg2 = cloudLoadCfgFor(addr2);
      cfg2.leader = !cfg2.leader;
      cloudSaveCfgFor(addr2, cfg2);
      wireCloudSyncButtons();
    };
  }
  if(f){
    f.onclick = forceSyncNow;
  }

  // initial status
  if(!addr) setCloudDotState("offline","No address");
  else setCloudDotState(cfg.enabled ? "online" : "offline", cfg.enabled ? "Online" : "Disabled");
}
...
// Re-wire whenever the Settings page becomes visible (covers menu navigation)
document.addEventListener("click", (e) => {
  const t = e.target;
  if(!t) return;
  const goSettings = t.closest && t.closest('[data-page="settings"], .menu-item-settings, #openSettings');
  if(goSettings){
    setTimeout(wireCloudSyncButtons, 0);
  }
});
