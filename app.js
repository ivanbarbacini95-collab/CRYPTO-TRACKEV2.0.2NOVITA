...
/* === Net Worth: Market-synced view (mirrors INJ chart shape) ===
   The displayed Net Worth curve is built from INJUSDT klines (same plan as Price Chart),
   but values are wallet USD countervalue = (Total INJ owned) * (INJ price series).
   This guarantees the curve shape matches the INJ chart for every timeframe.
*/
const NW_MKT_CACHE_TTL_MS = 35_000; // refresh market series periodically
const nwMktCache = Object.create(null);        // tf -> { ts, labels, prices }
const nwMktLoading = Object.create(null);      // tf -> boolean

function nwTotalInjNow(){
  // Prefer live account totals; fallback to last stored net worth INJ amount.
  const live = safe(availableInj) + safe(stakeInj) + safe(rewardsInj);
  if (Number.isFinite(live) && live > 0) return live;
  const lastStored = nwInjAll.length ? safe(nwInjAll[nwInjAll.length - 1]) : 0;
  return (Number.isFinite(lastStored) && lastStored > 0) ? lastStored : 0;
}

function nwMarketPlan(tf, now = Date.now()){
  let interval = "1m";
  ...
  if (tf === "live"){
    interval = "1m";
    const r = (typeof priceLiveRange === "function") ? priceLiveRange(now) : { start: now - NW_LIVE_WINDOW_MS, end: now };
    start = safe(r.start) || (now - NW_LIVE_WINDOW_MS);
    // 15m window + small cushion
...
function ensureNWMarket(tf, force=false){
  if (!hasInternet()) return false;

  const key = String(tf || "live");
  const now = Date.now();
  const c = nwMktCache[key];

  if (!force && c && (now - safe(c.ts)) < NW_MKT_CACHE_TTL_MS && Array.isArray(c.labels) && c.labels.length > 1){
    return true;
  }
  if (nwMktLoading[key]) return false;

  nwMktLoading[key] = true;
  safeAsync(async () => {
    try{
      const plan = nwMarketPlan(key, Date.now());
      const kl = await fetchKlinesRange("INJUSDT", plan.interval, plan.start, plan.end, Math.min(2400, plan.maxTotal));
      if (!Array.isArray(kl) || !kl.length) return;

      const labels = kl.map(k => tsLabel(safe(k?.[0])));
      const prices = kl.map(k => safe(k?.[4]));

      nwMktCache[key] = { ts: Date.now(), labels, prices, interval: plan.interval, start: plan.start, end: plan.end };
    } catch (e){
      console.warn("[NW market load]", e);
    } finally {
      nwMktLoading[key] = false;
      // redraw with the new market series
      drawNW(true);
    }
  }, "ensureNWMarket");
...
function nwBuildViewLocal(tf){
  // Legacy local-series view (offline fallback)
  const now = Date.now();
  const w = nwWindowMs(tf);
  let minT = (tf === "all") ? 0 : (now - w);
  if (tf === "1d") minT = startOfDayRome(now);
  ...
  return { labels, data, source: "local" };
}
...
function nwBuildView(tf){
  const key = String(tf || "live").toLowerCase();

  // Try market-synced series first (preferred)
  const c = nwMktCache[key];
  if (c && Array.isArray(c.labels) && Array.isArray(c.prices) && c.labels.length > 1 && c.prices.length === c.labels.length){
    const totalInj = nwTotalInjNow();
    const labels = c.labels.slice();
    const data = (totalInj > 0)
      ? c.prices.map(px => {
          const p = safe(px);
          return (Number.isFinite(p) && p > 0) ? (totalInj * p) : null;
        })
      : c.prices.map(() => null);

    // In LIVE, keep last point aligned to current real-time price (while preserving the market shape)
    if (key === "live" && totalInj > 0 && Number.isFinite(+targetPrice) && +targetPrice > 0 && data.length){
      data[data.length - 1] = totalInj * safe(targetPrice);
    }

    return { labels, data, source: "market" };
  }

  // If online, trigger async market load; drawNW() will re-render on completion
  if (hasInternet()){
    ensureNWMarket(key, false);
  }

  // Offline/initial fallback: show whatever we have locally
  return nwBuildViewLocal(key);
}
...
function attachNWHold(ch, overlayEl, formatter, holdMs = 6500){
  ...
  const clearTimer = () => {
    if (ch.$nwHoldTimer) clearTimeout(ch.$nwHoldTimer);
    ch.$nwHoldTimer = null;
  };
  ...
  const setHold = (idx) => {
    ...
    clearTimer();
    ch.$nwHoldTimer = setTimeout(() => {
      // after inactivity, release and go back to live readout (hidden unless interacted again)
      clearHold();
    }, holdMs);
  };
  ...
  // Leaving canvas doesn't immediately reset: we keep the pinned value until holdMs expires.
...
function initNWChart(){
  ...
  attachNWHold(netWorthChart, $("nwReadout"), (i, lbs, ds) => {
  const t = labelToTs(lbs?.[i]);
  const raw = ds?.[i];
  const v = (raw == null) ? NaN : Number(raw);
  const when = t ? new Date(t).toLocaleString() : "—";
  return `${when} • $${Number.isFinite(v) ? v.toFixed(2) : "—"}`;
}, 6500);
}
...
function updateNWTFButtons(){
  ...
  wrap.querySelectorAll(".tf-btn").forEach((b) => {
    // ✅ Always unlocked (market series is available immediately)
    b.disabled = false;
...
function drawNW(force=false){
  ...
  // Make sure market series is loading/refreshing in background
  if (hasInternet()) ensureNWMarket(nwTf, false);
  ...
  // Log scale only when we have positive values
  const canLog = (nwScale === "log") && Array.isArray(view.data) && view.data.some(v => Number.isFinite(+v) && (+v) > 0);
  netWorthChart.options.scales.y.type = canLog ? "logarithmic" : "linear";

  try { netWorthChart.update("none"); } catch(e){ console.warn("[netWorthChart.update]", e); }

  // Mini row: total INJ owned
  try { updateNetWorthMiniRows(); } catch {}

  // PnL (ignore nulls)
  const pnlEl = $("netWorthPnl");
  if (pnlEl && Array.isArray(view.data) && view.data.length >= 2){
    ...
    for (let i=view.data.length-1;i>=0;i--){
      const v = view.data[i];
      const n = (v == null) ? NaN : Number(v);
      if (Number.isFinite(n)){
        last = n;
  ...
  // Readout: keep pinned value during interaction; otherwise keep last value ready (hidden unless interacted)
  const ro = $("nwReadout");
  if (ro){
    const ds = netWorthChart.data.datasets?.[0]?.data || [];
    const lbs = netWorthChart.data.labels || [];
    const pin = netWorthChart.$nwPinned;

    if (pin != null && Number.isInteger(pin)){
      if (pin >= ds.length){
        netWorthChart.$nwPinned = null;
        ro.classList.remove("show");
      } else {
        const t = labelToTs(lbs?.[pin]);
        const raw = ds?.[pin];
        const v = (raw == null) ? NaN : Number(raw);
        ro.textContent = `${t ? new Date(t).toLocaleString() : "—"} • $${Number.isFinite(v) ? v.toFixed(2) : "—"}`;
...
$("nwTfSwitch")?.addEventListener("click", (e) => {
  const btn = e.target?.closest(".tf-btn");
  if (!btn) return;
  const tf = String(btn.dataset.tf || "live").toLowerCase();
  if (!["live","1d","1w","1m","1y","all"].includes(tf)) return;

  nwTf = tf;

  // Reset zoom/pan so ALL (and other TFs) always fill the whole card
  try { if (netWorthChart?.resetZoom) netWorthChart.resetZoom(); } catch {}

  // Clear any pinned readout on timeframe change
  try{
    if (netWorthChart){
      netWorthChart.$nwPinned = null;
    ...
    $("nwReadout")?.classList.remove("show");
  } catch {}

  saveNWLocal();

  // Force-refresh market series for the selected TF
  if (hasInternet()) ensureNWMarket(nwTf, true);
