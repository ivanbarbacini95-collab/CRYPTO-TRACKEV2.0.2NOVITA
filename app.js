...
function maybeAddTamPoint(currInj){
  const v = safe(currInj);
  if (!Number.isFinite(v) || v < 0) return false;

  // High-frequency polling (2s) can be jittery; we aggregate and commit
  // a single, averaged point every 5 minutes (smooth line like APR).
  tamAccSum += v;
  tamAccCount++;
  ...
  const pointV = Number(avg.toFixed(6));
...
/**
 * True if a localStorage key belongs to the given address AND is safe to include in a per-wallet backup.
 * This is future-proof: any new per-address keys that include the address will be captured automatically.
 */
function backupIsKeyForAddr(key, addr){
  const k = String(key || "");
  const a = String(addr || "").trim();
  if (!k || !a) return false;
  ...
  // Only our app namespace
  if (!kl.startsWith("inj_")) return false;

  // Block global / non-address keys (avoid overwriting user UI prefs across devices)
  const BLOCK_PREFIXES = [
    "inj_theme",
    "inj_mode",
    "inj_view",
    "inj_last_ok_ts",
    "inj_ticker_speed_mult",
    "inj_last_address",
    "inj_card_order_",
    "inj_privacy_on",
    "inj_price_tf_",
  ...
  for (const p of BLOCK_PREFIXES){
    if (kl.startsWith(p)) return false;
  }

  // Snapshot key: inj_snap_<addr>
  if (kl.startsWith("inj_snap_")){
    return kl.endsWith(al);
  }

  // Most of our per-wallet keys contain the full address; treat that as the main rule.
  if (kl.includes(al)) return true;

  // Extra-safe fallbacks for possible future patterns
  if (kl.includes("_" + al) || kl.includes("__" + al) || kl.endsWith(al)) return true;
...
function backupKeysForAddr(addr){
  ...
  // Core per-wallet blocks (even if not present yet)
  const core = [
    nwStoreKey(a),
    stakeStoreKey(a),
    wdStoreKey(a),
  ...
  for (const k of core){
    if (backupIsKeyForAddr(k, a)) keys.add(k);
  }

  // Discover any other per-wallet keys automatically (future-proof)
  try{
...
// Clean current per-wallet blocks first to avoid mixing histories (safe even on fresh devices)
let applied = 0, skipped = 0, purged = 0;
try{
  const existing = backupKeysForAddr(target);
  for (const k of existing){
    if (!backupIsKeyForAddr(k, target)) continue;
    try{ localStorage.removeItem(k); purged++; }catch(_){}
  }
}catch(_){}

for (const [k, v] of Object.entries(data)){
  if (!backupIsKeyForAddr(k, target)){ skipped++; continue; }
  ...
  pushEvent({ kind:"info", title:"Restore completato", detail:`Ripuliti ${purged} blocchi • Importati ${applied} (saltati ${skipped}).`, status:"ok" });
