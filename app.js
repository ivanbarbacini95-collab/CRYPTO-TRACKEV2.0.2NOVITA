...
/* ================= CLOUD SYNC SETTINGS ================= */
let cloudEnabled = true;   // per-address
let cloudLeader  = false;  // per-address

function cloudCfgKey(addr){ return `inj_cloud_cfg__${String(addr||"").trim().toLowerCase()}`; }
function cloudLoadCfg(addr){
  try{
    const raw = localStorage.getItem(cloudCfgKey(addr));
    if(!raw) return;
    const cfg = JSON.parse(raw);
    if(typeof cfg.enabled === "boolean") cloudEnabled = cfg.enabled;
    if(typeof cfg.leader === "boolean") cloudLeader = cfg.leader;
  }catch(_){}
}
function cloudSaveCfg(addr){
  try{
    localStorage.setItem(cloudCfgKey(addr), JSON.stringify({ enabled: cloudEnabled, leader: cloudLeader, ts: Date.now() }));
  }catch(_){}
}

function cloudSettingsSetPill(state, text){
  const pill = $("cloudSettingsStatus");
  const last = $("cloudSettingsLast");
  if(pill){
  ...
  if(last){
    ...
    else last.textContent = "Last: " + nowLabel();
  }
}

// Map existing cloudSetState -> settings pill + menu status (keeps existing UI)
const _cloudSetStateOrig = cloudSetState;
cloudSetState = function(state){
  try{ _cloudSetStateOrig(state); } catch {}
  // normalize
  if(state==="saving" || state==="syncing") cloudSettingsSetPill("loading","Loading");
  else if(state==="error") cloudSettingsSetPill("offline","Offline");
  else cloudSettingsSetPill("online","Online");
};

function wireCloudSettings(){
  const addr = address || "";
  cloudLoadCfg(addr);
  ...
  if(t){
    t.textContent = cloudEnabled ? "ON" : "OFF";
    t.classList.toggle("primary", cloudEnabled);
    t.onclick = () => {
      cloudEnabled = !cloudEnabled;
      cloudSaveCfg(address || "");
      wireCloudSettings();
      cloudSettingsSetPill(cloudEnabled ? "online" : "offline", cloudEnabled ? "Online" : "Disabled");
    };
  }
  if(l){
    l.textContent = cloudLeader ? "ON" : "OFF";
    l.classList.toggle("primary", cloudLeader);
    l.onclick = () => {
      cloudLeader = !cloudLeader;
      cloudSaveCfg(address || "");
      wireCloudSettings();
    };
  }
...
// Force sync uses /api/load + /api/save with leader behavior
async function forceCloudSyncNow(){
  if(!address) return;
  if(!cloudEnabled){
    cloudSettingsSetPill("offline","Disabled");
    return;
  }
  // If leader: push then pull; else pull then push
  if(cloudLeader){
    try{ cloudSetState("saving"); } catch {}
...
// Pull once (no timers) from /api/load
async function cloudPullNow(){
  if(!address) return;
  if(!hasInternet()) { cloudSetState("synced"); return; }
  try{ cloudSetState("syncing"); } catch {}
  const url = `${CLOUD_LOAD_API}?address=${encodeURIComponent(address)}`;
  const res = await fetchJSON(url);
  if (!res?.ok) { cloudLastFailAt = Date.now(); cloudSetState("error"); return; }
  const row = res.data || res; // fetchJSON wrapper returns {ok,data} or raw
  const payload = row?.payload || row?.data?.payload || null;
  if(!payload){ cloudSetState("synced"); return; }

  try{
    mergeStakeByLabel(payload.stake || payload?.stakeSeries || payload?.stake);
    mergeWd(payload.wd || payload?.wd);
    mergeNW(payload.nw || payload?.nw);
    mergeEvents(payload.events || payload?.events);
  }catch{}
  ...
  try{ rebuildWdView(); } catch {}
  try{ rebuildEventsView(); } catch {}

  cloudSetState("synced");
}

// Save once to /api/save (uses buildCloudPayload())
async function cloudSaveNow(leaderFlag){
  if(!address) return;
  ...
  const url = `${CLOUD_SAVE_API}?address=${encodeURIComponent(address)}`;
  const res = await fetchJSON(url, {
    method:"POST",
    headers:{ "Content-Type":"application/json" },
    body: JSON.stringify({ payload, leader: !!leaderFlag, updatedAt: payload.updatedAt })
  });
...
async function cloudPull(){
  ...
  if (!cloudEnabled) { cloudSetState("synced"); return; }
if (!address) return;
  if (!hasInternet()) { cloudSetState("synced"); return; }
  ...
  try{
    const data = res.data?.payload ? res.data.payload : res.data;
...
try{
  ...
  if (pulled?.ok && pulled.data){
    const data = pulled.data?.payload ? pulled.data.payload : pulled.data;
...
const res = await fetchJSON(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ payload, leader: !!cloudLeader, updatedAt: Date.now() })
  });
