...
const addrTopRow = $("addrTopRow");
const addrTopValue = $("addrTopValue");
...
function setAddressDisplay(addr) {
  ...
  if (addressDisplay){
    if (!addr) addressDisplay.innerHTML = "";
    else addressDisplay.innerHTML = `<span class="tag"><strong>Wallet:</strong> ${shortAddr(addr)}</span>`;
  }

  // My Dashboard row (masked only, no copy icon)
  if (addrTopValue){
    addrTopValue.textContent = addr ? shortAddr(addr) : "—";
  }
  if (addrTopRow){
    const on = !!addr;
    addrTopRow.setAttribute("aria-hidden", on ? "false" : "true");
    addrTopRow.style.display = on ? "flex" : "none";
  }
}
...
const TICKER_ITEMS = [
  { key:"crypto_mcap", label:"Crypto MCap" },
  { key:"crypto_vol",  label:"Vol 24h" },
  { key:"btc_dom",     label:"BTC Dom" },
  { key:"btc_price",   label:"BTC" },
  { key:"eth_price",   label:"ETH" },
  { key:"inj_price",   label:"INJ" },
  { key:"inj_low",     label:"INJ Low" },
  { key:"inj_high",    label:"INJ High" },
  { key:"inj_vol",     label:"INJ Vol" },
  { key:"inj_mcap",    label:"INJ MCap" },
  { key:"inj_chg",     label:"INJ 24h" }
];
...
function fmtUsdCompact(n){
  ...
  const dec = (abs >= 1e12) ? 2 : (abs >= 1e9) ? 2 : (abs >= 1e6) ? 2 : (abs >= 1e3) ? 1 : 0;
  return `$${v.toFixed(dec)}${suf}`;
}
function escHtml(s){
  return String(s)
    .replace(/&/g,"&amp;")
...
function baseTickerColor(){
  return (document.body.dataset.theme === "light") ? "rgba(15,23,42,.86)" : "rgba(249,250,251,.92)";
}
function colorStringDiff(el, next, prev, dir){
  if (!el) return;
  const base = baseTickerColor();
  const up = "rgb(34,197,94)";
  const dn = "rgb(239,68,68)";
  const hi = dir === "up" ? up : dir === "down" ? dn : base;

  const n = (next == null) ? "—" : String(next);
  const p = (prev == null) ? "" : String(prev);

  if (n === "—" || !n.length) { el.textContent = "—"; return; }
  if (!p || p === "—") { el.textContent = n; return; }

  let out = "";
  for (let i=0; i<n.length; i++){
    const c = n[i];
    const oc = p[i] || "";
    const col = (c !== oc) ? hi : base;
    out += `<span style="color:${col}">${escHtml(c)}</span>`;
  }
  el.innerHTML = out;
}
...
function tickerBuild(){
  ...
  const base = TICKER_ITEMS.map((it, i) => {
    const sep = (i < TICKER_ITEMS.length - 1) ? `<span class="ticker-sep">•</span>` : "";
...
function tickerRecalibrate(){
  if (!tickerRunner) return;
  clearTimeout(tickerCalibT);
  tickerCalibT = setTimeout(() => {
    const halfW = Math.max(1, tickerRunner.scrollWidth / 2);
    const speed = 70; // px/sec
    const dur = clamp(halfW / speed, 18, 60);
    tickerRunner.style.setProperty("--tickerDur", dur.toFixed(2) + "s");
  }, 60);
}
window.addEventListener("resize", tickerRecalibrate, { passive:true });

function tickerSetMoney(key, num, dec, compact=false){
  const nodes = tickerNodes?.[key];
  if (!nodes || !nodes.length) return;

  const prev = tickerPrev[key];
  tickerPrev[key] = num;

  const dir = (Number.isFinite(prev) && Number.isFinite(num))
    ? (num > prev ? "up" : num < prev ? "down" : "flat")
    : "flat";

  nodes.forEach(el => {
    if (compact){
      const nS = fmtUsdCompact(num);
      const pS = Number.isFinite(prev) ? fmtUsdCompact(prev) : "";
      colorStringDiff(el, nS, pS, dir);
    } else {
      colorMoney(el, num, prev, dec);
    }
  });
}

function tickerSetPct(key, num, dec, signBased=false){
  const nodes = tickerNodes?.[key];
  if (!nodes || !nodes.length) return;
  ...
  const dir = signBased
    ...
    : (Number.isFinite(prev) && Number.isFinite(num)) ? (num > prev ? "up" : num < prev ? "down" : "flat") : "flat";

  const nS = Number.isFinite(num) ? `${num.toFixed(dec)}%` : "—";
  const pS = Number.isFinite(prev) ? `${prev.toFixed(dec)}%` : "";

  nodes.forEach(el => colorStringDiff(el, nS, pS, dir));
}

async function tickerFetch(){
  if (!tickerRunner || !hasInternet()) return;

  try{
    const [g, p] = await Promise.all([
      fetchJSON("https://api.coingecko.com/api/v3/global"),
      fetchJSON("https://api.coingecko.com/api/v3/simple/price?ids=injective-protocol,bitcoin,ethereum&vs_currencies=usd&include_market_cap=true&include_24hr_vol=true&include_24hr_change=true&include_24hr_low=true&include_24hr_high=true")
    ]);

    const gd = g?.data || {};
    const mcap = safe(gd?.total_market_cap?.usd);
    const vol  = safe(gd?.total_volume?.usd);
    const dom  = safe(gd?.market_cap_percentage?.btc);

    const inj = p?.["injective-protocol"] || {};
    const btc = p?.bitcoin || {};
    const eth = p?.ethereum || {};

    tickerSetMoney("crypto_mcap", mcap, 0, true);
    tickerSetMoney("crypto_vol",  vol,  0, true);
    tickerSetPct ("btc_dom", dom, 2, false);

    tickerSetMoney("btc_price", safe(btc?.usd), 0, false);
    tickerSetMoney("eth_price", safe(eth?.usd), 0, false);

    tickerSetMoney("inj_price", safe(inj?.usd), 4, false);
    tickerSetMoney("inj_low", safe(inj?.usd_24h_low), 4, false);
    tickerSetMoney("inj_high", safe(inj?.usd_24h_high), 4, false);
    tickerSetMoney("inj_vol", safe(inj?.usd_24h_vol), 0, true);
    tickerSetMoney("inj_mcap", safe(inj?.usd_market_cap), 0, true);
    tickerSetPct ("inj_chg", safe(inj?.usd_24h_change), 2, true);
  } catch(err){
    console.warn("[tickerFetch]", err);
  }
}
...
function tickerStart(){
  ...
  tickerTimer = setInterval(tickerFetch, TICKER_POLL_MS);
}
tickerStart();

window.addEventListener("online", () => { try{ tickerFetch(); }catch{} }, { passive:true });
function openSearch() {
