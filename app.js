...
function setBarBeam(el, dir){
  ...
  // Avoid resetting CSS animation every frame
...
/* ================= ATH / ATL HIGHLIGHTS ================= */
const extreme = {
  d:{ hi:null, lo:null, ath:false, atl:false },
  w:{ hi:null, lo:null, ath:false, atl:false },
  m:{ hi:null, lo:null, ath:false, atl:false },
  y:{ hi:null, lo:null, ath:false, atl:false }
};

function setExtremeClass(ids, on){
  for (const id of ids){
    const el = $(id);
    if (el) el.classList.toggle("extreme", !!on);
  }
}

function updateExtremeFor(tf, val, hi, lo, ids){
  const st = extreme[tf];
  if (!st) return { ath:false, atl:false };

  const EPS = 1e-9;

  // init
  if (st.hi == null && Number.isFinite(hi)) st.hi = hi;
  if (st.lo == null && Number.isFinite(lo)) st.lo = lo;

  // New ATH / ATL
  if (Number.isFinite(hi) && (st.hi == null || hi > st.hi + EPS)) {
    st.hi = hi;
    st.ath = true; // stays until price retreats
    st.atl = false;
  }
  if (Number.isFinite(lo) && (st.lo == null || lo < st.lo - EPS)) {
    st.lo = lo;
    st.atl = true; // stays until price retreats
    st.ath = false;
  }

  // Retreat logic
  if (st.ath && Number.isFinite(val) && Number.isFinite(st.hi) && val < (st.hi - (st.hi*0.001))) st.ath = false;
  if (st.atl && Number.isFinite(val) && Number.isFinite(st.lo) && val > (st.lo + (Math.abs(st.lo)*0.001 + 1e-9))) st.atl = false;

  const active = st.ath || st.atl;
  setExtremeClass(ids, active);
  return { ath:st.ath, atl:st.atl };
}

function updateExtremeHighlights(){
  const v = safe(targetPrice);

  let any = false;
  if (tfReady.d) any = any || updateExtremeFor("d", v, candle.d.high, candle.d.low, ["priceMax","priceMin"]).ath || extreme.d.atl;
  else setExtremeClass(["priceMax","priceMin"], false);

  if (tfReady.w) any = any || updateExtremeFor("w", v, candle.w.high, candle.w.low, ["weekMax","weekMin"]).ath || extreme.w.atl;
  else setExtremeClass(["weekMax","weekMin"], false);

  if (tfReady.m) any = any || updateExtremeFor("m", v, candle.m.high, candle.m.low, ["monthMax","monthMin"]).ath || extreme.m.atl;
  else setExtremeClass(["monthMax","monthMin"], false);

  if (tfReady.y) any = any || updateExtremeFor("y", v, candle.y.high, candle.y.low, ["yearMax","yearMin"]).ath || extreme.y.atl;
  else setExtremeClass(["yearMax","yearMin"], false);

  // Header price glows if ANY bar is in ATH/ATL highlight
  const priceEl = $("price");
  if (priceEl) priceEl.classList.toggle("extreme", !!any);
}
...
const REFRESH_LOOP_MS = 30_000; // refresh mode periodic sync (used only if auto-refresh enabled)
const REFRESH_AUTO = false; // ✅ requested: in Refresh mode update ONLY on manual refresh
let refreshLoopTimer = null;

function startRefreshLoop(){
  // In Refresh mode we do NOT auto-update; updates happen only on manual refresh (pull-to-refresh / button)
  if (!REFRESH_AUTO) return;
  if (refreshLoopTimer) return;
  refreshLoopTimer = setInterval(() => safeAsync(() => refreshLoadAllOnce(true), "refreshLoadAllOnce"), REFRESH_LOOP_MS);
}
...
async function refreshLoadAllOnce(isManual=false){
  ...
  try{
    ...
    // In REFRESH mode we "rebuild" the time gap: fill a flat bridge between last point and now,
    // then record a fresh Net Worth point so charts reflect the pause correctly.
    if (isManual && address) {
      try { recordNetWorthPointManual(); } catch (e) { console.warn("[recordNetWorthPointManual]", e); }
    }
    refreshLoaded = true;
    modeLoading = false;
    refreshConnUI();
...
function ptrSet(state, pullPx=0){
  const wrap = $("pullRefresh");
  const label = $("pullRefreshLabel");
  const spinner = $("pullRefreshSpinner");
  const bar = $("pullRefreshBar");
  if (!wrap || !label || !spinner || !bar) return;

  wrap.classList.toggle("show", state !== "hide");
  wrap.classList.toggle("ready", state === "ready");
  wrap.classList.toggle("loading", state === "loading");

  const pct = clamp(pullPx / PTR_THRESHOLD, 0, 1);
  bar.style.transform = `scaleX(${pct})`;

  if (state === "pull") label.textContent = "Pull to refresh";
  if (state === "ready") label.textContent = "Release to refresh";
  if (state === "loading") label.textContent = "Refreshing…";
}

async function manualRefreshNow(){
  // Only meaningful when an address exists; still allow price refresh
  ptrSet("loading", PTR_THRESHOLD);
  ...
  try{
    if (liveMode){
      ...
      await loadChartToday(true);
      if (address) await loadAccount(true);
      // record a point so the chart stays continuous
      if (address) { try{ recordNetWorthPoint(); }catch{} }
    } else {
      await refreshLoadAllOnce(true);
    }
  } finally {
    // settle animation
    setTimeout(() => { ptrSet("hide", 0); }, 450);
  }
}
...
function initPullToRefresh(){
  ...
  const onMove = (y) => {
    ...
    if (dy <= 0) { ptrPull = 0; ptrSet("pull", 0); return; }
    // Resist so it feels natural
    ptrPull = Math.min(140, dy * 0.55);
    if (ptrPull >= PTR_THRESHOLD) ptrSet("ready", ptrPull);
    else ptrSet("pull", ptrPull);
  };
  const onEnd = async () => {
    if (!ptrActive) return;
    const should = ptrPull >= PTR_THRESHOLD;
    ptrActive = false;
    if (should) await safeAsync(() => manualRefreshNow(), "manualRefreshNow");
    else ptrSet("hide", 0);
  };

  // Touch
  window.addEventListener("touchstart", (e) => onStart(e.touches?.[0]?.clientY ?? 0), { passive:true });
  window.addEventListener("touchmove", (e) => onMove(e.touches?.[0]?.clientY ?? 0), { passive:true });
  window.addEventListener("touchend", () => { onEnd(); }, { passive:true });

  // Pointer (desktop)
  window.addEventListener("pointerdown", (e) => onStart(e.clientY), { passive:true });
  window.addEventListener("pointermove", (e) => onMove(e.clientY), { passive:true });
...
function recordNetWorthPointManual(){
  if (!address) return;
  ...
  // Fill gaps with "flat" points so the chart doesn't jump when Refresh mode pauses live updates.
  const lastT = nwTAll.length ? safe(nwTAll[nwTAll.length - 1]) : 0;
  const lastUsd = nwUsdAll.length ? safe(nwUsdAll[nwUsdAll.length - 1]) : 0;
  const lastInj = nwInjAll.length ? safe(nwInjAll[nwInjAll.length - 1]) : totalInj;

  const STEP = ONE_MIN_MS;
  const MAX_FILL = 240; // up to 4h of flat bridge (keeps memory bounded)

  if (lastT && now - lastT > STEP) {
    let t = lastT + STEP;
    let n = 0;
    while (t < now && n < MAX_FILL) {
      nwTAll.push(t);
      nwUsdAll.push(lastUsd);
      nwInjAll.push(lastInj);
      t += STEP;
      n++;
    }
    clampNWArrays();
  ...
  saveNWLocal();
  drawNW(false);
  // Mark cloud dirty so other devices see the new point
  try{ cloudMarkDirty({ nw:true }); }catch{}
}
...
(async function boot() {
  ...
  applyView(viewMode);
  applyPrivacy(privacyOn);
  // Pull-to-refresh works on mobile + desktop (especially useful in Refresh mode)
  try{ initPullToRefresh(); } catch {}
  updatePriceScaleBtn();
...
function animate() {
  ...
  try{ updateExtremeHighlights(); }catch{}
