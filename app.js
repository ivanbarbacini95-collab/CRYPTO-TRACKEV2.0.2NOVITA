...
function setAddressDisplay(addr) {
  ...
  // New header wallet tag + copy button (preferred)
  const tag = $("walletTag");
  const shortEl = $("walletShort");
  const btn = $("copyAddrBtn");

  if (tag && shortEl && btn){
    shortEl.textContent = has ? shortAddr(addr) : "—";
    tag.style.display = has ? "inline-flex" : "none";
  ...
  if (!has) { addressDisplay.innerHTML = ""; return; }
  addressDisplay.innerHTML = `<span class="tag"><strong>Wallet:</strong> ${shortAddr(addr)}</span>`;
}
setAddressDisplay(address);


/* ================= MARKET TICKER (My Dashboard) =================
   Goal: a scrolling, real-time strip that never "snaps" to 0 on transient API errors.
   Data sources:
   - Binance Market Data (prices + 24h stats via WebSocket; today hi/lo via REST klines)
   - CoinGecko (global market cap/volume + BTC dominance; INJ circulating supply)
*/
const marketTicker = $("marketTicker");
const tickerRunner = $("tickerRunner");

const BINANCE_REST = "https://data-api.binance.vision";
const BINANCE_WSS_URLS = [
  "wss://data-stream.binance.vision:443/stream?streams=btcusdt@ticker/ethusdt@ticker/injusdt@ticker",
  "wss://stream.binance.com:9443/stream?streams=btcusdt@ticker/ethusdt@ticker/injusdt@ticker"
];

const TICKER_CACHE_KEY = "inj_ticker_cache_v1";
const TICKER_GLOBAL_POLL_MS = 60_000;
const TICKER_INJ_HILO_POLL_MS = 120_000;
const TICKER_SUPPLY_POLL_MS = 6 * 60 * 60 * 1000;

let tickerBuilt = false;
let tickerNodes = {};
let tickerPrevNum = {};
let tickerLastGood = {};
let tickerGlobalTimer = 0;
let tickerHiLoTimer = 0;
let tickerSupplyTimer = 0;
let tickerRestTimer = 0;

let injCirculating = NaN;
let inj24hLow = NaN;
let inj24hHigh = NaN;

let binanceWS = null;
let binanceWSUrlIdx = 0;
let binanceWSRetryT = 0;
let binanceWSLastMsg = 0;

const TICKER_ITEMS = [
  { key:"crypto_mcap", label:"Crypto MCap", kind:"money", dec:0, compact:true },
  { key:"crypto_vol",  label:"Vol 24h",     kind:"money", dec:0, compact:true },
  { key:"btc_dom",     label:"BTC Dom",     kind:"pct",   dec:2, signBased:false },

  { key:"btc_price",   label:"BTC",         kind:"money", dec:0, compact:false },
  { key:"eth_price",   label:"ETH",         kind:"money", dec:0, compact:false },

  { key:"inj_price",   label:"INJ",         kind:"money", dec:4, compact:false },
  { key:"inj_low",     label:"INJ Low Today",  kind:"money", dec:4, compact:false },
  { key:"inj_high",    label:"INJ High Today", kind:"money", dec:4, compact:false },
  { key:"inj_vol",     label:"INJ Vol 24h", kind:"money", dec:0, compact:true },
  { key:"inj_mcap",    label:"INJ MCap",    kind:"money", dec:0, compact:true },
  { key:"inj_chg",     label:"INJ 24h",     kind:"pct",   dec:2, signBased:true }
];

const TICKER_META = Object.fromEntries(TICKER_ITEMS.map(x => [x.key, x]));

function tkrAsNum(x){
  const n = Number(x);
  return Number.isFinite(n) ? n : NaN;
}
function tkrLoadCache(){
  try{
    ...
    tickerLastGood = obj;
    // Render cached values immediately (no direction coloring on first paint)
    for (const k of Object.keys(obj)){
      const v = tkrAsNum(obj[k]);
      if (!Number.isFinite(v)) continue;
      tickerPrevNum[k] = v;
      tkrRender(k, v, v, true);
    }
  }catch{}
}
let tkrSaveT = 0;
function tkrSaveCacheSoon(){
  if (tkrSaveT) clearTimeout(tkrSaveT);
  tkrSaveT = setTimeout(() => {
    try{ localStorage.setItem(TICKER_CACHE_KEY, JSON.stringify(tickerLastGood)); }catch{}
  }, 450);
}

function tkrSepNeeded(i){
  return (i < TICKER_ITEMS.length - 1) ? `<span class="ticker-sep">•</span>` : "";
}
function tkrBuild(){
  if (!tickerRunner || tickerBuilt) return;
  const base = TICKER_ITEMS.map((it, i) => {
    return `<span class="ticker-item" data-key="${it.key}">
      <span class="tk-label">${it.label}</span>
      <span class="tk-value" data-role="value">—</span>
    </span>${tkrSepNeeded(i)}`;
  }).join("");
  tickerRunner.innerHTML = base + base;

  tickerNodes = {};
  TICKER_ITEMS.forEach(it => {
    tickerNodes[it.key] = tickerRunner.querySelectorAll(`.ticker-item[data-key="${it.key}"] [data-role="value"]`);
  });
  tickerBuilt = true;

  // calibrate animation distance (if CSS uses transform)
  requestAnimationFrame(() => {
    try{
      const viewport = tickerRunner.parentElement;
      if (!viewport) return;
      // ensure a width update triggers layout
      void tickerRunner.offsetWidth;
    }catch{}
  });
}
...
function tkrBaseTextColor(){
  return (document.body.dataset.theme === "light") ? "#0f172a" : "#f9fafb";
}
function tkrColorStringDiff(newStr, oldStr, dir){
  if (!oldStr) return newStr;
  if (newStr === oldStr) return newStr;

  // align lengths (pad left)
  const a = String(newStr);
  const b = String(oldStr);
  const L = Math.max(a.length, b.length);
  const ap = a.padStart(L, " ");
  const bp = b.padStart(L, " ");
  const up = dir > 0 ? "#22c55e" : (dir < 0 ? "#ef4444" : tkrBaseTextColor());

  let out = "";
  for (let i=0;i<L;i++){
    const ch = ap[i];
    const changed = ch !== bp[i];
    const col = changed ? up : tkrBaseTextColor();
    out += `<span style="color:${col}">${tkrEsc(ch)}</span>`;
  }
  return out.replace(/<span[^>]*> <\/span>/g, " ");
}

function tkrFmtUsdPlain(n, dec){
  const x = Number(n);
  if (!Number.isFinite(x)) return "—";
  return `$${x.toFixed(dec)}`;
}
function tkrFmtUsdCompact(n){
  const x = Number(n);
  if (!Number.isFinite(x)) return "—";
...
function tkrFmtPct(n, dec, signBased){
  const x = Number(n);
  if (!Number.isFinite(x)) return "—";
  const s = x.toFixed(dec).replace(/\.0+$/, "").replace(/(\.\d*?)0+$/, "$1");
  if (signBased) return `${x > 0 ? "+" : ""}${s}%`;
  return `${s}%`;
}

function tkrWrite(key, htmlStr){
  const nodes = tickerNodes[key];
  if (!nodes) return;
  nodes.forEach(n => { n.innerHTML = htmlStr; });
}

function tkrRender(key, num, prevNum, plain=false){
  const meta = TICKER_META[key];
  if (!meta) return;

  if (meta.kind === "money"){
    const newStr = meta.compact ? tkrFmtUsdCompact(num) : tkrFmtUsdPlain(num, meta.dec);
    const oldStr = meta.compact ? tkrFmtUsdCompact(prevNum) : tkrFmtUsdPlain(prevNum, meta.dec);
    if (plain || newStr === oldStr){ tkrWrite(key, tkrEsc(newStr)); return; }
    const dir = (num > prevNum) ? 1 : -1;
    tkrWrite(key, tkrColorStringDiff(newStr, oldStr, dir));
    return;
  }

  if (meta.kind === "pct"){
    const newStr = tkrFmtPct(num, meta.dec, meta.signBased);
    const oldStr = tkrFmtPct(prevNum, meta.dec, meta.signBased);
    if (plain || newStr === oldStr){ tkrWrite(key, tkrEsc(newStr)); return; }

    const dir = meta.signBased ? (num > 0 ? 1 : (num < 0 ? -1 : 0))
                               : (num > prevNum ? 1 : -1);
    tkrWrite(key, tkrColorStringDiff(newStr, oldStr, dir));
  }
}

function tkrSet(key, num){
  if (!tickerBuilt) return;
  const meta = TICKER_META[key];
  if (!meta) return;

  // never snap to 0 on transient errors
  if (meta.kind === "money"){
    if (!Number.isFinite(num) || num <= 0) return;
  } else {
    if (!Number.isFinite(num)) return;
    // avoid accidental "0%" glitch for non sign-based metrics (e.g. BTC dominance)
    if (!meta.signBased && num === 0 && Number.isFinite(tickerPrevNum[key]) && tickerPrevNum[key] !== 0) return;
  }

  const prev = Number.isFinite(tickerPrevNum[key]) ? tickerPrevNum[key] : num;
  tickerPrevNum[key] = num;
  tickerLastGood[key] = num;
  tkrSaveCacheSoon();
  tkrRender(key, num, prev, false);
}

async function tkrFetchGlobal(){
  if (!tickerBuilt || !hasInternet()) return;
  try{
    const g = await fetchJSON("https://api.coingecko.com/api/v3/global");
    const gd = g?.data || {};

    tkrSet("crypto_mcap", tkrAsNum(gd?.total_market_cap?.usd));
    tkrSet("crypto_vol",  tkrAsNum(gd?.total_volume?.usd));
    tkrSet("btc_dom",     tkrAsNum(gd?.market_cap_percentage?.btc));
  }catch(err){
    console.warn("[ticker/global]", err);
  }
}

async function tkrFetchInjSupply(){
  if (!tickerBuilt || !hasInternet()) return;
  try{
    const arr = await fetchJSON("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=injective-protocol&price_change_percentage=24h");
    const inj = Array.isArray(arr) ? (arr[0] || {}) : {};
    const supply = tkrAsNum(inj?.circulating_supply);
    if (Number.isFinite(supply) && supply > 0) injCirculating = supply;

    // fallback values (in case WS isn't available)
    const mcap = tkrAsNum(inj?.market_cap);
    if (Number.isFinite(mcap) && mcap > 0) tkrSet("inj_mcap", mcap);

    const vol = tkrAsNum(inj?.total_volume);
    if (Number.isFinite(vol) && vol > 0) tkrSet("inj_vol", vol);

    const price = tkrAsNum(inj?.current_price);
    if (Number.isFinite(price) && price > 0) tkrSet("inj_price", price);

    const chg = tkrAsNum(inj?.price_change_percentage_24h);
    if (Number.isFinite(chg)) tkrSet("inj_chg", chg);
  }catch(err){
    console.warn("[ticker/injSupply]", err);
  }
}

async function tkrFetchInjTodayHiLo(){
  if (!tickerBuilt || !hasInternet()) return;
  try{
    const start = startOfDayRome(Date.now());
    const url = `${BINANCE_REST}/api/v3/klines?symbol=INJUSDT&interval=5m&startTime=${start}&limit=288`;
    const kl = await fetchJSON(url);
    if (!Array.isArray(kl) || kl.length === 0) throw new Error("no klines");
    let lo = Infinity, hi = -Infinity;
    for (const r of kl){
      const h = tkrAsNum(r?.[2]);
      const l = tkrAsNum(r?.[3]);
      if (Number.isFinite(h) && h > hi) hi = h;
      if (Number.isFinite(l) && l < lo) lo = l;
    }
    if (Number.isFinite(lo) && lo < Infinity) tkrSet("inj_low", lo);
    if (Number.isFinite(hi) && hi > 0) tkrSet("inj_high", hi);
  }catch(err){
    // fallback to 24h window if available
    if (Number.isFinite(inj24hLow))  tkrSet("inj_low", inj24hLow);
    if (Number.isFinite(inj24hHigh)) tkrSet("inj_high", inj24hHigh);
    console.warn("[ticker/injHiLo]", err);
  }
}


async function tkrFetchBinanceRest(){
  if (!tickerBuilt || !hasInternet()) return;
  try{
    const [btc, eth, inj] = await Promise.all([
      fetchJSON(`${BINANCE_REST}/api/v3/ticker/24hr?symbol=BTCUSDT`),
      fetchJSON(`${BINANCE_REST}/api/v3/ticker/24hr?symbol=ETHUSDT`),
      fetchJSON(`${BINANCE_REST}/api/v3/ticker/24hr?symbol=INJUSDT`)
    ]);

    // BTC / ETH
    tkrSet("btc_price", tkrAsNum(btc?.lastPrice));
    tkrSet("eth_price", tkrAsNum(eth?.lastPrice));

    // INJ fallbacks (24h)
    const p = tkrAsNum(inj?.lastPrice);
    if (Number.isFinite(p) && p > 0){
      tkrSet("inj_price", p);
      if (Number.isFinite(injCirculating) && injCirculating > 0){
        tkrSet("inj_mcap", p * injCirculating);
      }
    }
    tkrSet("inj_vol", tkrAsNum(inj?.quoteVolume));
    tkrSet("inj_chg", tkrAsNum(inj?.priceChangePercent));

    const lo = tkrAsNum(inj?.lowPrice);
    const hi = tkrAsNum(inj?.highPrice);
    if (Number.isFinite(lo) && lo > 0) inj24hLow = lo;
    if (Number.isFinite(hi) && hi > 0) inj24hHigh = hi;
  }catch(err){
    console.warn("[ticker/binanceRest]", err);
  }
}

function binanceWSStop(){
  try{ binanceWS?.close?.(); }catch{}
  binanceWS = null;
  binanceWSLastMsg = 0;
  if (binanceWSRetryT) clearTimeout(binanceWSRetryT);
  binanceWSRetryT = 0;
}
...
function binanceWSConnect(){
  ...
  const url = BINANCE_WSS_URLS[binanceWSUrlIdx % BINANCE_WSS_URLS.length];
  try{
    binanceWS = new WebSocket(url);
  }catch(err){
    console.warn("[ticker/ws]", err);
    binanceWS = null;
    return;
  }

  binanceWS.addEventListener("open", () => {
    binanceWSLastMsg = Date.now();
  });

  binanceWS.addEventListener("message", (ev) => {
    binanceWSLastMsg = Date.now();
    let msg = null;
    try{ msg = JSON.parse(ev?.data || "{}"); }catch{ return; }
    const d = msg?.data || msg; // combined stream vs single
    const sym = String(d?.s || "").toUpperCase();
    if (!sym) return;

    const last = tkrAsNum(d?.c);
    const pct = tkrAsNum(d?.P);
    const qVol = tkrAsNum(d?.q);
    const low = tkrAsNum(d?.l);
    const high = tkrAsNum(d?.h);

    if (sym === "BTCUSDT"){
      if (Number.isFinite(last) && last > 0) tkrSet("btc_price", last);
      return;
    }
    if (sym === "ETHUSDT"){
      if (Number.isFinite(last) && last > 0) tkrSet("eth_price", last);
      return;
    }
    if (sym === "INJUSDT"){
      if (Number.isFinite(last) && last > 0){
        tkrSet("inj_price", last);
        if (Number.isFinite(injCirculating) && injCirculating > 0){
          tkrSet("inj_mcap", last * injCirculating);
        }
      }
      if (Number.isFinite(qVol) && qVol > 0) tkrSet("inj_vol", qVol);
      if (Number.isFinite(pct)) tkrSet("inj_chg", pct);

      // keep 24h fallback hi/lo
      if (Number.isFinite(low) && low > 0) inj24hLow = low;
      if (Number.isFinite(high) && high > 0) inj24hHigh = high;
    }
  });
  ...
  const scheduleReconnect = () => {
    ...
    const wait = Math.min(12_000, 1500 + (binanceWSUrlIdx % 6) * 1200);
    binanceWSUrlIdx++;
    binanceWSRetryT = setTimeout(() => binanceWSConnect(), wait);
  };

  binanceWS.addEventListener("error", scheduleReconnect);
  binanceWS.addEventListener("close", scheduleReconnect);

  // watchdog: if WS stalls, reconnect
  setTimeout(() => {
    if (!binanceWS) return;
    const age = Date.now() - (binanceWSLastMsg || 0);
    if (age > 45_000){
      scheduleReconnect();
    }
  }, 50_000);
...
function tickerStart(){
  if (!marketTicker || !tickerRunner) return;
  tkrBuild();
  tkrLoadCache();

  // kick immediate fetches
  tkrFetchGlobal();
  tkrFetchInjSupply();
  tkrFetchInjTodayHiLo();
  binanceWSConnect();
  tkrFetchBinanceRest();

  // timers
  if (tickerGlobalTimer) clearInterval(tickerGlobalTimer);
  tickerGlobalTimer = setInterval(tkrFetchGlobal, TICKER_GLOBAL_POLL_MS);

  if (tickerHiLoTimer) clearInterval(tickerHiLoTimer);
  tickerHiLoTimer = setInterval(tkrFetchInjTodayHiLo, TICKER_INJ_HILO_POLL_MS);

  if (tickerSupplyTimer) clearInterval(tickerSupplyTimer);
  if (tickerRestTimer) clearInterval(tickerRestTimer);
  tickerSupplyTimer = setInterval(tkrFetchInjSupply, TICKER_SUPPLY_POLL_MS);

  if (tickerRestTimer) clearInterval(tickerRestTimer);
  tickerRestTimer = setInterval(() => { if (!binanceWS) tkrFetchBinanceRest(); }, 30_000);
}
...
function tickerStop(){
  ...
  if (tickerSupplyTimer) clearInterval(tickerSupplyTimer);
  if (tickerRestTimer) clearInterval(tickerRestTimer);
  tickerGlobalTimer = tickerHiLoTimer = tickerSupplyTimer = tickerRestTimer = 0;
  binanceWSStop();
}
