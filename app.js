...
function maybeRecordRewardWithdrawal(newRewards) {
  ...
  if (diff > REWARD_WITHDRAW_THRESHOLD) {
    ...
    // Fees card: pull latest reward-withdraw tx fees
...
function feesStoreKey(addr){
  const a = (addr || "").trim();
  return a ? `inj_fees_wd_v${FEES_LOCAL_VER}_${a}` : null;
}
...
async function feesBackfill(force=false){
  ...
  let j = null;
  try{
    const ev = encodeURIComponent(`message.sender='${address}'`);
    j = await fetchLCD(`/cosmos/tx/v1beta1/txs?events=${ev}&order_by=ORDER_BY_DESC&pagination.limit=50`);
  } catch {
    return;
  }
  ...
  for (let i = 0; i < n; i++){
    const tx = txs[i];
    if (!txHasWithdrawDelegatorReward(tx)) continue;
