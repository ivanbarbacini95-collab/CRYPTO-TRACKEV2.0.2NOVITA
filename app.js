...
/* === Net Worth view: mirror INJ market chart ===
   We rebuild the displayed Net Worth curve from INJUSDT klines so the shape
   matches the Injective Price Chart. Values are shown as USD countervalue
   of the selected address, using the current (or last known) INJ amount.
*/
const NW_MKT_CACHE_TTL_LIVE_MS = 12000;
const NW_MKT_CACHE_TTL_MS      = 60000;

let nwMktViewCache = { tf: null, labels: [], data: [], builtAt: 0, injAmt: 0 };
let nwMktLastAppliedAt = 0;
let nwMktFetchInFlight = false;

function nwGetInjAmountForMarketView(){
  // Prefer current live amounts (available + staked + rewards)
  const liveAmt = safe(availableInj) + safe(stakeInj) + safe(rewardsInj);
  if (Number.isFinite(liveAmt) && liveAmt > 0) return liveAmt;

  // Fallback: derive from last known USD and price
  const p = safe(displayed?.price);
  const u = safe(displayed?.netWorthUsd);
  if (p > 0 && u > 0) return u / p;

  // Fallback: last stored point inj amount (if any)
  const lastIdx = (nwInjAll && nwInjAll.length) ? (nwInjAll.length - 1) : -1;
  const lastStored = safe(lastIdx >= 0 ? nwInjAll[lastIdx] : 0);
  if (Number.isFinite(lastStored) && lastStored > 0) return lastStored;

  return 0;
}

function nwMarketParamsForTf(tf){
  const now = Date.now();
  let interval = "1m";
  let start = 0;
  let maxTotal = 1440;
...
async function ensureNWMarketView(tf, force=false){
  if (!hasInternet()) return false;

  const ttl = (tf === "live") ? NW_MKT_CACHE_TTL_LIVE_MS : NW_MKT_CACHE_TTL_MS;
  const now = Date.now();
  if (!force && nwMktViewCache.tf === tf && nwMktViewCache.data.length && (now - safe(nwMktViewCache.builtAt)) < ttl){
    return true;
  }
  if (nwMktFetchInFlight) return false;
  nwMktFetchInFlight = true;

  try{
    const p = nwMarketParamsForTf(tf);
    const kl = await fetchKlinesRange("INJUSDT", p.interval, p.start, p.now, Math.min(2400, p.maxTotal));
    if (!kl || !kl.length) return false;

    const injAmt = nwGetInjAmountForMarketView();
    if (!Number.isFinite(injAmt) || injAmt <= 0) return false;

    const labels = kl.map(k => tsLabel(safe(k[0])));
    const data = kl.map(k => safe(k[4]) * injAmt);

    nwMktViewCache = { tf, labels, data, builtAt: Date.now(), injAmt };
    return true;
  } catch(e){
    console.warn("[ensureNWMarketView]", e);
    return false;
  } finally {
    nwMktFetchInFlight = false;
  }
}

function nwHasMarketView(tf){
  return (nwMktViewCache.tf === tf && Array.isArray(nwMktViewCache.data) && nwMktViewCache.data.length > 2);
}
...
function nwBuildView(tf){
  if (nwHasMarketView(tf)){
    return { labels: nwMktViewCache.labels.slice(), data: nwMktViewCache.data.slice() };
  }
  const now = Date.now();
...
function drawNW(force=false){
  ...
  const view = nwBuildView(nwTf);
  if (nwHasMarketView(nwTf)) nwMktLastAppliedAt = safe(nwMktViewCache.builtAt) || nwMktLastAppliedAt;
...
// Keep the Net Worth curve aligned with the INJ market chart (async refresh).
const _tfWanted = nwTf;
const _appliedAt = nwMktLastAppliedAt;
ensureNWMarketView(_tfWanted).then((ok) => {
  if (!ok) return;
  if (_tfWanted !== nwTf) return; // user changed tf while fetching
  if (nwHasMarketView(_tfWanted) && safe(nwMktViewCache.builtAt) !== safe(_appliedAt)){
    nwMktLastAppliedAt = safe(nwMktViewCache.builtAt) || Date.now();
    drawNW(true);
  }
}).catch(()=>{});
