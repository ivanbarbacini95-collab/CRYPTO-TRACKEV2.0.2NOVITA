...
function setBarBeam(el, dir){
  ...
  // avoid resetting CSS animations by re-setting the same attribute every frame
  if (el.getAttribute("data-beam") !== d) el.setAttribute("data-beam", d);
}

/* ================= BAR SMOOTHING ================= */
// Smoothly animates bar width without "reset" feeling (keeps the beam continuous)
const __barAnim = new Map();
function setBarPctSmooth(el, pct){
  if (!el) return;
  const t = clamp(safe(pct), 0, 100);
  const key = el.id || el;
  const st = __barAnim.get(key) || { cur: null, raf: 0 };
  if (st.cur == null) st.cur = t;

  // If change is tiny, snap
  if (Math.abs(t - st.cur) < 0.08){
    st.cur = t;
    el.style.width = st.cur.toFixed(3) + "%";
    __barAnim.set(key, st);
    return;
  }

  // Cancel previous RAF (do not restart CSS beam; only updates width)
...
function setLinePctSmooth(el, pct){
  if (!el) return;
  const t = clamp(safe(pct), 0, 100);
  el.style.left = t.toFixed(3) + "%";
}
...
function refreshConnUI() {
  ...
  const setState = (state, text) => {
    ...
    // Search lens dot (under the magnifier)
    try{
      if (lensDot){
        lensDot.classList.remove("online","loading");
        if (state === "online") lensDot.classList.add("online");
        else if (state === "loading") lensDot.classList.add("loading");
        // offline => default red (no class)
      }
      if (lensLabel){
        if (state === "loading"){
          lensLabel.classList.add("show");
          lensLabel.textContent = "Loading";
        } else {
...
/* ================= ATH / ATL FLASH (INJ Price bars) ================= */
function toggleClass(el, cls, on){
  if (!el) return;
  el.classList.toggle(cls, !!on);
}

// Keep flashing while price is at/over ATH or at/under ATL for each timeframe.
// Also highlights the main header price when ANY timeframe is at an extreme.
function applyAthAtlFlash(){
  const p = safe(targetPrice);
  if (!Number.isFinite(p) || p <= 0) {
    toggleClass($("price"), "price-glow-yellow", false);
    return;
  }

  let anyExtreme = false;

  const check = (tfKey, lowElId, highElId) => {
    if (!tfReady?.[tfKey]) {
      toggleClass($(lowElId), "flash-yellow", false);
      toggleClass($(highElId), "flash-yellow", false);
      return;
    }
    const lo = safe(candle?.[tfKey]?.low);
    const hi = safe(candle?.[tfKey]?.high);
    if (!Number.isFinite(lo) || !Number.isFinite(hi) || hi <= 0) return;

    // epsilon to avoid float jitter
    const eps = Math.max(1e-9, hi * 1e-8);

    const atHigh = p >= (hi - eps);
    const atLow  = p <= (lo + eps);

    toggleClass($(highElId), "flash-yellow", atHigh);
    toggleClass($(lowElId),  "flash-yellow", atLow);

    if (atHigh || atLow) anyExtreme = true;
  };

  check("d", "priceMin", "priceMax");
  check("w", "weekMin",  "weekMax");
  check("m", "monthMin", "monthMax");
  check("y", "yearMin",  "yearMax");

  toggleClass($("price"), "price-glow-yellow", anyExtreme);
}
}
...
$("eventsClearBtn")?.addEventListener("click", (e) => {
  ...
  try{
    ...
    const f = $("eventsFilter"); if (f) f.value = "all";
  }catch{}
  renderEvents();
  try{ pushToast?.("Events cleared"); }catch{}
}, { passive:false });
...
function animate() {
  ...
  // ATH/ATL highlight for all price bars
  try{ applyAthAtlFlash(); }catch{}
  ...
  setBarPctSmooth(stakeBar, stakePct);
  setLinePctSmooth(stakeLine, stakePct);
...
/* =========================================================
   v2.0.2 • Tools page + Pull-to-refresh (Refresh mode)
...
function showPull(msg="Refreshing…"){
  if (pullText) pullText.textContent = msg;
  if (pullIndicator){
...
async function doPullRefresh(){
  ...
  try{
    ...
    // Keep tools fresh too
    try{ await toolsRefreshAll(true); } catch {}
  } finally {
    setTimeout(() => hidePull(), 420);
  }
}

function enablePullToRefresh(enable){
  pullEnabled = !!enable;
  if (!pullEnabled){
    pullActive = false; pullStartY = 0; pullDist = 0;
    hidePull();
  }
}
...
window.addEventListener("touchmove", (e) => {
  if (!pullEnabled || !pullActive) return;
  if (liveMode) return;
  if (document.scrollingElement?.scrollTop > 0) { pullActive=false; hidePull(); return; }
  const y = e.touches?.[0]?.clientY || 0;
  pullDist = Math.max(0, y - pullStartY);
  if (pullDist > 10) showPull(pullDist > 70 ? "Release to refresh" : "Pull to refresh");
}, { passive:true });

window.addEventListener("touchend", async () => {
  if (!pullEnabled || !pullActive) return;
  pullActive = false;
  if (liveMode) { hidePull(); return; }
  if (pullDist > 70){
    await doPullRefresh();
  } else {
    hidePull();
...
/* Also support mouse/trackpad (desktop) */
...
window.addEventListener("pointermove", (e) => {
  ...
  const y = e.clientY || 0;
  pullDist = Math.max(0, y - pullStartY);
  if (pullDist > 18) showPull(pullDist > 90 ? "Release to refresh" : "Pull to refresh");
}, { passive:true });
...
window.addEventListener("pointerup", async () => {
  ...
  pullActive = false;
  if (liveMode) { hidePull(); return; }
  if (pullDist > 90) await doPullRefresh();
  else hidePull();
  pullDist = 0;
}, { passive:true });
...
try{
  const __origSetMode = setMode;
  setMode = function(isLive){
    __origSetMode(isLive);
    enablePullToRefresh(!liveMode);
  };
} catch {}
...
let fxUsdPerEur = 0;           // USD per 1 EUR
let fxLastTs = 0;
...
const volCache = { "15m": {v:null, ts:0}, "1h": {v:null, ts:0}, "4h": {v:null, ts:0}, "1d": {v:null, ts:0} };
let volTf = "15m";

function toolsOnOpen(){
  // refresh UI quickly when opening the page
  try{ updateToolsUI(); } catch {}
  // background refresh (non-blocking)
  safeAsync(() => toolsRefreshAll(false), "toolsRefreshAll(open)");
}

function toolsUsdPrice(){
  // prefer displayed price (smoothed) but fall back to targetPrice
  return safe(displayed?.price || targetPrice || 0);
}

/* --- FX rate (EUR->USD) --- */
async function fetchFxEurUsd(){
  // Frankfurter updates daily; refresh every 30min anyway
  const now = Date.now();
  if (fxUsdPerEur > 0 && (now - fxLastTs) < 30*60*1000) return fxUsdPerEur;

  // Prefer frankfurter.dev (new) and fallback to frankfurter.app
  const urlA = "https://api.frankfurter.dev/v1/latest?base=EUR&symbols=USD";
  const urlB = "https://api.frankfurter.app/latest?from=EUR&to=USD";

  const a = await fetchJSON(urlA);
  const r1 = safe(a?.rates?.USD);
  if (r1 > 0){
    fxUsdPerEur = r1; fxLastTs = now;
    return fxUsdPerEur;
  }
  const b = await fetchJSON(urlB);
  // frankfurter.app format: {amount, base, date, rates:{USD:...}} OR {amount,base,date,rates}
  const r2 = safe(b?.rates?.USD);
  if (r2 > 0){
    fxUsdPerEur = r2; fxLastTs = now;
  }
  return fxUsdPerEur;
}
...
async function fetchFearGreed(){
  ...
  if (fngValue != null && (now - fngTs) < 5*60*1000) return { value:fngValue, cls:fngClass, ts:fngTs };

  const r = await fetchJSON("https://api.alternative.me/fng/?limit=1&format=json");
  const row = r?.data?.[0];
  const v = row ? parseInt(row.value, 10) : NaN;
  if (Number.isFinite(v)){
    fngValue = clamp(v,0,100);
    fngClass = String(row.value_classification || "");
    // timestamp is seconds
    fngTs = safe(row.timestamp) ? safe(row.timestamp) * 1000 : now;
  }
  return { value:fngValue, cls:fngClass, ts:fngTs };
}

/* --- Volatility (realized) --- */
function realizedVolFromCloses(closes){
  const arr = closes.map(x => safe(x)).filter(x => x > 0);
  if (arr.length < 3) return 0;
  const rets = [];
  for (let i=1;i<arr.length;i++){
    rets.push(Math.log(arr[i]/arr[i-1]));
  }
  const n = rets.length;
  ...
  // annualize for comparability (approx minutes -> year)
  const ann = sd * Math.sqrt(525600); // minutes in year
  return ann;
}

async function fetchVol(tfKey){
  const now = Date.now();
  const cached = volCache[tfKey];
  const ttl = 2*60*1000; // 2 minutes
  if (cached?.v != null && (now - cached.ts) < ttl) return cached.v;

  // choose minutes window
  const minutes = tfKey === "15m" ? 15 : tfKey === "1h" ? 60 : tfKey === "4h" ? 240 : 1440;
  const limit = Math.min(1500, minutes + 2);

  // Binance 1m klines (close is index 4)
  const url = `https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=1m&limit=${limit}`;
  const kl = await fetchJSON(url);
  const closes = Array.isArray(kl) ? kl.map(k => k?.[4]) : [];
  const vol = realizedVolFromCloses(closes);

  volCache[tfKey] = { v: vol, ts: now };
  return vol;
}
...
function updateConverterUI(){
  const eurIn = $("convEur");
  const outUsd = $("convUsd");
  const outInj = $("convInj");
  const mini = $("convMini");
  if (!eurIn || (!outUsd && !outInj && !mini)) return;

  const eur = safe(String(eurIn.value || "").replace(",", "."));
  const px = toolsUsdPrice();

  const fx = fxUsdPerEur || 0;
  const usd = (eur > 0 && fx > 0) ? eur * fx : 0;
  const inj = (usd > 0 && px > 0) ? (usd / px) : 0;

  if (outUsd) outUsd.textContent = (eur>0 && fx>0) ? `$${usd.toFixed(2)}` : "—";
  if (outInj) outInj.textContent = (eur>0 && fx>0 && px>0) ? `${inj.toFixed(4)} INJ` : "—";

  if (mini){
    const fxTxt = fx>0 ? `1€ = $${fx.toFixed(4)}` : "Rate: —";
    const pxTxt = px>0 ? `INJ: $${px.toFixed(4)}` : "INJ: —";
    mini.textContent = `${fxTxt} • ${pxTxt}`;
  }
}

function updateFngUI(){
  const vEl = $("fngValue");
  const cEl = $("fngClass");
  const nEl = $("fngNeedle");
  const uEl = $("fngUpdated");
  if (!vEl && !cEl && !nEl) return;

  const v = (fngValue == null) ? null : clamp(Number(fngValue),0,100);
  if (vEl) vEl.textContent = (v==null) ? "—" : String(v);
  if (cEl) cEl.textContent = (v==null) ? "—" : (fngClass || "");
  if (nEl) nEl.style.left = (v==null) ? "50%" : (v + "%");
  if (uEl){
    uEl.textContent = fngTs ? (`Updated: ${new Date(fngTs).toLocaleString()}`) : "—";
  }
}

function volToPct(vol){
  // Map annualized vol to 0..100 gauge (heuristic):
  // 0.2 -> ~20, 0.6 -> ~50, 1.2 -> ~80, 2.0 -> 100
  const v = safe(vol);
  const pct = clamp((v / 2.0) * 100, 0, 100);
  return pct;
}
...
function updateVolUI(){
  ...
  const val = $("volValue");
  const upd = $("volUpdated");
  const v = volCache?.[volTf]?.v;

  if (fill) fill.style.width = (v==null) ? "0%" : (volToPct(v).toFixed(1) + "%");
  if (needle) needle.style.left = (v==null) ? "0%" : (volToPct(v).toFixed(1) + "%");
  if (val) val.textContent = (v==null) ? "—" : `${(safe(v)*100).toFixed(1)}%`;
  if (upd) upd.textContent = volCache?.[volTf]?.ts ? (`Updated: ${new Date(volCache[volTf].ts).toLocaleString()}`) : "—";
}
...
/* Refresh all tool feeds */
async function toolsRefreshAll(force=false){
  try{ await fetchFxEurUsd(); } catch {}
  try{ await fetchFearGreed(); } catch {}
  try{ await fetchVol(volTf); } catch {}
  updateToolsUI();
}

/* Converter input listener */
$("convEur")?.addEventListener("input", () => updateConverterUI(), { passive:true });

/* Vol timeframe buttons */
$("volTfSwitch")?.addEventListener("click", (e) => {
  const b = e.target?.closest("button[data-vtf]");
  if (!b) return;
  const tf = b.dataset.vtf;
  if (!tf) return;
  volTf = tf;
  // toggle active
  const btns = $("volTfSwitch")?.querySelectorAll("button") || [];
  btns.forEach(x => x.classList.toggle("active", x.dataset.vtf === volTf));
  safeAsync(async () => { await fetchVol(volTf); updateVolUI(); }, "fetchVol(tf)");
}, { passive:true });

/* Light periodic refresh (only while app open) */
setInterval(() => {
  // Update converter as price moves
  try{ updateConverterUI(); } catch {}

  // Only fetch remote data if Tools page is visible OR user is in dashboard (low cost)
  const toolsVisible = pageTools?.classList.contains("active");
  if (toolsVisible){
    safeAsync(() => toolsRefreshAll(false), "toolsRefreshAll(interval)");
  }
}, 20_000);
