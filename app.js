...
function syncStakeTimelineUI(forceToEnd=false){
  ...
  if (stakeChart){
    stakeChart.options.scales.x.min = minIdx;
    stakeChart.options.scales.x.max = maxIdx;
    stakeChart.update("none");
  }

  const fromTs = labelToTs(stakeLabels[minIdx]);
  const toTs   = labelToTs(stakeLabels[maxIdx]);
  const from = fromTs ? fmtDTShort(fromTs) : (stakeLabels[minIdx] || "");
  const to   = toTs ? fmtDTShort(toTs) : (stakeLabels[maxIdx] || "");
  meta.textContent = n <= 1 ? `${to}` : `${from} → ${to}`;
}

$("stakeTimeline")?.addEventListener("input", () => {
  stakeFollow = false;
...
$("stakeLiveBtn")?.addEventListener("click", () => {
  stakeFollow = true;
  const slider = $("stakeTimeline");
  if (slider) slider.value = String(Math.max(0, stakeData.length - 1));
...
/* === Net Worth chart view (market-synced to INJ price chart) ===
   We render Net Worth as: (Total INJ owned) × (INJUSDT market series)
   so the curve matches Injective's chart shape 1:1, but on USD countervalue.
*/
const NW_MKT_VIEW_TTL_MS = 60_000; // refresh per-TF at most once/min
const nwMktCache = Object.create(null);    // tf -> { labels, data, injAmt, builtAt }
const nwMktInflight = Object.create(null); // tf -> Promise

function nwFirstTs(){
  const t0 = safe(nwTAll?.[0]);
  return (Number.isFinite(t0) && t0 > 0) ? t0 : 0;
}
function nwMarketInjAmount(){
  const cur = safe(availableInj) + safe(stakeInj) + safe(rewardsInj);
  if (Number.isFinite(cur) && cur > 0) return cur;

  const last = safe(nwInjAll?.[nwInjAll.length - 1]);
  if (Number.isFinite(last) && last > 0) return last;

  const lastUsd = safe(nwUsdAll?.[nwUsdAll.length - 1]);
  const px = safe(displayed?.price);
  if (Number.isFinite(lastUsd) && lastUsd > 0 && Number.isFinite(px) && px > 0) return lastUsd / px;

  return 0;
}
function nwTfParams(tf, now){
  let interval = "1m";
  let start = 0;
  let maxTotal = 1440;
  ...
  if (tf === "live"){
    ...
    interval = "1w";
    start = Math.max(0, nwFirstTs() || 0);
    maxTotal = 1000;
  }
  return { interval, start, maxTotal };
}

function nwBuildLocalView(tf){
  const now = Date.now();
  ...
  // local series (best-effort, offline-friendly)
  for (let i = 0; i < nwTAll.length; i++){
    const t = safe(nwTAll[i]);
    const u = safe(nwUsdAll[i]);
...
async function ensureNWMarketView(tf){
  if (!address) return null;
  if (!hasInternet()) return null;

  const now = Date.now();
  const injAmt = nwMarketInjAmount();
  if (!Number.isFinite(injAmt) || injAmt <= 0) return null;

  const cached = nwMktCache[tf];
  if (cached && (now - cached.builtAt) < NW_MKT_VIEW_TTL_MS && Math.abs(cached.injAmt - injAmt) < 1e-6){
    return cached;
  }

  if (nwMktInflight[tf]) return null;

  const { interval, start, maxTotal } = nwTfParams(tf, now);

  nwMktInflight[tf] = (async () => {
    try{
      ...
      const data   = kl.map(k => safe(k[4]) * injAmt);

      nwMktCache[tf] = { labels, data, injAmt, builtAt: Date.now() };

      // redraw immediately if user is on this TF
      if (tf === nwTf) drawNW(true);

      return nwMktCache[tf];
    } catch (e){
      console.warn("[ensureNWMarketView]", e);
      return null;
    } finally {
      try{ delete nwMktInflight[tf]; } catch {}
    }
  })();
...
function nwBuildView(tf){
  ...
  // 1) instant mirror when the INJ price chart is already loaded on the same TF
  try{
    if (injAmt > 0 && chart && priceTf === tf){
      const lbs = chart.data?.labels || [];
      const ds  = chart.data?.datasets?.[0]?.data || [];
      if (Array.isArray(lbs) && Array.isArray(ds) && lbs.length > 1 && ds.length === lbs.length){
        return { labels: lbs.slice(), data: ds.map(v => safe(v) * injAmt) };
      }
    }
  } catch {}

  // 2) cached market view (preferred)
  const c = nwMktCache[tf];
  if (c && Array.isArray(c.labels) && c.labels.length > 1 && (now - c.builtAt) < NW_MKT_VIEW_TTL_MS){
    if (Number.isFinite(injAmt) && injAmt > 0 && Math.abs((c.injAmt || 0) - injAmt) < 1e-6){
      return { labels: c.labels, data: c.data };
    }
  }

  // 3) kick a market fetch in the background (no UI freeze)
  if (Number.isFinite(injAmt) && injAmt > 0){
    safeAsync(() => ensureNWMarketView(tf), "ensureNWMarketView");
  }

  // 4) fallback local view (offline / no data yet)
  return nwBuildLocalView(tf);
}
...
function initNWChart(){
  ...
  }, { idleMs: 4200, endOnTouchEnd: false, endOnLeave: false });
}
...
function drawNW(force=false){
  ...
  updateNWTFButtons();
  // keep readout alive only when user is NOT interacting (prevents snapping back while inspecting)
  if (netWorthChart && $("nwReadout")) {
    const isPinned = !!netWorthChart.$crosshairActive && (netWorthChart.$crosshairIndex != null);
    if (!isPinned) {
...
function resetPerAddressStateUI(){
  ...
  stakeFollow = true;
  try{ setText("stakeLast","—"); setText("stakeTimelineMeta","—"); const s=$("stakeTimeline"); if(s){ s.min=0; s.max=0; s.value=0; } }catch{}
...
function attachCrosshair2(ch, overlayEl, formatter, opts){
  ...
  const o = opts || {};
  const idleMs = (Number.isFinite(+o.idleMs) ? +o.idleMs : 950);
  const endOnLeave = (o.endOnLeave === undefined) ? true : !!o.endOnLeave;
  const endOnTouchEnd = (o.endOnTouchEnd === undefined) ? true : !!o.endOnTouchEnd;
