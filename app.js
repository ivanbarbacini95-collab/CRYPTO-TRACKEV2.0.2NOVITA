...
/* ================= DASHBOARD TICKER ================= */
const DASH_TICKER_POLL_MS = 25_000;
const DASH_TICKER_CACHE_KEY = "inj_dash_ticker_cache_v1";
const DASH_TICKER_STALE_MS = 6 * 60 * 60 * 1000; // 6h

const dashTickerEl = $("dashTicker");
const dashTickerTrack = $("dashTickerTrack");

const dashTickerCfg = [
  { key:"mcap",     label:"Market cap",   kind:"usd_compact", positiveOnly:true },
  { key:"vol24",    label:"Volume 24h",   kind:"usd_compact", positiveOnly:true },
  { key:"btc_dom",  label:"BTC dom",      kind:"pct2",        positiveOnly:false },
  { key:"btc",      label:"BTC",          kind:"usd_price0",  positiveOnly:true },
  { key:"eth",      label:"ETH",          kind:"usd_price0",  positiveOnly:true },
  { key:"inj",      label:"INJ",          kind:"usd_price2",  positiveOnly:true },
  { key:"inj_low",  label:"INJ low 24h",  kind:"usd_price2",  positiveOnly:true },
  { key:"inj_high", label:"INJ high 24h", kind:"usd_price2",  positiveOnly:true },
  { key:"inj_vol",  label:"INJ vol 24h",  kind:"usd_compact", positiveOnly:true },
];

const dashTickerRaw = Object.create(null);
const dashTickerPrev = Object.create(null);
const dashTickerFmt = Object.create(null);
...
function fmtUSDCompact(n){
  ...
  const sign = n < 0 ? "-" : "";
  let unit = "", div = 1;
  if (av >= 1e12){ unit = "T"; div = 1e12; }
  else if (av >= 1e9){ unit = "B"; div = 1e9; }
  else if (av >= 1e6){ unit = "M"; div = 1e6; }
  else if (av >= 1e3){ unit = "K"; div = 1e3; }
...
function dashTickerFormat(kind, n){
  if (kind === "usd_compact") return fmtUSDCompact(n);
  if (kind === "usd_price0") return fmtUSD(n, 0);
  if (kind === "usd_price2") return fmtUSD(n, 2);
  if (kind === "pct2") return fmtPct(n, 2);
  return String(n ?? "—");
}

function dashTickerBuild(){
  if (!dashTickerTrack) return;
  const sep = '<span class="tk-sep">•</span>';
  const one = dashTickerCfg.map(c =>
    `<div class="tk"><span class="tk-k">${c.label}</span><span class="tk-v neu" data-tk="${c.key}">—</span></div>`
  ).join(sep);
  dashTickerTrack.innerHTML = one + sep + one;

  // show strip (even if some values are cached)
  if (dashTickerEl){
    dashTickerEl.setAttribute("aria-hidden","false");
    dashTickerEl.style.display = "";
  }

  // set duration based on width (keeps constant speed)
  requestAnimationFrame(() => {
    try{
      const half = dashTickerTrack.scrollWidth / 2;
      const speed = 70; // px/s
      const dur = Math.max(18, Math.min(70, half / speed));
      dashTickerTrack.style.setProperty("--ticker-dur", `${Math.round(dur)}s`);
    }catch{}
  });
}

function dashTickerColorText(el, ns, os, dir){
  if (!el) return;
  ns = String(ns ?? "—");
  os = String(os ?? "");
  if (ns === os) { el.textContent = ns; return; }
  ...
  const upCol = "#22c55e";
  const dnCol = "#ef4444";
  const diffCol = (dir > 0) ? upCol : ((dir < 0) ? dnCol : baseCol);

  const out = [];
  for (let i = 0; i < ns.length; i++){
    const c = ns[i];
    const oc = os[i];
    const col = (c !== oc) ? diffCol : baseCol;
    out.push(`<span style="color:${col}">${c}</span>`);
  }
  el.innerHTML = out.join("");
}

function dashTickerSet(key, raw, kind, positiveOnly){
  const v = Number(raw);
  if (!Number.isFinite(v)) return false;
  if (positiveOnly && v <= 0) return false;

  dashTickerPrev[key] = dashTickerRaw[key];
  dashTickerRaw[key] = v;
  dashTickerFmt[key] = dashTickerFormat(kind, v);
  return true;
}

function dashTickerRender(){
  if (!dashTickerTrack) return;

  for (const c of dashTickerCfg){
    const ns = dashTickerFmt[c.key];
    if (!ns) continue;

    const prev = dashTickerPrev[c.key];
    const cur  = dashTickerRaw[c.key];
    const dir = (Number.isFinite(prev) && Number.isFinite(cur)) ? (cur - prev) : 0;

    dashTickerTrack.querySelectorAll(`[data-tk="${c.key}"]`).forEach((el) => {
      const os = el.dataset.prev || "";
      dashTickerColorText(el, ns, os, dir);
      el.dataset.prev = ns;

      el.classList.toggle("pos", dir > 0);
      el.classList.toggle("neg", dir < 0);
      el.classList.toggle("neu", !(dir > 0 || dir < 0));
    });
  }
}

function dashTickerSaveCache(){
  try{
    const payload = { t: Date.now(), raw: { ...dashTickerRaw }, fmt: { ...dashTickerFmt } };
    localStorage.setItem(DASH_TICKER_CACHE_KEY, JSON.stringify(payload));
  }catch{}
}
function dashTickerLoadCache(){
  try{
    const raw = localStorage.getItem(DASH_TICKER_CACHE_KEY);
    if (!raw) return;
    const obj = JSON.parse(raw);
    if (!obj || !obj.t) return;
    if ((Date.now() - obj.t) > DASH_TICKER_STALE_MS) return;

    if (obj.raw) Object.assign(dashTickerRaw, obj.raw);
    if (obj.fmt) Object.assign(dashTickerFmt, obj.fmt);
    dashTickerRender();
  }catch{}
}
...
async function fetchGlobalStats(){
  // primary: coinlore
  try{
    const g = await fetchJSON("https://api.coinlore.net/api/global/");
    if (Array.isArray(g) && g[0]){
      return {
        mcap: safe(+g[0].total_mcap),
        vol24: safe(+g[0].total_volume),
        btc_dom: safe(+g[0].btc_d)
      };
    }
  }catch{}
  // fallback: coingecko
  try{
    const cg = await fetchJSON("https://api.coingecko.com/api/v3/global");
    if (cg && cg.data){
      return {
        mcap: safe(cg.data.total_market_cap?.usd),
        vol24: safe(cg.data.total_volume?.usd),
        btc_dom: safe(cg.data.market_cap_percentage?.btc)
      };
    }
  }catch{}
  return null;
...
async function fetchBinance24h(symbol){
  ...
  if (!d) return null;
  return {
    last: safe(+d.lastPrice),
    high: safe(+d.highPrice),
    low: safe(+d.lowPrice),
    volQuote: safe(+d.quoteVolume),
    chgPct: safe(+d.priceChangePercent)
  };
}

async function dashTickerRefresh(){
  if (!dashTickerTrack || !hasInternet()) return;

  try{
    const [g, btc, eth, inj] = await Promise.all([
      fetchGlobalStats(),
      fetchBinancePrice("BTCUSDT").catch(() => null),
      fetchBinancePrice("ETHUSDT").catch(() => null),
      fetchBinance24h("INJUSDT").catch(() => null),
    ]);

    if (g){
      dashTickerSet("mcap", g.mcap, "usd_compact", true);
      dashTickerSet("vol24", g.vol24, "usd_compact", true);
      dashTickerSet("btc_dom", g.btc_dom, "pct2", false);
    }
    if (btc) dashTickerSet("btc", btc, "usd_price0", true);
    if (eth) dashTickerSet("eth", eth, "usd_price0", true);

    if (inj){
      dashTickerSet("inj", inj.last, "usd_price2", true);
      dashTickerSet("inj_low", inj.low, "usd_price2", true);
      dashTickerSet("inj_high", inj.high, "usd_price2", true);
      dashTickerSet("inj_vol", inj.volQuote, "usd_compact", true);
    }

    dashTickerRender();
    dashTickerSaveCache();
  }catch(err){
    // Never reset to 0 on errors: keep last good values
    console.warn("[dashTicker]", err);
  }
}

let dashTickerTimer = null;
function startDashTicker(){
  if (!dashTickerTrack) return;
  dashTickerBuild();
  dashTickerLoadCache();
  dashTickerRefresh();
  if (dashTickerTimer) clearInterval(dashTickerTimer);
  dashTickerTimer = setInterval(dashTickerRefresh, DASH_TICKER_POLL_MS);
}
...
const headerCopyBtn = $("headerCopyBtn");
...
function setCopyButtonState(ok, whichId){
  const ids = whichId ? [whichId] : ["headerCopyBtn","copyAddrBtn"];
  for (const id of ids){
    const btn = $(id);
    if (!btn) continue;
...
function setAddressDisplay(addr) {
  ...
  const btn = headerCopyBtn || $("headerCopyBtn");
  if (!addr) {
    addressDisplay.innerHTML = "";
    if (btn){ btn.classList.remove("show"); btn.disabled = true; }
    return;
  }
  addressDisplay.innerHTML = `<span class="tag"><strong>Wallet:</strong> ${shortAddr(addr)}</span>`;
  if (btn){ btn.classList.add("show"); btn.disabled = false; }
}
...
["headerCopyBtn","copyAddrBtn"].forEach((id) => {
