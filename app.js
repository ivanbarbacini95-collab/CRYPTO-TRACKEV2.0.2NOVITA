...
/* v9.0 — automatically fit long dashboard values on phones without clipping */
(() => {
...
/* v9.4 — stable INJ Price Calculator + USD reward forecast */
(() => {
  const simulatorIds = new Set([
    'simCurrentPrice','simCurrentMarketCap','simPriceMultiple','simPotentialPct',
    'simulatedInjPrice','simulatedWalletInj','simulatedWalletCurrentUsd',
    'simulatedWalletUsd','simulatedWalletGain','simMarketCapFormatted',
    'walletSupplyShare','walletStakedShare','simulatorSupplyUsed'
  ]);

  /* The simulator updates whenever market price changes. Its values remain live,
     but they are painted directly so the whole card never pulses or rolls. */
  if(typeof ROLLING_IDS !== 'undefined') simulatorIds.forEach(id=>ROLLING_IDS.delete(id));

  const previousSignal = typeof signalDashboardCard === 'function' ? signalDashboardCard : null;
  signalDashboardCard = function(id,numericValue){
    const el=document.getElementById(id);
    if(el?.closest('.price-simulator-card')){
      if(Number.isFinite(Number(numericValue))) el.dataset.frameValue=String(Number(numericValue));
      return;
    }
    return previousSignal?.(id,numericValue);
  };

  const clearSimulatorPulse=()=>{
    const card=document.querySelector('#dashboardView .price-simulator-card');
    if(!card) return;
    card.classList.remove(
      'data-pulse','data-pulse-up','data-pulse-down','live-data-up','live-data-down',
      'value-glow-up','value-glow-down','motion-enter'
    );
    card.classList.add('motion-visible');
  };

  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',clearSimulatorPulse,{once:true});
  }else clearSimulatorPulse();

  const simulatorCard=document.querySelector('#dashboardView .price-simulator-card');
  if(simulatorCard){
    new MutationObserver(clearSimulatorPulse).observe(simulatorCard,{attributes:true,attributeFilter:['class']});
  }
})();
