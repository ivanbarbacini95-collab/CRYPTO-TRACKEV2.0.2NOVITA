...
function setAddressDisplay(addr) {
  ...
  addressDisplay.innerHTML = `
    <span class="tag">
      <strong>Wallet:</strong> ${shortAddr(addr)}
      <button class="wallet-copy-btn" id="walletCopyBtn" type="button" aria-label="Copy address" title="Copy address">⧉</button>
    </span>`;
}
...
// Wallet copy (delegated, because the button is injected in setAddressDisplay)
document.addEventListener("click", async (e) => {
  const btn = e.target?.closest?.("#walletCopyBtn");
  if (!btn) return;
  e?.preventDefault?.();
...
const TICKER_SPEED_PX_PER_S = 36; // smooth default

let tickerRAF = 0;
let tickerLastTs = 0;
let tickerX = 0;
let tickerHalfW = 0;
let tickerLastVals = Object.create(null);

function tickerDir(key, next){
  const prev = Number.isFinite(+tickerLastVals[key]) ? +tickerLastVals[key] : NaN;
  tickerLastVals[key] = next;
  if (!Number.isFinite(prev) || !Number.isFinite(next)) return "flat";
...
function renderTicker(items){
  ...
  const html = items.map(it => {
    ...
    return `<div class="ticker-item">
      ...
      <span class="ticker-arrow ${it.dir}">${arrow}</span>
  ...
  requestAnimationFrame(() => {
    ...
    tickerHalfW = w > 0 ? (w / 2) : 0;
    if (tickerHalfW > 0) tickerX = tickerX % tickerHalfW;
    host.removeAttribute("aria-hidden");
  });
}

function initTickerPlaceholder(){
  // ensures the bar is visible immediately (even before the first network response)
  renderTicker([
    { label:"BTC DOM", dir:"flat", value:"—" },
    { label:"MKT CAP", dir:"flat", value:"—" },
    { label:"VOL 24H", dir:"flat", value:"—" },
    { label:"INJ", dir:"flat", value:"—" },
  ]);
  startTicker();
}
...
function startTicker(){
  ...
  const step = (ts) => {
    ...
    const dt = Math.min(0.06, (ts - tickerLastTs) / 1000); // clamp to avoid big jumps
    tickerLastTs = ts;

    if (tickerHalfW > 0){
      tickerX -= TICKER_SPEED_PX_PER_S * dt;
...
async function updateMarketTicker(force=false){
  if (!hasInternet()){
    // keep the last rendered content; just keep animation running
    startTicker();
    return;
  }

  // Global crypto stats (CoinGecko)
  let dom = NaN, cap = NaN, vol = NaN;
  try{
    const g = await fetchJSON("https://api.coingecko.com/api/v3/global");
    const d = g?.data;
    dom = safe(d?.market_cap_percentage?.btc);
    cap = safe(d?.total_market_cap?.usd);
    vol = safe(d?.total_volume?.usd);
  }catch{}

  // fallback to last good values if a request fails
  if (!Number.isFinite(dom)) dom = Number.isFinite(+tickerLastVals.btc_dom) ? +tickerLastVals.btc_dom : NaN;
  if (!Number.isFinite(cap)) cap = Number.isFinite(+tickerLastVals.mkt_cap) ? +tickerLastVals.mkt_cap : NaN;
  if (!Number.isFinite(vol)) vol = Number.isFinite(+tickerLastVals.vol_24h) ? +tickerLastVals.vol_24h : NaN;

  // INJ daily OHLC (Binance) + current price (targetPrice)
  let injOpen = NaN, injHigh = NaN, injLow = NaN;
  try{
    const k = await fetchJSON("https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=1d&limit=1");
  ...
  if (!Number.isFinite(injOpen)) injOpen = Number.isFinite(+tickerLastVals.inj_open) ? +tickerLastVals.inj_open : NaN;
  if (!Number.isFinite(injHigh)) injHigh = Number.isFinite(+tickerLastVals.inj_high) ? +tickerLastVals.inj_high : NaN;
  if (!Number.isFinite(injLow))  injLow  = Number.isFinite(+tickerLastVals.inj_low)  ? +tickerLastVals.inj_low  : NaN;

  const injPx = safe(targetPrice);

  // Build items (keep last known value if API blips)
  const items = [];
  if (Number.isFinite(dom) && dom > 0){
    items.push({ label:"BTC DOM", dir: tickerDir("btc_dom", dom), value: `${dom.toFixed(2)}%` });
  }
  if (Number.isFinite(cap) && cap > 0){
    items.push({ label:"MKT CAP", dir: tickerDir("mkt_cap", cap), value: `$${fmtSmart(cap)}` });
  }
  if (Number.isFinite(vol) && vol > 0){
    items.push({ label:"VOL 24H", dir: tickerDir("vol_24h", vol), value: `$${fmtSmart(vol)}` });
  }
  if (Number.isFinite(injOpen) && injOpen > 0){
    const d = Number.isFinite(injPx) && injPx > 0 ? (injPx - injOpen) : NaN;
    const dir = Number.isFinite(d) ? (d>0?"up":d<0?"down":"flat") : "flat";
    items.push({ label:"INJ OPEN", dir, value: `$${injOpen.toFixed(4)}` });
    tickerLastVals.inj_open = injOpen;
  }
  if (Number.isFinite(injHigh) && injHigh > 0){
    items.push({ label:"INJ HIGH", dir: "flat", value: `$${injHigh.toFixed(4)}` });
    tickerLastVals.inj_high = injHigh;
  }
  if (Number.isFinite(injLow) && injLow > 0){
    items.push({ label:"INJ LOW", dir: "flat", value: `$${injLow.toFixed(4)}` });
    tickerLastVals.inj_low = injLow;
  }
  if (Number.isFinite(injPx) && injPx > 0){
    items.push({ label:"INJ", dir: tickerDir("inj_px", injPx), value: `$${injPx.toFixed(4)}` });
  }
...
window.addEventListener("resize", () => {
  // recompute width without restarting animation
  const track = $("tickerTrack");
  if (!track) return;
  const w = track.scrollWidth;
  tickerHalfW = w > 0 ? (w/2) : 0;
...
function startAllTimers(){
  ...
  restSyncTimer = setInterval(() => {
    ...
    safeAsync(() => updateMarketTicker(false), "updateMarketTicker");
  }, REST_SYNC_MS);
  chartSyncTimer = setInterval(() => {
    safeAsync(() => loadChartToday(false), "loadChartToday");
    safeAsync(() => loadNWMarketSeries(false), "loadNWMarketSeries");
  }, CHART_SYNC_MS);
...
/* === Net Worth: market-synced view (same klines as INJ Price chart) ===
   For every timeframe we build Net Worth from INJUSDT klines and multiply by
   current total INJ owned (available + staked + rewards). This guarantees the
   curve matches the INJ market chart 1:1 while showing portfolio countervalue.
*/
const nwMarketCache = new Map(); // tf -> { labels, prices, fetchedAt }
let nwMarketFetching = false;

function nwTotalInjNow(){
  return safe(availableInj) + safe(stakeInj) + safe(rewardsInj);
}

function nwTfParams(tf, now=Date.now()){
  let interval = "1m";
  let start = 0;
  let maxTotal = 1440;
  ...
  return { interval, start, end: now, maxTotal };
}

function nwCacheStaleMs(tf){
  if (tf === "live") return 12_000;
  if (tf === "1d") return 45_000;
  if (tf === "1w") return 5*60_000;
  if (tf === "1m") return 12*60_000;
  if (tf === "1y") return 60*60_000;
  return 4*60*60_000; // all
}

function nwResetViewport(){
  if (!netWorthChart) return;
  try{ netWorthChart.resetZoom?.(); } catch {}
  try{
    if (netWorthChart.options?.scales?.x){
      delete netWorthChart.options.scales.x.min;
      delete netWorthChart.options.scales.x.max;
    }
  }catch{}
  // also clear crosshair pin
  try{
...
async function loadNWMarketSeries(force=false){
  if (!hasInternet()) return false;
  const tf = nwTf;
  const now = Date.now();
  const cached = nwMarketCache.get(tf);
  const stale = nwCacheStaleMs(tf);
  if (!force && cached && (now - safe(cached.fetchedAt)) < stale){
    drawNW(true);
    return true;
  }
  if (nwMarketFetching) return true;
  nwMarketFetching = true;
  try{
    const p = nwTfParams(tf, now);
    const kl = await fetchKlinesRange("INJUSDT", p.interval, p.start, p.end, Math.min(2400, p.maxTotal));
    if (!Array.isArray(kl) || !kl.length) return false;

    const labels = kl.map(k => tsLabel(safe(k?.[0])));
    const prices = kl.map(k => safe(k?.[4]));

    nwMarketCache.set(tf, { labels, prices, fetchedAt: now });
    drawNW(true);
    return true;
  } catch {
    return false;
  } finally {
    nwMarketFetching = false;
  }
}
...
function initNWChart(){
  ...
  const view = (() => {
    const cached = nwMarketCache.get(nwTf);
    if (cached?.labels?.length && cached?.prices?.length){
      const injNow = nwTotalInjNow();
      return { labels: cached.labels, data: cached.prices.map(p => safe(p) * injNow) };
    }
    return nwBuildView(nwTf);
  })();
...
function updateNWTFButtons(){
  ...
  btns.forEach(b => {
    // Always unlocked: the series is market-synced so timeframes are immediately usable.
    const enabled = true;
    b.disabled = false;
...
function drawNW(force=false){
  ...
  if (cached?.labels?.length && cached?.prices?.length){
    ...
    data = cached.prices.map(p => safe(p) * injNow);
  } else {
  ...
  if (netWorthChart && $("nwReadout")) {
    // Do not override while the user is inspecting a point (crosshair active)
    if (!netWorthChart.$crosshairActive){
...
$("nwTfSwitch")?.addEventListener("click", (e) => {
  ...
  nwTf = tf;
  saveNWLocal();
  // Fix: when switching to ALL (or any TF) make sure the chart fills the whole card.
  // Reset zoom/pan viewport so no previous interactions keep a partial window.
  try{ nwResetViewport(); } catch {}
  if (hasInternet()) safeAsync(() => loadNWMarketSeries(true), "loadNWMarketSeries");
  drawNW(true);
}, { passive:true });
...
function attachCrosshair2(ch, overlayEl, formatter, holdMs=950){
  if (!ch || !overlayEl) return;
...
(async function boot() {
  ...
  // Top market ticker (always available)
  try{ initTickerPlaceholder(); }catch{}
  safeAsync(() => updateMarketTicker(true), "updateMarketTicker");
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  // Net Worth market-synced series (same klines as INJ chart)
  try{ await loadNWMarketSeries(true); }catch(e){ console.warn("boot loadNWMarketSeries", e); }
