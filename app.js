...
// Speed multiplier (optional settings UI can control it)
const TICKER_SPEED_KEY = "inj_ticker_speed_mult";
let tickerSpeedMult = (() => {
  const v = parseFloat(localStorage.getItem(TICKER_SPEED_KEY) || "1");
  return clamp(Number.isFinite(v) ? v : 1, 0.50, 1.80);
})();

// Like safe(), but returns NaN instead of 0 (prevents ticker flashing to 0 on transient missing data)
...
function startMarketTickerLoop(){
  ...
  const step = (ts) => {
    ...
    // clamp dt to reduce visible "jumps" on mobile frame drops
...
function tuneTickerSpeed(){
  ...
  const dur = Math.max(22, Math.min(90, half / pxPerSec));
...
function setAddressDisplay(addr) {
  ...
  if (btn){ btn.disabled = false; btn.style.display = "grid"; }
}
...
(async function boot() {
  ...
  // Market ticker (global stats)
  try{ initMarketTicker(); }catch(_){ }
  try{ safeAsync(() => loadMarketGlobal(true), "loadMarketGlobal"); }catch(_){ }
...
function animate() {
  ...
  // Market ticker: keep aligned with current market + avoid heavy DOM work per frame
  try{
    // ensure initialized once the dashboard DOM exists
    initMarketTicker();
    updateMarketInjTargets();

    const nowMt = Date.now();
    animate._mtUiAt = animate._mtUiAt || 0;
    animate._mtFetchAt = animate._mtFetchAt || 0;

    // update UI ~4x/sec (smooth enough, cheap)
    if ((nowMt - animate._mtUiAt) > 260){
      animate._mtUiAt = nowMt;
      updateMarketTickerUI();
    }

    // fetch global market stats every ~25s (guarded inside loadMarketGlobal)
    if ((nowMt - animate._mtFetchAt) > 5000){
      animate._mtFetchAt = nowMt;
