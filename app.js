...
window.addEventListener("online", () => {
  ...
  if (liveMode){
    ...
    // REFRESH mode: do not auto-refresh on reconnect; user triggers manual refresh via pull-to-refresh.
    refreshLoaded = false;
...
const TAB_ADDR_KEY = "inj_tab_address_v1";
const LAST_ADDR_KEY = "inj_last_address";
const WATCHLIST_KEY = "inj_watchlist_v1";
// live | refresh
...
let address = ((sessionStorage.getItem(TAB_ADDR_KEY) || localStorage.getItem("inj_address") || localStorage.getItem(LAST_ADDR_KEY) || "") + "").trim();
...
const pageWatchlist = $("pageWatchlist");
...
function showPage(key){
  ...
  else if (key === "watchlist") pageWatchlist?.classList.add("active");
  else pageDashboard?.classList.add("active");

  // Watchbar only on dashboard + when we have saved addresses
  try{ refreshWatchbarVisibility(); }catch{}
}
...
const watchAddrSaveBtn = $("watchAddrSaveBtn");
const watchAddrList = $("watchAddrList");
const watchAddrError = $("watchAddrError");
...
function loadWatchList(){
  try{
    ...
    for (const x of arr){
      const a = (x || "").trim();
      if (!a || !isValidInjAddress(a)) continue;
...
function setWatchError(msg){
  if (!watchAddrError) return;
  watchAddrError.textContent = msg || "";
}
function addWatchAddress(a){
  const addr = (a || "").trim();
  if (!isValidInjAddress(addr)){
    setWatchError("Invalid address. It must start with inj...");
    return false;
  }
  const list = loadWatchList();
  const exists = list.some(x => x.toLowerCase() === addr.toLowerCase());
  if (exists){
    setWatchError("Already in your list.");
    return false;
  }
  list.unshift(addr);
  saveWatchList(list);
...
function removeWatchAddress(addr){
  const a = (addr || "").trim();
  if (!a) return;
  const list = loadWatchList().filter(x => x.toLowerCase() !== a.toLowerCase());
  saveWatchList(list);
  renderWatchbar();
  renderWatchlistPage();
}
function renderWatchbar(){
  ...
  watchBar.innerHTML = "";
  if (!list.length){
    watchBar.setAttribute("aria-hidden","true");
    return;
  }
  for (const a of list){
    const b = document.createElement("button");
    b.type = "button";
    b.className = "watch-chip" + ((address && a.toLowerCase() === address.toLowerCase()) ? " active" : "");
    b.dataset.addr = a;
    b.title = a;
    b.textContent = maskAddr(a);
    watchBar.appendChild(b);
  }
  refreshWatchbarVisibility();
}
function refreshWatchbarVisibility(){
  if (!watchBar) return;
  const list = loadWatchList();
  const dashActive = pageDashboard?.classList.contains("active");
  watchBar.setAttribute("aria-hidden", (dashActive && list.length) ? "false" : "true");
  try{
    const chips = watchBar.querySelectorAll(".watch-chip");
    chips.forEach(ch => ch.classList.toggle("active", ch.dataset.addr && address && ch.dataset.addr.toLowerCase() === address.toLowerCase()));
  }catch{}
}
watchBar?.addEventListener("click", (e) => {
  const btn = e.target?.closest?.(".watch-chip");
  const a = btn?.dataset?.addr;
  if (!a) return;
  safeAsync(async () => {
    await commitAddress(a);
...
function renderWatchlistPage(){
  if (!watchAddrList) return;
  const list = loadWatchList();
  watchAddrList.innerHTML = "";
  ...
  if (!list.length){
    ...
    watchAddrList.appendChild(empty);
    refreshWatchbarVisibility();
    return;
  }
  ...
  for (const a of list){
    ...
    sel.type = "button";
    sel.className = "watch-btn";
    sel.dataset.addr = a;
    sel.dataset.action = "select";
    sel.textContent = "Select";

    const del = document.createElement("button");
    del.type = "button";
    del.className = "watch-btn danger";
    del.dataset.addr = a;
    ...
    watchAddrList.appendChild(row);
  }

  refreshWatchbarVisibility();
}

watchAddrSaveBtn?.addEventListener("click", (e) => {
  e?.preventDefault?.();
  addWatchAddress(watchAddrInput?.value);
...
watchAddrList?.addEventListener("click", (e) => {
...
function setMode(isLive){
  ...
  if (!liveMode) {
    try{ prSet("Pull to refresh", { show:false, spinning:false, y: -PULL_H }); }catch{}
    stopAllTimers();
    stopAllSockets();
    wsTradeOnline = false;
    wsKlineOnline = false;
    ...
    refreshLoaded = false;
    refreshLoading = false;
    // REFRESH mode: stop automatic updates; user triggers manual refresh via pull-to-refresh.
    refreshConnUI();
  } else {
...
/* ================= PULL_TO_REFRESH_V1 (Refresh mode only) ================= */
const pullRefresh = $("pullRefresh");
const pullText = $("pullText");
const pullSpinner = $("pullSpinner");

const PULL_H = 56;
const PULL_THRESH = 74;
let pullActive = false;
let pullStartY = 0;
let pullDelta = 0;
let pullArmed = false;

function prSet(text, { show=true, spinning=false, y=null } = {}){
  if (!pullRefresh) return;
  if (typeof text === "string" && pullText) pullText.textContent = text;
  if (pullSpinner) pullSpinner.classList.toggle("spinning", !!spinning);

  const ty = (y === null) ? -PULL_H : y;
  pullRefresh.style.transform = `translateY(${ty}px)`;
  pullRefresh.classList.toggle("show", !!show);
  pullRefresh.setAttribute("aria-hidden", show ? "false" : "true");
}
...
async function doManualRefresh(){
  ...
  prSet("Refreshing…", { show:true, spinning:true, y: 0 });
  try{
    // Pull latest cloud points for this address first (merge), then refresh data.
    safeAsync(() => cloudPull(), "cloudPull");
    await refreshLoadAllOnce();
    // Also refresh the INJ price chart explicitly
    try{ await loadPriceChart(true); }catch(e){ console.warn("manual loadPriceChart", e); }
    prSet("Updated", { show:true, spinning:false, y: 0 });
  } finally {
    setTimeout(() => prSet("Pull to refresh", { show:false, spinning:false, y: -PULL_H }), 520);
  }
}

function prOnStart(y){
  if (liveMode) return false;
  if (refreshLoading) return false;
  if (window.scrollY > 0) return false;
  pullActive = true;
  pullStartY = y;
  pullDelta = 0;
  pullArmed = false;
  prSet("Pull to refresh", { show:true, spinning:false, y: -PULL_H + 1 });
  return true;
}
function prOnMove(y, ev){
  if (!pullActive) return;
  pullDelta = y - pullStartY;
  if (pullDelta <= 0){
    prSet("Pull to refresh", { show:false, spinning:false, y: -PULL_H });
    return;
  }
  // prevent native overscroll only while pulling down
  try{ ev?.preventDefault?.(); }catch{}
  const eased = Math.min(PULL_H, Math.round(pullDelta * 0.55));
  pullArmed = pullDelta >= PULL_THRESH;
  prSet(pullArmed ? "Release to refresh" : "Pull to refresh", { show:true, spinning:false, y: -PULL_H + eased });
}
function prOnEnd(){
  if (!pullActive) return;
  pullActive = false;
  ...
  if (pullArmed){
    prSet("Refreshing…", { show:true, spinning:true, y: 0 });
    safeAsync(() => doManualRefresh(), "doManualRefresh");
  } else {
    prSet("Pull to refresh", { show:false, spinning:false, y: -PULL_H });
  }
}
...
window.addEventListener("touchmove", (e) => {
  ...
  const t = e.touches && e.touches[0];
  if (!t) return;
  prOnMove(t.clientY, e);
}, { passive:false });

window.addEventListener("touchend", () => prOnEnd(), { passive:true });
window.addEventListener("touchcancel", () => prOnEnd(), { passive:true });

/* Pointer support (desktop + modern mobile) */
...
async function commitAddress(newAddr) {
  ...
  try{ localStorage.setItem("inj_address", address); }catch{}
...
// cloud pull for this address
  ...
  if (liveMode) await loadAccount();
    ...
    // REFRESH mode: do not auto-load remote data. Use pull-to-refresh to refresh.
    refreshLoaded = false;
    refreshLoading = false;
    modeLoading = false;
    refreshConnUI();
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  const savedAddr = ((sessionStorage.getItem(TAB_ADDR_KEY) || localStorage.getItem("inj_address") || localStorage.getItem(LAST_ADDR_KEY) || "") + "").trim();
const hasSavedAddr = !!savedAddr && /^inj[a-z0-9]{20,80}$/i.test(savedAddr);
  ...
  if (liveMode) {
    ...
    refreshLoaded = false;
    refreshConnUI();
    // REFRESH mode: manual pull-to-refresh only
  }
  try{ renderWatchbar(); }catch{}
