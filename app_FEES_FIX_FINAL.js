...
function loadFeesLocal(){
  ...
  // 1) Try new storage (all tx fees)
  try{
    const raw = localStorage.getItem(k);
    if (raw){
      const obj = JSON.parse(raw);
      if (obj && obj.v === FEES_LOCAL_VER){
        feesIdsAll    = Array.isArray(obj.ids)    ? obj.ids    : [];
        feesLabelsAll = Array.isArray(obj.labels) ? obj.labels : [];
  ...
  // 2) Legacy migration (reward-withdraw only) → keep old points, then continue adding new ones
  try{
    const lk = feesStoreKeyLegacy(address);
    if (!lk) return false;
    const raw2 = localStorage.getItem(lk);
    if (!raw2) return false;
    const obj2 = JSON.parse(raw2);
    if (!obj2) return false;

    const v = Number(obj2.v);
    if (v !== FEES_LEGACY_VER) return false;

    feesIdsAll    = Array.isArray(obj2.ids)    ? obj2.ids    : [];
    feesLabelsAll = Array.isArray(obj2.labels) ? obj2.labels : [];
    feesValuesAll = Array.isArray(obj2.values) ? obj2.values : [];
    feesTimesAll  = Array.isArray(obj2.times)  ? obj2.times  : [];
...
function updateFeesUI(){
  ...
  if (totEl) totEl.textContent = `${total.toFixed(6)} INJ`;
...
async function feesBackfill(force=false){
  ...
  const now = Date.now();
  // keep it reactive after any tx, but avoid hammering the LCD
  if (!force && (now - feesLastSyncAt) < 25_000) return;
  feesLastSyncAt = now;

  const have = new Set(feesIdsAll);
  let added = 0;

  // We fetch txs where THIS address is a signer (i.e., fees paid by this wallet)
  const mkUrl = (ev, nextKey) => {
    const keyPart = nextKey ? `&pagination.key=${encodeURIComponent(nextKey)}` : "";
    return `/cosmos/tx/v1beta1/txs?events=${encodeURIComponent(ev)}&order_by=ORDER_BY_DESC&pagination.limit=50${keyPart}`;
  };

  // Best effort: Injective / Cosmos indexing varies by endpoint/version.
  // 1) tx.signer catches EVERYTHING if available
  // 2) message.sender catches most wallet-signed txs
  // 3) transfer.sender catches bank sends even on some older indexers
  // 4) coin_spent.spender is an extra catch-all for some indexers
  const eventsToTry = [
    `tx.signer='${address}'`,
  ...
  const maxPages = force ? 12 : 3;
  let signerWorked = false;

  for (let evIndex = 0; evIndex < eventsToTry.length; evIndex++){
    const ev = eventsToTry[evIndex];
    ...
    while (true){
      let j = null;
      try{
        j = await fetchLCD(mkUrl(ev, nextKey));
      } catch {
        // invalid key / indexer doesn't support this event key
        break;
      }

      const txs = Array.isArray(j?.txs) ? j.txs : [];
      const rs  = Array.isArray(j?.tx_responses) ? j.tx_responses : [];
      const n   = Math.min(txs.length, rs.length);

      // event supported but empty → go next event
      if (page === 0 && n > 0 && ev.startsWith("tx.signer=")) signerWorked = true;
      if (!n) break;
      ...
      for (let i = 0; i < n; i++){
        ...
        const tx = txs[i];
        const ts = Date.parse(tr.timestamp || "") || 0;

        const fee = txFeeInj(tx);
        if (!Number.isFinite(fee) || fee <= 0) continue;

        feesIdsAll.push(hash);
        feesTimesAll.push(ts || now);
        feesLabelsAll.push(tsLabel(ts || now));
      ...
      // On non-force sync: if we already hit known txs in this page, going deeper is usually unnecessary.
      if (!force && encounteredKnown) break;
    }

    // If tx.signer works, it's already a superset of the other queries → stop here.
    if (signerWorked) break;

    // If we already pulled some data on normal mode, don't hammer all fallbacks.
    if (!force && added > 0) break;
  }
  ...
  if (added){
    ...
    saveFeesLocal();
    rebuildFeesView();
    try { flashGreen($("feesTotal")); flashGreen($("feesTotalUsd")); } catch {}
  }
}
