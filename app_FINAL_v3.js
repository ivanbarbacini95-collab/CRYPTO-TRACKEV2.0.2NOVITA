...
/* ================= TOP LIVE TICKER =================
   Smooth marquee without visible restarts:
   - Two identical segments
   - Fixed-width fields + tabular-nums
   - Animation distance = segment width (set once, resize-only)
==================================================== */
const TICKER_MARKET_MS = 6_000;
const TICKER_MCAP_MS   = 30_000;
const TICKER_GLOBAL_MS = 30_000;
const TICKER_BLOCK_MS  = 4_500;

let __tickerStarted = false;
let __tickerClockT = null, __tickerMarketT = null, __tickerMcapT = null, __tickerGlobalT = null, __tickerBlockT = null;
...
function fmtRomeDateTime(ts = Date.now()){
  try{
    const t = tzParts(ts, ROME_TZ);
    const pad = (n) => String(n).padStart(2, "0");
    return `${t.y}-${pad(t.m)}-${pad(t.d)} ${pad(t.hh)}:${pad(t.mm)}:${pad(t.ss)}`;
  } catch {
    const d = new Date(ts);
    const pad = (n) => String(n).padStart(2, "0");
...
const __tkNum = Object.create(null);
function tkBump(el, dir){
  if (!el) return;
  el.classList.remove("bump-up","bump-down");
  // force reflow to retrigger animation
  void el.offsetWidth;
  if (dir > 0) el.classList.add("bump-up");
  else if (dir < 0) el.classList.add("bump-down");
}

function setTickerText(key, text){
  document.querySelectorAll(`.tk-val[data-k="${key}"]`).forEach(el => {
    if (el.textContent !== text) el.textContent = text;
  });
}

function setTickerMetric(key, num, text, clsOverride=null){
  const els = document.querySelectorAll(`.tk-val[data-k="${key}"]`);
  if (!els.length) return;

  const prev = __tkNum[key];
  let dir = 0;
  if (Number.isFinite(prev) && Number.isFinite(num)){
    dir = num > prev ? 1 : (num < prev ? -1 : 0);
  }
  __tkNum[key] = num;

  const cls = clsOverride || (dir > 0 ? "pos" : (dir < 0 ? "neg" : "neu"));
  els.forEach(el => {
    if (el.textContent !== text) el.textContent = text;
    el.classList.remove("pos","neg","neu");
    el.classList.add(cls);
    if (dir !== 0) tkBump(el, dir);
  });
}

function setTickerSigned(key, signedNum, text){
  const cls = signedNum > 0 ? "pos" : (signedNum < 0 ? "neg" : "neu");
  setTickerMetric(key, signedNum, text, cls);
}

function setTickerChangeClasses(pct){
  // kept for compatibility; use setTickerSigned instead
  const cls = (pct > 0) ? "pos" : (pct < 0 ? "neg" : "neu");
  document.querySelectorAll(`.tk-val[data-k="injChange"]`).forEach(el => {
    el.classList.remove("pos","neg","neu");
    el.classList.add(cls);
  });
}

async function tickerFetchBinance24h(){
  if (!hasInternet()) return;
  try{
    const d = await fetchBinanceJSON("/api/v3/ticker/24hr?symbol=INJUSDT");
    const price = Number(d?.lastPrice);
    const pct   = Number(d?.priceChangePercent);
    const volQ  = Number(d?.quoteVolume); // USDT quote volume
    const high  = Number(d?.highPrice);
    const low   = Number(d?.lowPrice);

    const nbsp = "\u00A0";

    // Price + movement (delta vs previous fetch)
    if (Number.isFinite(price)){
      setTickerMetric("injPrice", price, fmtPriceUSD(price).padStart(10, nbsp));
      // also feed the main INJ Price card
      targetPrice = price;
      if (!displayed.price) displayed.price = price;
      try{ renderTargetsNow(); }catch(e){}
    }

    // 24h change (color by sign)
    setTickerSigned("injChange", pct, fmtPct2(pct).padStart(9, nbsp));

    if (Number.isFinite(high)){
      setTickerMetric("injHigh", high, fmtPriceUSD(high).padStart(10, nbsp));
    }
    if (Number.isFinite(low)){
      setTickerMetric("injLow", low, fmtPriceUSD(low).padStart(10, nbsp));
    }

    if (Number.isFinite(volQ)){
      setTickerMetric("vol24h", volQ, fmtUSDShort(volQ).padStart(12, nbsp));
    }
  } catch {}
}

async function tickerFetchMarketCap(){
  if (!hasInternet()) return;
  const nbsp = "\u00A0";
  let mc = null;

  // 1) CoinGecko (fixed id: injective)
  try{
    const d = await fetchJSON("https://api.coingecko.com/api/v3/simple/price?ids=injective&vs_currencies=usd&include_market_cap=true");
    mc = Number(d?.injective?.usd_market_cap);
  } catch {}

  // 2) CoinPaprika fallback
  if (!Number.isFinite(mc) || mc <= 0){
    try{
      const p = await fetchJSON("https://api.coinpaprika.com/v1/tickers/inj-injective-protocol");
      mc = Number(p?.quotes?.USD?.market_cap);
    } catch {}
  }
  ...
  if (!Number.isFinite(mc) || mc <= 0){
    try{
      const c = await fetchJSON("https://api.coincap.io/v2/assets/injective-protocol");
      mc = Number(c?.data?.marketCapUsd);
    } catch {}
  }

  if (Number.isFinite(mc) && mc > 0){
    setTickerMetric("mcap", mc, fmtUSDShort(mc).padStart(12, nbsp));
  }
}

async function tickerFetchGlobal(){
  if (!hasInternet()) return;
  const nbsp = "\u00A0";
  try{
    const g = await fetchJSON("https://api.coinpaprika.com/v1/global");
    const mcap = Number(g?.market_cap_usd ?? g?.market_cap_usd ?? g?.market_cap);
    const vol  = Number(g?.volume_24h_usd ?? g?.volume_24h_usd ?? g?.volume_24h);
    const dom  = Number(
      g?.bitcoin_dominance_percentage ??
      g?.btc_dominance_percentage ??
      g?.bitcoin_dominance ??
      g?.btc_dominance
    );

    if (Number.isFinite(mcap) && mcap > 0){
      setTickerMetric("gMcap", mcap, fmtUSDShort(mcap).padStart(12, nbsp));
    }
    if (Number.isFinite(vol) && vol > 0){
      setTickerMetric("gVol", vol, fmtUSDShort(vol).padStart(12, nbsp));
    }
    if (Number.isFinite(dom) && dom > 0){
      setTickerMetric("btcDom", dom, fmtPct2NoPlus(dom).padStart(8, nbsp));
    }
  } catch {}
}

async function tickerFetchBlock(){
  try{
    const b = await fetchLCD("/cosmos/base/tendermint/v1beta1/blocks/latest");
    const h = b?.block?.header?.height ?? b?.header?.height;
    if (h != null){
      const hn = Number(h);
      const txt = ("#" + fmtInt(hn)).padStart(12, "\u00A0");
      setTickerMetric("block", hn, txt);
    }
  } catch {}
}

function tickerTickClock(){
  setTickerText("time", fmtRomeDateTime(Date.now()));
}

function setupTickerMarquee(){
  const segA = $("tickerSegA");
  const segB = $("tickerSegB");
  const track = $("tickerTrack");
  if (!segA || !segB || !track) return;

  // Duplicate content once
  if (!segB.innerHTML || segB.innerHTML.trim().length < 10){
    segB.innerHTML = segA.innerHTML;
  }

  const apply = () => {
    const w = segA.getBoundingClientRect().width;
    if (w > 0){
      track.style.setProperty("--segW", `${w}px`);
      const dur = Math.max(18, w / 55); // ~55px/s
  ...
  requestAnimationFrame(() => {
    apply();
    // a second measure helps after fonts settle
    setTimeout(apply, 350);
  });

  window.addEventListener("resize", debounce(apply, 200), { passive: true });
}

function initTopTicker(){
  if (__tickerStarted) return;
  __tickerStarted = true;

  setupTickerMarquee();

  tickerTickClock();
  tickerFetchBinance24h();
  tickerFetchMarketCap();
  tickerFetchGlobal();
  tickerFetchBlock();

  __tickerClockT  = setInterval(tickerTickClock, 1000);
  __tickerMarketT = setInterval(tickerFetchBinance24h, TICKER_MARKET_MS);
  __tickerMcapT   = setInterval(tickerFetchMarketCap, TICKER_MCAP_MS);
  __tickerGlobalT = setInterval(tickerFetchGlobal, TICKER_GLOBAL_MS);
  __tickerBlockT  = setInterval(tickerFetchBlock, TICKER_BLOCK_MS);
}
/* ================= GLOBAL ERROR GUARDS ================= */
...
/* ================= BINANCE MARKET DATA (CORS-SAFE) =================
   Many browsers block https://api.binance.com via CORS.
   Binance provides a "market data only" base endpoint: https://data-api.binance.vision
   We'll use it first, with fallback to the standard api.binance.com.
*/
const BINANCE_REST_BASES = ["https://data-api.binance.vision", "https://api.binance.com"];
let BINANCE_REST_BASE = BINANCE_REST_BASES[0];

function binanceUrl(pathOrUrl){
  if (!pathOrUrl) return "";
  if (/^https?:\/\//i.test(pathOrUrl)){
    return pathOrUrl.replace("https://api.binance.com", BINANCE_REST_BASE);
  }
  const p = pathOrUrl.startsWith("/") ? pathOrUrl : ("/" + pathOrUrl);
  return BINANCE_REST_BASE + p;
}

async function fetchBinanceJSON(pathOrUrl, opts = {}){
  for (const base of BINANCE_REST_BASES){
    const u = /^https?:\/\//i.test(pathOrUrl)
      ? pathOrUrl.replace("https://api.binance.com", base)
      : base + (pathOrUrl.startsWith("/") ? pathOrUrl : ("/" + pathOrUrl));
    const d = await fetchJSON(u, opts);
    if (d !== null && d !== undefined){
      BINANCE_REST_BASE = base;
      return d;
    }
  }
  return null;
...
const addrShortText = $("addrShortText");

function setAddressDisplay(addr) {
  if (!addressDisplay || !addrTag || !addrShortText) return;
  const a = String(addr || "").trim();
  if (!a) { addrTag.style.display = "none"; return; }
  addrTag.style.display = "inline-flex";
  addrShortText.textContent = shortAddr(a);
  addrTag.title = a;
}
...
searchBtn?.addEventListener("click", async (e) => {
  ...
  e?.stopPropagation?.();
  try{
    if (!searchWrap?.classList?.contains("open")) { openSearch(); return; }
    ...
    console.warn("[search click] error", err);
  }
}, { passive: false });
...
// --- Card order migration (one-time) ---
const CARD_ORDER_MIG_KEY = "inj_card_order_mig_v2";
function migrateCardOrderOnce(orderIds){
  try{
    if (localStorage.getItem(CARD_ORDER_MIG_KEY)) return orderIds;
    const out = Array.isArray(orderIds) ? orderIds.slice() : null;
    if (out && out.includes("injPriceCard")){
      // Move INJ Price to top for the new default experience
      const next = ["injPriceCard", ...out.filter(x => x !== "injPriceCard")];
      saveCardOrder(next);
      localStorage.setItem(CARD_ORDER_MIG_KEY, "1");
      return next;
    }
    localStorage.setItem(CARD_ORDER_MIG_KEY, "1");
    return orderIds;
  } catch { return orderIds; }
}
}
...
function startAllTimers(){
  ...
  chartSyncTimer = setInterval(() => {
    safeAsync(() => loadChartToday(false), "loadChartToday");
    if (priceTf === "live" || priceTf === "1d") safeAsync(() => loadPriceChart(false), "loadPriceChart");
  }, CHART_SYNC_MS);
...
async function refreshLoadAllOnce(){
  ...
  try{
    ...
    try{ renderTargetsNow(); }catch(e){ console.warn("refresh renderTargetsNow", e); }
    await loadChartToday(true);
    try{ updatePriceTFButtons(); updatePriceTitle(); }catch(e){}
    try{ await loadPriceChart(true); }catch(e){ console.warn("refresh loadPriceChart", e); }
...
function startPricePoll(){
  try{ if (pricePollTimer) clearInterval(pricePollTimer); } catch {}
  pricePollTimer = null;
  if (!liveMode) return;
  if (!hasInternet()) return;

  pricePollTimer = setInterval(async () => {
    // If trade WS is alive, it will update targetPrice faster than polling
    if (wsTradeOnline) return;
    const d = await fetchBinanceJSON("/api/v3/ticker/price?symbol=INJUSDT");
    const p = safe(d?.price);
    if (p){
      targetPrice = p;
      markLastOk();
    }
...
async function feesBackfill(force=false){
  ...
  // Optional: show source + scanned count in meta line if present
  const meta = $("feesTimelineMeta");
  if (meta && source !== "none"){
    const s = `${source.toUpperCase()} • scanned ${scanned}${added ? ` • +${added}` : ""}`;
...
(async function boot() {
  ...
  applyCardOrder(migrateCardOrderOnce(loadCardOrder()));
  setAddressDisplay(address);
...
safeAsync(() => cloudPull(), "cloudPull");
  ...
  try{ renderTargetsNow(); }catch(e){ console.warn("boot renderTargetsNow", e); }
