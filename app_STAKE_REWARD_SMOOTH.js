...
// --- STAKE anti-spike offset (per-address, keeps chart readable without wiping history) ---
const STAKE_OFFSET_VER = 1;
function stakeOffsetKey(addr){
  const a = (addr || "").trim();
  return a ? `inj_stake_offset_v${STAKE_OFFSET_VER}_${a}` : null;
}
let stakeOffset = 0;        // INJ offset applied ONLY to stake chart series (not to displayed Stake value)
let stakeLastRawSeen = NaN; // last raw stake seen (INJ) to detect sudden jumps

function stakeLoadOffsetLocal(){
  const k = stakeOffsetKey(address);
  if (!k) return false;
  try{
    ...
    if (Number.isFinite(v)) { stakeOffset = v; return true; }
  } catch {}
  return false;
}
function stakeSaveOffsetLocal(){
  const k = stakeOffsetKey(address);
  if (!k) return;
  try{
    localStorage.setItem(k, JSON.stringify({ v: STAKE_OFFSET_VER, ts: Date.now(), offset: stakeOffset }));
  } catch {}
}
// Adaptive spike threshold (INJ) for stake series.
// Big delegate/undelegate jumps flatten the chart; we neutralize only the chart series.
function stakeSpikeThreshold(lastRaw){
  const base = 0.25;        // 0.25 INJ min
  const pct  = 0.004;       // or 0.40% of level
...
function loadStakeSeriesLocal() {
  ...
  try {
    ...
    // load per-address stake offset first
    try{ stakeLoadOffsetLocal(); }catch{}
    // Optional: de-spike historic series once (keeps history, improves readability)
    try{ stakeDeSpikeSeriesOnce(); }catch{}
    // seed raw baseline for spike detection
    try{ if (stakeData.length){ stakeLastRawSeen = safe(stakeData[stakeData.length-1]) + stakeOffset; } }catch{}
...
function stakeDeSpikeSeriesOnce(){
  // Rewrite stakeData in-place (once) to remove historic outliers WITHOUT deleting history.
  // It keeps the overall trend while avoiding a single huge jump flattening the whole curve.
  try{
    if (!Array.isArray(stakeData) || stakeData.length < 3) return false;
    const kflag = stakeOffsetKey(address);
    if (!kflag) return false;
    const flagKey = kflag + "_despiked";
    if (localStorage.getItem(flagKey) === "1") return false;

    let off = safe(stakeOffset); // start from current saved offset (usually 0 at first run)
    let lastRaw = safe(stakeData[0]) + off; // assume first point is baseline
    const out = [ safe(stakeData[0]) ]; // normalized values to rewrite
    for (let i=1;i<stakeData.length;i++){
      const raw = safe(stakeData[i]) + off;
      const d = raw - lastRaw;
      const thr = stakeSpikeThreshold(lastRaw);
      if (Math.abs(d) > thr){
        off += d;            // neutralize jump
      }
      const norm = (safe(stakeData[i]) + safe(stakeOffset)) - off; // keep continuity
      out.push(norm);
      lastRaw = raw;
    }

    // commit rewritten series + new offset
    stakeOffset = off;
    stakeData = out.map(v => Number(safe(v).toFixed(6)));

    // rebuild moves/types based on rewritten series for consistent UI
    stakeMoves = [];
    stakeTypes = [];
    for (let i=0;i<stakeData.length;i++){
      if (i===0){ stakeMoves.push(1); stakeTypes.push("Baseline (current)"); continue; }
      const d = stakeData[i] - stakeData[i-1];
      stakeMoves.push(d >= 0 ? 1 : -1);
      stakeTypes.push(d >= 0 ? "Delegate / Compound" : "Undelegate");
    }

    stakeSaveOffsetLocal();
...
function maybeAddStakePoint(currentStake) {
  const sRaw = safe(currentStake);
  if (!Number.isFinite(sRaw)) return;
  const roundedRaw = Number(sRaw.toFixed(6));

  // Anti-spike normalization for chart series (does NOT change displayed stake value)
  if (Number.isFinite(stakeLastRawSeen)){
    const dRaw = roundedRaw - stakeLastRawSeen;
    const thr = stakeSpikeThreshold(stakeLastRawSeen);
    if (Math.abs(dRaw) > thr){
      stakeOffset += dRaw;
      stakeSaveOffsetLocal();
    }
  }
  stakeLastRawSeen = roundedRaw;

  const rounded = Number((roundedRaw - stakeOffset).toFixed(6));
...
// --- REWARDS chart anti-spike offset (per-address, for withdrawal-amount chart readability) ---
const WD_OFFSET_VER = 1;
function wdOffsetKey(addr){
  const a = (addr || "").trim();
  return a ? `inj_wd_chart_offset_v${WD_OFFSET_VER}_${a}` : null;
}
let wdChartOffset = 0;
function wdLoadChartOffsetLocal(){
  const k = wdOffsetKey(address);
  if (!k) return false;
  try{
    const raw = localStorage.getItem(k);
    if (!raw) return false;
    const obj = JSON.parse(raw);
    const v = safe(obj?.offset);
    if (Number.isFinite(v)) { wdChartOffset = v; return true; }
  }catch{}
  return false;
}
function wdSaveChartOffsetLocal(){
  const k = wdOffsetKey(address);
  if (!k) return;
  try{
    localStorage.setItem(k, JSON.stringify({ v: WD_OFFSET_VER, ts: Date.now(), offset: wdChartOffset }));
  }catch{}
}
function wdSpikeThreshold(lastRaw){
  const base = 0.02;        // 0.02 INJ
  const pct  = 0.08;        // or 8% of typical withdrawal size
  const lvl = Math.abs(safe(lastRaw));
  return Math.max(base, lvl * pct);
}
...
function wdDeSpikeAllOnce(){
  try{
    const k = wdOffsetKey(address);
    if (!k) return false;
    const flagKey = k + "_despiked";
    if (localStorage.getItem(flagKey) === "1") return false;

    if (!Array.isArray(wdValuesAll) || wdValuesAll.length < 3) return false;

    // load current offset (if any)
    try{ wdLoadChartOffsetLocal(); }catch{}
    let off = safe(wdChartOffset);
    let lastRaw = safe(wdValuesAll[0]) + off;
    const out = [ safe(wdValuesAll[0]) ];

    for (let i=1;i<wdValuesAll.length;i++){
      const raw = safe(wdValuesAll[i]) + off;
      const d = raw - lastRaw;
      const thr = wdSpikeThreshold(lastRaw);
      if (Math.abs(d) > thr){
        off += d;
      }
      const norm = (safe(wdValuesAll[i]) + safe(wdChartOffset)) - off;
      out.push(norm);
      lastRaw = raw;
    }

    wdChartOffset = off;
    wdValuesAll = out.map(v => Number(safe(v).toFixed(6)));

    wdSaveChartOffsetLocal();
    saveWdAllLocalSilent();
    localStorage.setItem(flagKey, "1");
    return true;
  }catch{ return false; }
}
...
function rebuildWdView() {
  ...
  for (let i = 0; i < wdValuesAll.length; i++) {
    const vRaw = safe(wdValuesAll[i]);
    const v = safe(vRaw - wdChartOffset);
    if (v >= wdMinFilter) {
...
function tamSpikeThreshold(lastRaw){
  const base = 0.15;                 // minimum 0.15 INJ
  const pct  = 0.0015;               // or 0.15% of level
  const lvl = Math.abs(safe(lastRaw));
  return Math.max(base, lvl * pct);
}

const TAM_NORM_VER = 1;
const TAM_NORM_KEY = `inj_tam_norm_v${TAM_NORM_VER}`;

function tamNormalizeSeriesOnce(){
  // Retroactively remove old spikes without wiping history.
  // We scan the stored series and apply the same offset logic cumulatively.
  try{
    const done = localStorage.getItem(TAM_NORM_KEY);
    if (done) return false;
  } catch {}

  try{
    const n = tamValues.length;
    if (n < 3) { localStorage.setItem(TAM_NORM_KEY, "1"); return false; }

    let off = 0;
    let prevRaw = safe(tamValues[0]);           // treat stored as raw baseline
    let prevNorm = prevRaw;
    ...
    for (let i=1;i<n;i++){
      const raw = safe(tamValues[i]);
      const dRaw = raw - prevRaw;

      const thr = tamSpikeThreshold(prevRaw);
      if (Math.abs(dRaw) > thr){
        off += dRaw; // classify as move; neutralize on chart
      }

      const norm = raw - off;
      newVals[i] = norm;
      newMoves[i] = norm - prevNorm;
    ...
    // Persist computed offset for future real-time adjustments
    tamOffset = off;
    tamLastRawSeen = prevRaw;

    try{ tamSaveOffsetLocal(); }catch{}
    try{ saveTamSeriesLocal(); }catch{}
    try{ localStorage.setItem(TAM_NORM_KEY, "1"); }catch{}
    return true;
  }catch{
    try{ localStorage.setItem(TAM_NORM_KEY, "1"); }catch{}
    return false;
...
(async function boot() {
  ...
  try{ loadTamSeriesLocal(); }catch{}
    try{ tamNormalizeSeriesOnce(); }catch{}
