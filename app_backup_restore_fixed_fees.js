...
async async function feesBackfill(force=false){
  ...
  // Try signer query first; if the chain/index doesn't expose it, we fallback once to message.sender
  let primaryEv = `tx.signer='${address}'`;
  let fallbackTried = false;

  while (true){
    let j = null;
    try{
      j = await fetchLCD(mkUrl(primaryEv, nextKey));
      const txs0 = Array.isArray(j?.txs) ? j.txs : [];
      const rs0  = Array.isArray(j?.tx_responses) ? j.tx_responses : [];

      // if signer returns nothing, fallback to message.sender (once)
      if (page === 0 && txs0.length === 0 && rs0.length === 0 && !fallbackTried){
        fallbackTried = true;
        primaryEv = `message.sender='${address}'`;
        j = await fetchLCD(mkUrl(primaryEv, nextKey));
      }
    } catch {
      return;
